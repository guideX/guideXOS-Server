...
if ([string]::IsNullOrWhiteSpace($ArtifactDirectory)) { $ArtifactDirectory = Join-Path $repoRoot 'out\aarch64-phase3' }
else { $ArtifactDirectory = [IO.Path]::GetFullPath($ArtifactDirectory) }
$esp = Join-Path $ArtifactDirectory 'esp'
if (!(Test-Path -LiteralPath (Join-Path $esp 'EFI\BOOT\BOOTAA64.EFI') -PathType Leaf)) { throw 'Phase 3 ESP is missing BOOTAA64.EFI' }
if (!(Test-Path -LiteralPath (Join-Path $esp 'kernel.elf') -PathType Leaf)) { throw 'Phase 3 ESP is missing kernel.elf' }
...
if ($output -match 'AARCH64_PHASE3_PASS') { exit 0 }
exit 1
