...
Assert-Match -Path $virtioGpuCpp -Pattern 'MAP_FLAG_NON_USER' -Message 'virtio_gpu.cpp should request kernel-only, no-exec, uncached MMIO mappings.'
Assert-Match -Path $virtioGpuCpp -Pattern 'MAP_FLAG_NO_EXEC' -Message 'virtio_gpu.cpp should request kernel-only, no-exec, uncached MMIO mappings.'
Assert-Match -Path $virtioGpuCpp -Pattern 'MAP_FLAG_UNCACHED' -Message 'virtio_gpu.cpp should request kernel-only, no-exec, uncached MMIO mappings.'
...
Assert-Match -Path $probeSmoke -Pattern 'gpuMmioSummaryLine' -Message 'runtime smoke should track the MMIO transport summary line.'
