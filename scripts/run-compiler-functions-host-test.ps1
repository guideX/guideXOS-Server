...
$testBinary = Join-Path $outputDirectory "compiler-functions-host-test.exe"
New-Item -ItemType Directory -Force -Path $outputDirectory | Out-Null

$gxx = (Get-Command g++ -ErrorAction Stop).Source
$sources = @(
    (Join-Path $repoRoot "tests/compiler_functions_host_test.cpp"),
...
if ($LASTEXITCODE -ne 0) { throw "compiler functions host test build failed" }

& $testBinary
if ($LASTEXITCODE -ne 0) { throw "compiler functions host test failed" }
