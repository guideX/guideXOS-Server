...
$version = '0.1.0-phase18-aida-i219-hw-control'

Write-Host "[phase18-iso] Building I219 SPT CTRL_EXT.DRV_LOAD image ($version)" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    throw 'canonical build failed for Phase 18 I219 hardware-control image'
}
...
if ($LASTEXITCODE -ne 0) {
    throw 'release ISO packaging failed for Phase 18 I219 hardware-control image'
}

Write-Host "[phase18-iso] Created guideXOS-Server-v$version-amd64.iso with SHA-256 and manifest" `
