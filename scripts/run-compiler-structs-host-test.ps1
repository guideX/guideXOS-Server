...
$testBinary = Join-Path $outputDirectory "compiler-structs-host-test.exe"
New-Item -ItemType Directory -Force -Path $outputDirectory | Out-Null
$gxx = (Get-Command g++ -ErrorAction Stop).Source
$sources = @(
    (Join-Path $repoRoot "tests/compiler_structs_host_test.cpp"),
...
if ($LASTEXITCODE -ne 0) { throw "compiler structs host test build failed" }
& $testBinary
if ($LASTEXITCODE -ne 0) { throw "compiler structs host test failed" }
