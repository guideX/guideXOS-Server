...
$version = '0.1.0-phase20-aida-i219-post-reset-rearm'

Write-Host "[phase20-iso] Building I219 post-reset rearm image ($version)" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) { throw 'canonical build failed for Phase 20 I219 post-reset rearm image' }
...
if ($LASTEXITCODE -ne 0) { throw 'release ISO packaging failed for Phase 20 I219 post-reset rearm image' }

Write-Host "[phase20-iso] Created guideXOS-Server-v$version-amd64.iso with SHA-256 and manifest" -ForegroundColor Green
