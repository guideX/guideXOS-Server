...
$SmokeLog = Join-Path $LogDir "imageviewer-phase1a-smoke-$stamp.log"
...
try {
    ...
    $doubleClickMatch = Find-FirstMatch -LiteralPath (Join-Path $Root "file_explorer.cpp") -Pattern 'kDoubleClickThresholdMs'

    $report = @(
        "[ImageViewerPhase1ASmoke]",
        "result=PASS",
        ...
        "doubleClickThreshold=$(Format-EvidenceLine $doubleClickMatch)",
