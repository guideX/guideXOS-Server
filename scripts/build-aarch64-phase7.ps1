...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) { $OutputDirectory = Join-Path $repoRoot 'out\aarch64-phase7' }
...
$loaderObject = Join-Path $OutputDirectory 'phase7_loader.obj'
...
$loaderFlags = @(
    '--target=aarch64-pc-windows-msvc', '-DGXOS_AARCH64_PHASE4', '-DGXOS_AARCH64_PHASE5', '-DGXOS_AARCH64_PHASE6', '-DGXOS_AARCH64_PHASE7',
...
$kernelSources = @(
    ...
    'kernel\core\phase7_input_proof.cpp','kernel\core\input_queue.cpp','kernel\core\input_manager.cpp',
    'kernel\core\display_input_mapper.cpp','kernel\core\virtio_input.cpp',
...
$kernelFlags = @(
    ...
    '-DGXOS_AARCH64_PHASE4','-DGXOS_AARCH64_PHASE5','-DGXOS_AARCH64_PHASE6','-DGXOS_AARCH64_PHASE7',
    '-DGXOS_BARE_METAL','-DKERNEL_HAS_VIRTIO_INPUT','-DKERNEL_HAS_COMMON_INPUT_QUEUE',
...
Write-Host '[3/9] Compiling ARM64 interactive desktop and common input sources...' -ForegroundColor Yellow
...
Write-Host '[4/9] Linking Phase 7 kernel.elf...' -ForegroundColor Yellow
...
$appObject = Join-Path $OutputDirectory 'phase7-arm64-proof.o'
$appElf = Join-Path $OutputDirectory 'phase7-arm64-proof.elf'
...
$hostTest = Join-Path $OutputDirectory 'aarch64-phase7-host-tests.exe'
Write-Host '[7/9] Running Phase-7 host negative controls...' -ForegroundColor Yellow
Invoke-Checked $hostCxx @('-std=c++17','-O2','-iquote',(Join-Path $repoRoot 'kernel\core\include'),
    (Join-Path $repoRoot 'tests\aarch64_phase7_host_tests.cpp'),
    (Join-Path $repoRoot 'kernel\core\input_queue.cpp'),
    (Join-Path $repoRoot 'kernel\core\display_input_mapper.cpp'),'-o',$hostTest)
& $hostTest
if ($LASTEXITCODE -ne 0) { throw 'Phase 7 host controls failed' }
...
if ($LASTEXITCODE -ne 0) { throw 'Phase 7 FAT32 ramdisk staging failed' }
...
& $llvmReadobj '--file-headers' '--program-headers' '--sections' '--symbols' $appElf | Set-Content (Join-Path $OutputDirectory 'phase7-app-llvm-readobj.txt') -Encoding ascii
...
Write-Host 'Build complete. Use scripts\test-aarch64-phase7.ps1 for three fresh interactive boots.' -ForegroundColor Green
