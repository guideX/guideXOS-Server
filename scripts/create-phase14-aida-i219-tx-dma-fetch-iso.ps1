...
$version = '0.1.0-phase14-aida-i219-tx-dma-fetch'

Write-Host "[phase14-iso] Building I219 TX DMA/fetch image ($version)" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) { throw 'canonical build failed for Phase 14 I219 TX DMA/fetch image' }
...
if ($LASTEXITCODE -ne 0) { throw 'release ISO packaging failed for Phase 14 I219 TX DMA/fetch image' }

Write-Host "[phase14-iso] Created guideXOS-Server-v$version-amd64.iso with SHA-256 and manifest" -ForegroundColor Green
