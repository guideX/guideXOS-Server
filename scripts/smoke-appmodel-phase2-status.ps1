...
try {
    ...
    if ($IncludeQemu) {
        ...
        try {
            ...
            if ($qemuEvidenceFresh) {
                ...
                $qemuRestoreEvidenceCoverage = Add-QemuEvidenceCheck "qemuRestoreEvidenceCoverage" @{
                    runtimeLaunchBehaviorChanged = "false"
                    persistentDesktopStorageWrites = "false"
                    realBranchDesktopStateRestored = "true"
                    realBranchFileAssociationsStateRestored = "true"
                    realBranchFileAssociationsRestoreVerificationConfirmed = "true"
                    realBranchFolderDesktopStateRestored = "true"
                    realBranchFolderRestoreVerificationConfirmed = "true"
                    realBranchSystemObjectRootFolderDesktopStateRestored = "true"
                    realBranchSystemObjectRootFolderRestoreVerificationConfirmed = "true"
                    realBranchSystemObjectFileManagerDesktopStateRestored = "true"
                    realBranchSystemObjectFileManagerRestoreVerificationConfirmed = "true"
                    realBranchSystemObjectTrashDesktopStateRestored = "true"
                    realBranchSystemObjectTrashRestoreVerificationConfirmed = "true"
                    realBranchSystemObjectSystemSettingsDesktopStateRestored = "true"
                    realBranchSystemObjectSystemSettingsRestoreVerificationConfirmed = "true"
                    realBranchPinnedDesktopNotepadStateRestored = "true"
                    realBranchPinnedDesktopNotepadRestoreVerificationConfirmed = "true"
                    realBranchStartMenuNotepadStateRestored = "true"
                    realBranchStartMenuNotepadRestoreVerificationConfirmed = "true"
                    realBranchStartMenuBuiltInAppsStateRestored = "true"
                    realBranchStartMenuBuiltInAppsRestoreVerificationConfirmed = "true"
                    realBranchStartMenuFilesStateRestored = "true"
                    realBranchStartMenuFilesRestoreVerificationConfirmed = "true"
                    realBranchStartMenuConsoleStateRestored = "true"
                    realBranchStartMenuConsoleRestoreVerificationConfirmed = "true"
                    realBranchStartMenuSettingsStateRestored = "true"
                    realBranchStartMenuSettingsRestoreVerificationConfirmed = "true"
                    realBranchStartMenuRightColumnShellActionsStateRestored = "true"
                    realBranchStartMenuRightColumnShellActionsRestoreVerificationConfirmed = "true"
                    realBranchStartMenuControlPanelStateRestored = "true"
                    realBranchStartMenuControlPanelRestoreVerificationConfirmed = "true"
                    realBranchStartMenuAppModelStateRestored = "true"
                    realBranchStartMenuAppModelRestoreVerificationConfirmed = "true"
                } "runtimeLaunchBehaviorChanged=false persistentDesktopStorageWrites=false and all real-branch restore markers"
                ...
                $qemuRestoreAndInvariantEvidenceConfirmed = $qemuRestoreAndInvariantEvidenceConfirmed -and $qemuTypedDispatchGateEvidence -and $qemuClockParity -and $qemuRestoreEvidenceCoverage
            } else {
    ...
    Add-Check "appmodel.phase3.legacy-labels-preserved" $(if ($phase3LegacyLabelsPreserved) { "PASS" } elseif ($IncludeQemu) { "FAIL" } else { "INFO" }) "QEMU real-branch evidence covers built-ins, TaskManager, DiskManager, Trash, Files, Settings, Console, right-column shell actions, Control Panel, and AppModel"
    ...
    $calculatorTypedDispatchReady =
        ...
        $calculatorRecord.readiness -eq "ready" -and
        $calculatorRecord.dispatchUsage -eq "typed-dispatch" -and
        $calculatorRecord.resolvedType -eq "BuiltInApp" -and
        $calculatorRecord.appId -eq "gxos.builtin.calculator" -and
    ...
    $taskManagerRecord = $phase3Readiness.records | Where-Object { $_.target -eq "TaskManager" } | Select-Object -First 1
    $taskManagerTypedDispatchReady =
        $null -ne $taskManagerRecord -and
        $taskManagerRecord.readiness -eq "ready" -and
        $taskManagerRecord.dispatchUsage -eq "typed-dispatch" -and
        $taskManagerRecord.resolvedType -eq "BuiltInApp" -and
        $taskManagerRecord.appId -eq "gxos.builtin.taskmanager" -and
        $taskManagerRecord.actualDispatch -eq "TaskManager"
    Add-Check "appmodel.v1_3.additionalTypedTarget" $(if ($taskManagerTypedDispatchReady) { "PASS" } else { "FAIL" }) "appmodel.v1_3.additionalTypedTarget=TaskManager TaskManagerTypedDispatchReady=$($taskManagerTypedDispatchReady.ToString().ToLowerInvariant()) TaskManagerBehaviorPreserved=$($taskManagerTypedDispatchReady.ToString().ToLowerInvariant()) target=TaskManager classification=BuiltInApp dispatchDecision=typed-dispatch appId=$($taskManagerRecord.appId) actualDispatch=TaskManager expected=true safe=true reason=normal-built-in-app"

    $trashRecord = $phase3Readiness.records | Where-Object { $_.target -eq "Trash" } | Select-Object -First 1
    $trashTypedDispatchReady =
        $null -ne $trashRecord -and
        $trashRecord.readiness -eq "ready" -and
        $trashRecord.dispatchUsage -eq "typed-dispatch" -and
        $trashRecord.resolvedType -eq "BuiltInApp" -and
        $trashRecord.appId -eq "gxos.builtin.trash" -and
        $trashRecord.actualDispatch -eq "Trash"
    Add-Check "appmodel.v1_4.additionalTypedTarget" $(if ($trashTypedDispatchReady) { "PASS" } else { "FAIL" }) "appmodel.v1_4.additionalTypedTarget=Trash TrashTypedDispatchReady=$($trashTypedDispatchReady.ToString().ToLowerInvariant()) TrashBehaviorPreserved=$($trashTypedDispatchReady.ToString().ToLowerInvariant()) target=Trash classification=BuiltInApp dispatchDecision=typed-dispatch appId=$($trashRecord.appId) actualDispatch=Trash expected=true safe=true reason=normal-built-in-app"

    $diskManagerRecord = $phase3Readiness.records | Where-Object { $_.target -eq "DiskManager" } | Select-Object -First 1
    $diskManagerTypedDispatchReady =
        $null -ne $diskManagerRecord -and
        $diskManagerRecord.readiness -eq "ready" -and
        $diskManagerRecord.dispatchUsage -eq "typed-dispatch" -and
        $diskManagerRecord.resolvedType -eq "BuiltInApp" -and
        $diskManagerRecord.appId -eq "gxos.builtin.diskmanager" -and
        $diskManagerRecord.actualDispatch -eq "DiskManager"
    Add-Check "appmodel.v1_5.additionalTypedTarget" $(if ($diskManagerTypedDispatchReady) { "PASS" } else { "FAIL" }) "appmodel.v1_5.additionalTypedTarget=DiskManager DiskManagerTypedDispatchReady=$($diskManagerTypedDispatchReady.ToString().ToLowerInvariant()) DiskManagerBehaviorPreserved=$($diskManagerTypedDispatchReady.ToString().ToLowerInvariant()) target=DiskManager classification=BuiltInApp dispatchDecision=typed-dispatch appId=$($diskManagerRecord.appId) actualDispatch=DiskManager expected=true safe=true reason=normal-built-in-app"
    ...
    $reportLines += @(
        ...
        "appmodel.v1_1.additionalTypedTarget=Calculator",
        ...
        "appmodel.v1_3.additionalTypedTarget=TaskManager",
        "TaskManagerTypedDispatchReady=$($taskManagerTypedDispatchReady.ToString().ToLowerInvariant())",
        "TaskManagerBehaviorPreserved=$($taskManagerTypedDispatchReady.ToString().ToLowerInvariant())",
        "appmodel.v1_3.taskManagerStatus=target=TaskManager classification=BuiltInApp dispatchDecision=typed-dispatch appId=$($taskManagerRecord.appId) actualDispatch=TaskManager expected=true safe=true reason=normal-built-in-app",
        "appmodel.v1_4.additionalTypedTarget=Trash",
        "TrashTypedDispatchReady=$($trashTypedDispatchReady.ToString().ToLowerInvariant())",
        "TrashBehaviorPreserved=$($trashTypedDispatchReady.ToString().ToLowerInvariant())",
        "appmodel.v1_4.trashStatus=target=Trash classification=BuiltInApp dispatchDecision=typed-dispatch appId=$($trashRecord.appId) actualDispatch=Trash expected=true safe=true reason=normal-built-in-app",
        "appmodel.v1_5.additionalTypedTarget=DiskManager",
        "DiskManagerTypedDispatchReady=$($diskManagerTypedDispatchReady.ToString().ToLowerInvariant())",
        "DiskManagerBehaviorPreserved=$($diskManagerTypedDispatchReady.ToString().ToLowerInvariant())",
