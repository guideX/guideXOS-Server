...
$version = '0.1.0-phase15-aida-i219-tx-engine'

Write-Host "[phase15-iso] Building I219 TX engine image ($version)" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) { throw 'canonical build failed for Phase 15 I219 TX engine image' }
...
if ($LASTEXITCODE -ne 0) { throw 'release ISO packaging failed for Phase 15 I219 TX engine image' }

Write-Host "[phase15-iso] Created guideXOS-Server-v$version-amd64.iso with SHA-256 and manifest" -ForegroundColor Green
