...
if ([string]::IsNullOrWhiteSpace($ArtifactDirectory)) { $ArtifactDirectory = Join-Path $repoRoot 'out\aarch64-phase11' }
$ArtifactDirectory = [IO.Path]::GetFullPath($ArtifactDirectory)
if ($BuildFirst) {
    $buildArgs = @('-LlvmRoot', $LlvmRoot, '-OutputDirectory', $ArtifactDirectory)
    if ($SkipDeveloperStudio) { $buildArgs += '-SkipDeveloperStudio' }
    & (Join-Path $PSScriptRoot 'build-aarch64-phase11.ps1') @buildArgs
    if ($LASTEXITCODE -ne 0) { throw 'Phase-11 build failed before QEMU launch' }
}
$esp = Join-Path $ArtifactDirectory 'esp'
foreach ($required in @('EFI\BOOT\BOOTAA64.EFI','kernel.elf','ramdisk.img')) {
    if (!(Test-Path -LiteralPath (Join-Path $esp $required) -PathType Leaf)) { throw "Phase 11 ESP is missing $required" }
}
...
if ([string]::IsNullOrWhiteSpace($FirmwareVars)) { $FirmwareVars = Join-Path $ArtifactDirectory 'edk2-aarch64-phase11-vars.fd' }
...
if ([string]::IsNullOrWhiteSpace($LogPath)) { $LogPath = Join-Path $ArtifactDirectory 'qemu-aarch64-phase11.log' }
...
while ([DateTime]::UtcNow -lt $deadline) {
    ...
    if ($output -match 'AARCH64_PHASE11_END_TO_END=PASS|AARCH64_PHASE11_END_TO_END=FAIL|AARCH64_PHASE11_PASS|AARCH64_PHASE11_ERROR|\[guideXOS\].*(FATAL|ERROR)') { break }
...
if ($output -match 'AARCH64_PHASE11_END_TO_END=PASS|AARCH64_PHASE11_PASS') { exit 0 }
exit 1
