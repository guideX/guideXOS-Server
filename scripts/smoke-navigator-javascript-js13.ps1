...
$outputDir = Join-Path $env:TEMP "guidex-navigator-javascript-js13-test"
New-Item -ItemType Directory -Force -Path $outputDir | Out-Null
$exe = Join-Path $outputDir "navigator_javascript_js13_test.exe"
...
$compileOutput = & g++ -std=c++17 -O2 -Wall -Wextra -Werror -pedantic `
    ...
    tests/navigator_javascript_js13_test.cpp `
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "Navigator JavaScript JS13 focused test build failed."
}

& $exe
if ($LASTEXITCODE -ne 0) { throw "Navigator JavaScript JS13 focused tests failed." }
...
if ($LASTEXITCODE -ne 0) {
    $strict | Write-Output
    throw "Navigator JavaScript JS13 strict adapter/runtime build failed."
}

Write-Output "Navigator JavaScript JS13 focused smoke PASS"
