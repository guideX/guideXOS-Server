...
{
    log(ctx, "DEVELOPER_STUDIO_PHASE28A_PRE_BREAKPOINT_ONCE");
    int breakpointValue = 28;
    breakpointValue = breakpointValue + 1;
    int window = gx_window_create(320, 180, "Developer Studio Phase 28A");
    if (window == 0) return -1;
    if (gx_window_set_text(window, "28A GUI 28") != 0) return -2;
