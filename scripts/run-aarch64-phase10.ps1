...
if ([string]::IsNullOrWhiteSpace($ArtifactDirectory)) { $ArtifactDirectory = Join-Path $repoRoot 'out\aarch64-phase10' }
...
foreach ($required in @('EFI\BOOT\BOOTAA64.EFI','kernel.elf','ramdisk.img')) {
    if (!(Test-Path -LiteralPath (Join-Path $esp $required) -PathType Leaf)) { throw "Phase 10 ESP is missing $required" }
}
...
if ([string]::IsNullOrWhiteSpace($LogPath)) { $LogPath = Join-Path $ArtifactDirectory 'qemu-aarch64-phase10.log' }
...
$arguments = @(
    ...
    '-drive',"file=fat:rw:$esp,format=raw",'-display',$DisplayBackend,'-device','ramfb',
    '-global','virtio-mmio.force-legacy=false','-device','virtio-keyboard-device','-device','virtio-tablet-device') + $monitorArguments
...
while ([DateTime]::UtcNow -lt $deadline) {
    ...
    if ($output -match 'AARCH64_PHASE10_PASS|AARCH64_PHASE10_ERROR|\[guideXOS\].*(FATAL|ERROR)') { break }
...
if ($output -match 'AARCH64_PHASE10_PASS') { exit 0 }
exit 1
