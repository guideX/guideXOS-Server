...
$testBinary = Join-Path $outputDirectory "compiler-multifile-host-test.exe"

New-Item -ItemType Directory -Force -Path $outputDirectory | Out-Null

$gxx = (Get-Command g++ -ErrorAction Stop).Source
$sources = @(
    (Join-Path $repoRoot "tests/compiler_multifile_host_test.cpp"),
...
if ($LASTEXITCODE -ne 0) { throw "multi-file compiler host test build failed" }

& $testBinary
if ($LASTEXITCODE -ne 0) { throw "multi-file compiler host test failed" }
