...
if ($Phase28OOnly) {
    ...
    $Phase27Y = $false; $Phase27Z = $false; $Phase28A = $false; $Phase28B = $false
    $Phase28C = $false; $Phase28D = $false; $Phase28E = $false; $Phase28F = $false; $Phase28G = $false
    $Phase28H = $false; $Phase28I = $false; $Phase28J = $false; $Phase28K = $false; $Phase28L = $false; $Phase28M = $true
} elseif ($Phase28LOnly) {
    $Phase27E = $false; $Phase27F = $false; $Phase27G = $false; $Phase27H = $false
    $Phase27I = $false; $Phase27J = $false; $Phase27K = $false; $Phase27L = $false
    $Phase27M = $false; $Phase27N = $false; $Phase27O = $false; $Phase27P = $false
    ...
    $Phase27Y = $false; $Phase27Z = $false; $Phase28A = $false; $Phase28B = $false
    $Phase28C = $false; $Phase28D = $false; $Phase28E = $false; $Phase28F = $false; $Phase28G = $false
    $Phase28H = $false; $Phase28I = $false; $Phase28J = $false; $Phase28K = $false; $Phase28L = $true
} elseif ($Phase28KOnly) {
    $Phase27E = $false; $Phase27F = $false; $Phase27G = $false; $Phase27H = $false
    $Phase27I = $false; $Phase27J = $false; $Phase27K = $false; $Phase27L = $false
    $Phase27M = $false; $Phase27N = $false; $Phase27O = $false; $Phase27P = $false
    ...
    $Phase27Y = $false; $Phase27Z = $false; $Phase28A = $false; $Phase28B = $false
    $Phase28C = $false; $Phase28D = $false; $Phase28E = $false; $Phase28F = $false; $Phase28G = $false
    $Phase28H = $false; $Phase28I = $false; $Phase28J = $false; $Phase28K = $true
} elseif ($Phase28JOnly) {
    $Phase27E = $false; $Phase27F = $false; $Phase27G = $false; $Phase27H = $false
    $Phase27I = $false; $Phase27J = $false; $Phase27K = $false; $Phase27L = $false
    $Phase27M = $false; $Phase27N = $false; $Phase27O = $false; $Phase27P = $false
    ...
    $Phase27U = $false; $Phase27V = $false; $Phase27W = $false; $Phase27X = $false
    $Phase27Y = $false; $Phase27Z = $false; $Phase28A = $false; $Phase28B = $false
    $Phase28C = $false; $Phase28D = $false; $Phase28E = $false; $Phase28F = $true; $Phase28G = $false; $Phase28H = $false
} elseif ($Phase28EOnly) {
    $Phase27E = $false; $Phase27F = $false; $Phase27G = $false; $Phase27H = $false
    $Phase27I = $false; $Phase27J = $false; $Phase27K = $false; $Phase27L = $false
    $Phase27M = $false; $Phase27N = $false; $Phase27O = $false; $Phase27P = $false
    ...
    $Phase27U = $false; $Phase27V = $false; $Phase27W = $false; $Phase27X = $false
    $Phase27Y = $false; $Phase27Z = $false; $Phase28A = $false; $Phase28B = $false
    $Phase28C = $false; $Phase28D = $false; $Phase28E = $true; $Phase28F = $false
} elseif ($Phase28DOnly) {
    $Phase27E = $false; $Phase27F = $false; $Phase27G = $false; $Phase27H = $false
    $Phase27I = $false; $Phase27J = $false; $Phase27K = $false; $Phase27L = $false
    $Phase27M = $false; $Phase27N = $false; $Phase27O = $false; $Phase27P = $false
    ...
    if ($Phase28N) { $Phase28M = $true }
    if ($Phase28L) { $Phase28K = $true }
    if ($Phase28K) { $Phase28J = $true }
...
if (($Phase28L -or $Phase28M -or $Phase28N -or $Phase28OOnly) -and $TimeoutSeconds -lt 120) { $TimeoutSeconds = 120 }
...
$phase28eFixtureDirectory = Join-Path $root "scripts/fixtures/phase28e"
...
$phase28kFixtureDirectory = Join-Path $root "scripts/fixtures/phase28k"
...
$tempDirectory = Join-Path ([System.IO.Path]::GetTempPath()) ("guidexos-phase28g-" + [guid]::NewGuid().ToString("N"))
...
function Stage-Phase28EProject([string]$target) {
    if (Test-Path -LiteralPath $target) {
        Remove-Item -LiteralPath $target -Recurse -Force
    }
    Copy-Item $phase28eFixtureDirectory $target -Recurse -Force
}

function Stage-Phase28FProject([string]$target) {
    if (Test-Path -LiteralPath $target) {
        Remove-Item -LiteralPath $target -Recurse -Force
    }
    Copy-Item $phase28fFixtureDirectory $target -Recurse -Force
}

function Stage-Phase28GProject([string]$target) {
    if (Test-Path -LiteralPath $target) {
        Remove-Item -LiteralPath $target -Recurse -Force
    }
    Copy-Item $phase28gFixtureDirectory $target -Recurse -Force
}

function Stage-Phase28HProject([string]$target) {
    if (Test-Path -LiteralPath $target) {
        Remove-Item -LiteralPath $target -Recurse -Force
    }
...
function Stage-Phase28KProject([string]$target) {
    if (Test-Path -LiteralPath $target) {
        Remove-Item -LiteralPath $target -Recurse -Force
    }
    Copy-Item $phase28kFixtureDirectory $target -Recurse -Force
}

function Stage-Phase28LProject([string]$target) {
    if (Test-Path -LiteralPath $target) {
        Remove-Item -LiteralPath $target -Recurse -Force
    }
    Copy-Item $phase28lFixtureDirectory $target -Recurse -Force
}

function Stage-Phase28MProject([string]$target) {
    if (Test-Path -LiteralPath $target) {
        Remove-Item -LiteralPath $target -Recurse -Force
    }
    Copy-Item $phase28mFixtureDirectory $target -Recurse -Force
}

function Stage-Phase28OProject([string]$target) {
    # Phase 28O is checked in under ESP/P28O so the fixture is also available
    # to the normal package/QEMU staging path. When the target is that source
    # directory, leave it in place and only let the caller create its output
    # directory.
    if ([IO.Path]::GetFullPath($target) -eq [IO.Path]::GetFullPath($phase28oFixtureDirectory)) {
        return
    }
    if (Test-Path -LiteralPath $target) {
        Remove-Item -LiteralPath $target -Recurse -Force
...
function Invoke-QemuProofBoot([int]$runNumber, [string]$qemu) {
    ...
    try {
        ...
        if ($Phase28E) {
            $requiredMarkers += @(
                "DEVELOPER_STUDIO_PHASE28E_BEGIN",
                "DEVELOPER_STUDIO_PHASE28E_BUILD_PASS",
                "DEVELOPER_STUDIO_PHASE28E_CALL_BREAKPOINT_PASS",
                "DEVELOPER_STUDIO_PHASE28E_STEP_INTO_PASS",
                "DEVELOPER_STUDIO_PHASE28E_CALLEE_PAUSED_PASS",
                "DEVELOPER_STUDIO_PHASE28E_FRAME_CAPTURE_PASS",
                "DEVELOPER_STUDIO_PHASE28E_RETURN_ADDRESS_PASS",
                "DEVELOPER_STUDIO_PHASE28E_STEP_OUT_REQUEST_PASS",
                "DEVELOPER_STUDIO_PHASE28E_TEMP_BREAKPOINT_PASS",
                "DEVELOPER_STUDIO_PHASE28E_REMAINING_CALLEE_CODE_PASS",
                "DEVELOPER_STUDIO_PHASE28E_NO_INTERMEDIATE_PAUSE_PASS",
                "DEVELOPER_STUDIO_PHASE28E_NESTED_OUT_PASS",
                "DEVELOPER_STUDIO_PHASE28E_RETURN_TRAP_PASS",
                "DEVELOPER_STUDIO_PHASE28E_CALLER_FRAME_PASS",
                "DEVELOPER_STUDIO_PHASE28E_CALLER_SOURCE_PASS",
                "DEVELOPER_STUDIO_PHASE28E_STEP_OVER_EQUIVALENCE_PASS",
                "DEVELOPER_STUDIO_PHASE28E_LEAF_PASS",
                "DEVELOPER_STUDIO_PHASE28E_ROOT_FRAME_PASS",
                "DEVELOPER_STUDIO_PHASE28E_CANCEL_PASS",
                "DEVELOPER_STUDIO_PHASE28E_STALE_PASS",
                "DEVELOPER_STUDIO_PHASE28E_RESUME_PASS",
                "DEVELOPER_STUDIO_PHASE28E_RENDER_PASS",
                "DEVELOPER_STUDIO_PHASE28E_CLOSE_PASS",
                "DEVELOPER_STUDIO_PHASE28E_CLEANUP_PASS",
                "DEVELOPER_STUDIO_PHASE28E_STEP_INTO_REGRESSION_PASS",
                "DEVELOPER_STUDIO_PHASE28E_STEP_OVER_REGRESSION_PASS",
                "DEVELOPER_STUDIO_PHASE28E_INSTRUCTION_STEP_REGRESSION_PASS",
                "DEVELOPER_STUDIO_PHASE28E_RUN_REGRESSION_PASS",
                "phase28e_build_pass=PASS",
                "phase28e_debug_pass=PASS",
                "phase28e_step_out_pass=PASS",
                "phase28e_step_over_equivalence_pass=PASS",
                "phase28e_run_pass=PASS",
                "phase28e=PASS",
        ...
        if ($Phase28F) {
            $requiredMarkers += @(
                "DEVELOPER_STUDIO_PHASE28F_BEGIN",
                "DEVELOPER_STUDIO_PHASE28F_BUILD_PASS",
                "DEVELOPER_STUDIO_PHASE28F_NESTED_PAUSED_PASS",
                "DEVELOPER_STUDIO_PHASE28F_STACK_ORDER_PASS",
                "DEVELOPER_STUDIO_PHASE28F_STACK_SOURCE_PASS",
                "DEVELOPER_STUDIO_PHASE28F_STACK_FRAME_EVIDENCE",
                "DEVELOPER_STUDIO_PHASE28F_READ_ONLY_STABLE_PASS",
                "DEVELOPER_STUDIO_PHASE28F_PAUSED_STABLE_PASS",
                "DEVELOPER_STUDIO_PHASE28F_STEP_OUT_STACK_PASS",
                "DEVELOPER_STUDIO_PHASE28F_STALE_PASS",
                "DEVELOPER_STUDIO_PHASE28F_RUNNING_REJECT_PASS",
                "DEVELOPER_STUDIO_PHASE28F_RENDER_PASS",
                "DEVELOPER_STUDIO_PHASE28F_CLOSE_PASS",
                "DEVELOPER_STUDIO_PHASE28F_CLEANUP_PASS",
                "DEVELOPER_STUDIO_PHASE28F_RUN_REGRESSION_PASS",
                "phase28f_build_pass=PASS",
                "phase28f_warm_cache_pass=PASS",
                "phase28f_call_stack_pass=PASS",
                "phase28f_run_pass=PASS",
                "phase28f_artifact=PASS",
                "phase28f_debug_pass=PASS",
                "phase28f=PASS",
        ...
        if ($Phase28G) {
            $requiredMarkers += @(
                "DEVELOPER_STUDIO_PHASE28G_BEGIN",
                "DEVELOPER_STUDIO_PHASE28G_BUILD_PASS",
                "DEVELOPER_STUDIO_PHASE28G_WARM_CACHE_PASS",
                "DEVELOPER_STUDIO_PHASE28G_PAUSED_PASS",
                "DEVELOPER_STUDIO_PHASE28G_CALL_STACK_REGRESSION_PASS",
                "DEVELOPER_STUDIO_PHASE28G_VARIABLE_METADATA_PASS",
                "DEVELOPER_STUDIO_PHASE28G_ARGUMENTS_PASS",
                "DEVELOPER_STUDIO_PHASE28G_LOCALS_PASS",
                "DEVELOPER_STUDIO_PHASE28G_FRAME_MATCH_PASS",
                "DEVELOPER_STUDIO_PHASE28G_REPEAT_QUERY_PASS",
                "DEVELOPER_STUDIO_PHASE28G_READ_ONLY_PASS",
                "DEVELOPER_STUDIO_PHASE28G_VALUE_CHANGE_PASS",
                "DEVELOPER_STUDIO_PHASE28G_LIVE_RANGE_PASS",
                "DEVELOPER_STUDIO_PHASE28G_STEP_INTO_FRAME_PASS",
                "DEVELOPER_STUDIO_PHASE28G_STEP_OUT_FRAME_PASS",
                "DEVELOPER_STUDIO_PHASE28G_STALE_PASS",
                "DEVELOPER_STUDIO_PHASE28G_RESUME_PASS",
                "DEVELOPER_STUDIO_PHASE28G_RENDER_PASS",
                "DEVELOPER_STUDIO_PHASE28G_CLOSE_PASS",
                "DEVELOPER_STUDIO_PHASE28G_CLEANUP_PASS",
                "DEVELOPER_STUDIO_PHASE28G_RUN_REGRESSION_PASS",
                "phase28g_build_pass=PASS",
                "phase28g_warm_cache_pass=PASS",
                "phase28g_variables_pass=PASS",
                "phase28g_run_pass=PASS",
                "phase28g_artifact=PASS",
                "phase28g_debug_pass=PASS",
                "phase28g=PASS",
        ...
        if ($Phase28H) {
            $requiredMarkers += @(
                "DEVELOPER_STUDIO_PHASE28H_BEGIN",
                "DEVELOPER_STUDIO_PHASE28H_BUILD_PASS",
                "DEVELOPER_STUDIO_PHASE28H_WARM_CACHE_PASS",
                "DEVELOPER_STUDIO_PHASE28H_NESTED_PAUSE_PASS",
                "DEVELOPER_STUDIO_PHASE28H_CALL_STACK_PASS",
                "DEVELOPER_STUDIO_PHASE28H_SELECT_FRAME0_PASS",
                "DEVELOPER_STUDIO_PHASE28H_FRAME0_VARIABLES_PASS",
                "DEVELOPER_STUDIO_PHASE28H_SELECT_FRAME1_PASS",
                "DEVELOPER_STUDIO_PHASE28H_FRAME1_VARIABLES_PASS",
                "DEVELOPER_STUDIO_PHASE28H_SELECT_FRAME2_PASS",
                "DEVELOPER_STUDIO_PHASE28H_FRAME2_VARIABLES_PASS",
                "DEVELOPER_STUDIO_PHASE28H_CALLER_LIVE_RANGE_PASS",
                "DEVELOPER_STUDIO_PHASE28H_ROUND_TRIP_PASS",
                "DEVELOPER_STUDIO_PHASE28H_READ_ONLY_GPRS_PASS",
                "DEVELOPER_STUDIO_PHASE28H_STACK_UNCHANGED_PASS",
                "DEVELOPER_STUDIO_PHASE28H_INVALID_FRAME_PASS",
                "DEVELOPER_STUDIO_PHASE28H_STEP_OUT_RESET_PASS",
                "DEVELOPER_STUDIO_PHASE28H_STALE_SELECTION_PASS",
                "DEVELOPER_STUDIO_PHASE28H_RESUME_PASS",
                "DEVELOPER_STUDIO_PHASE28H_RENDER_PASS",
                "DEVELOPER_STUDIO_PHASE28H_CLOSE_PASS",
                "DEVELOPER_STUDIO_PHASE28H_CLEANUP_PASS",
                ...
                "DEVELOPER_STUDIO_PHASE28H_RUN_REGRESSION_PASS",
        ...
        if ($Phase28K) {
            $requiredMarkers += @(
                "DEVELOPER_STUDIO_PHASE28K_BEGIN",
                "DEVELOPER_STUDIO_PHASE28K_BUILD_PASS",
                "DEVELOPER_STUDIO_PHASE28K_ADD_PASS",
                "DEVELOPER_STUDIO_PHASE28K_LIST_PASS",
                "DEVELOPER_STUDIO_PHASE28K_DUPLICATE_REJECT_PASS",
                "DEVELOPER_STUDIO_PHASE28K_CAPACITY_REJECT_PASS",
                "DEVELOPER_STUDIO_PHASE28K_FALSE_HIT_PASS",
                "DEVELOPER_STUDIO_PHASE28K_REARM_PASS",
                "DEVELOPER_STUDIO_PHASE28K_TRUE_HIT_PASS",
                "DEVELOPER_STUDIO_PHASE28K_DISABLE_PASS",
                "DEVELOPER_STUDIO_PHASE28K_ENABLE_PASS",
                "DEVELOPER_STUDIO_PHASE28K_THREE_ACTIVE_PASS",
                "DEVELOPER_STUDIO_PHASE28K_MAIN_HIT_PASS",
                "DEVELOPER_STUDIO_PHASE28K_REMOVE_PASS",
                "DEVELOPER_STUDIO_PHASE28K_STALE_ID_PASS",
                "DEVELOPER_STUDIO_PHASE28K_LEGACY_PASS",
                "DEVELOPER_STUDIO_PHASE28K_RENDER_PASS",
                "DEVELOPER_STUDIO_PHASE28K_CLOSE_PASS",
                "DEVELOPER_STUDIO_PHASE28K_CLEANUP_PASS",
                "DEVELOPER_STUDIO_PHASE28K_RUN_REGRESSION_PASS",
                "DEVELOPER_STUDIO_PHASE28K_PASS"
            )
        }
        if ($Phase28L) {
            $requiredMarkers += @(
                "DEVELOPER_STUDIO_PHASE28L_BEGIN",
                "DEVELOPER_STUDIO_PHASE28L_BUILD_PASS",
                "DEVELOPER_STUDIO_PHASE28L_CONFIGURE_PASS",
                "DEVELOPER_STUDIO_PHASE28L_CONDITION_LOG_PASS",
                "DEVELOPER_STUDIO_PHASE28L_HIT_COUNT_PASS",
                "DEVELOPER_STUDIO_PHASE28L_OUTPUT_FIFO_PASS",
                "DEVELOPER_STUDIO_PHASE28L_THREE_ACTIVE_PASS",
                "DEVELOPER_STUDIO_PHASE28L_COEXISTING_BREAK_PASS",
                "DEVELOPER_STUDIO_PHASE28L_GUI_COMPLETE_PASS",
                "DEVELOPER_STUDIO_PHASE28L_GENERATION_RESET_PASS",
                "DEVELOPER_STUDIO_PHASE28L_SECOND_RUN_PASS",
                "DEVELOPER_STUDIO_PHASE28L_CLEANUP_PASS",
                "DEVELOPER_STUDIO_PHASE28L_PASS"
            )
        }
        if ($Phase28M -and -not $Phase28OOnly) {
            $requiredMarkers += @(
                "DEVELOPER_STUDIO_PHASE28M_BEGIN",
                "DEVELOPER_STUDIO_PHASE28M_APP_DISCOVERY_PASS",
                "DEVELOPER_STUDIO_PHASE28M_APP_LAUNCH_PASS",
                "DEVELOPER_STUDIO_PHASE28M_WINDOW_VISIBLE_PASS",
                "DEVELOPER_STUDIO_PHASE28M_PROJECT_OPEN_PASS",
                "DEVELOPER_STUDIO_PHASE28M_DEBUG_START_UI_PASS",
                "DEVELOPER_STUDIO_PHASE28M_INITIAL_PAUSE_PASS",
                "DEVELOPER_STUDIO_PHASE28M_CALL_STACK_DEPTH_PASS",
                "DEVELOPER_STUDIO_PHASE28M_PANE_CALL_STACK_PASS",
                "DEVELOPER_STUDIO_PHASE28M_PANE_LOCALS_PASS",
                "DEVELOPER_STUDIO_PHASE28M_PANE_ARGUMENTS_PASS",
                "DEVELOPER_STUDIO_PHASE28M_PANE_WATCHES_PASS",
                "DEVELOPER_STUDIO_PHASE28M_WATCH_INVALID_ERROR_PASS",
                "DEVELOPER_STUDIO_PHASE28M_PANE_BREAKPOINTS_PASS",
                "DEVELOPER_STUDIO_PHASE28M_BREAKPOINT_ENABLE_DISABLE_PASS",
                "DEVELOPER_STUDIO_PHASE28M_BREAKPOINT_POLICY_HIT_COUNT_PASS",
                "DEVELOPER_STUDIO_PHASE28M_STEP_OUT_PASS",
                "DEVELOPER_STUDIO_PHASE28M_STEP_INTO_PASS",
                "DEVELOPER_STUDIO_PHASE28M_STEP_OVER_PASS",
                "DEVELOPER_STUDIO_PHASE28M_STEP_OUT_FINAL_PASS",
                "DEVELOPER_STUDIO_PHASE28M_CONTINUE_UI_PASS",
                "DEVELOPER_STUDIO_PHASE28M_PANE_OUTPUT_PASS",
                "DEVELOPER_STUDIO_PHASE28M_LOGPOINT_RUNTIME_OUTPUT_PASS",
                "DEVELOPER_STUDIO_PHASE28M_LOGPOINT_NO_PAUSE_PASS",
                "DEVELOPER_STUDIO_PHASE28M_STOP_UI_PASS",
                "DEVELOPER_STUDIO_PHASE28M_STALE_STATE_CLEAR_PASS",
                "DEVELOPER_STUDIO_PHASE28M_SECOND_SESSION_PASS",
                "DEVELOPER_STUDIO_PHASE28M_BOUNDS_PASS",
                "DEVELOPER_STUDIO_PHASE28M_SESSION_CONTROLS_PASS",
                "DEVELOPER_STUDIO_PHASE28M_PASS",
                "DEVELOPER_STUDIO_PHASE28M_GUEST_APP_PASS",
                "DEVELOPER_STUDIO_PHASE28M_GUEST_CLEANUP_PASS",
                "DEVELOPER_STUDIO_PHASE28M_GUEST_PASS"
            )
        }
        if ($Phase28N -and -not $Phase28OOnly) {
            $requiredMarkers += @(
                "DEVELOPER_STUDIO_PHASE28N_BEGIN",
                "DEVELOPER_STUDIO_PHASE28N_EDITOR_READY_PASS",
                "DEVELOPER_STUDIO_PHASE28N_EDITOR_BREAKPOINT_ADD_PASS",
                "DEVELOPER_STUDIO_PHASE28N_EXECUTION_FRAME0_PASS",
                "DEVELOPER_STUDIO_PHASE28N_CALL_STACK_NAVIGATION_PASS",
                "DEVELOPER_STUDIO_PHASE28N_BREAKPOINT_STATE_PASS",
                "DEVELOPER_STUDIO_PHASE28N_STEP_SOURCE_SYNC_PASS",
                "DEVELOPER_STUDIO_PHASE28N_LOGPOINT_PROJECTION_PASS",
                "DEVELOPER_STUDIO_PHASE28N_RUNNING_MARKER_CLEAR_PASS",
                "DEVELOPER_STUDIO_PHASE28N_EDITOR_BREAKPOINT_REMOVE_PASS",
                "DEVELOPER_STUDIO_PHASE28N_NEW_SESSION_MARKER_PASS",
                "DEVELOPER_STUDIO_PHASE28N_BOUNDS_PASS",
                "DEVELOPER_STUDIO_PHASE28N_PASS"
            )
        }
        if ($Phase28OOnly) {
            $requiredMarkers += @(
        ...
        if ($missingMarkers.Count -ne 0) {
            ...
            if ($Phase28E) {
                $serial -split "`r?`n" | Where-Object { $_ -match "phase28e|Phase 28E|P28E|DEVELOPER_STUDIO_PHASE28E|Compiler:.*(build|error|diagnostic)" } | ForEach-Object { Write-Host $_ }
            }
            if ($Phase28F) {
                $serial -split "`r?`n" | Where-Object { $_ -match "phase28f|Phase 28F|P28F|DEVELOPER_STUDIO_PHASE28F|Compiler:.*(build|error|diagnostic)" } | ForEach-Object { Write-Host $_ }
            }
            if ($Phase28G) {
            ...
            if ($Phase28K) {
                $serial -split "`r?`n" | Where-Object { $_ -match "phase28k|Phase 28K|P28K|DEVELOPER_STUDIO_PHASE28K|Compiler:.*(build|error|diagnostic)" } | ForEach-Object { Write-Host $_ }
            }
            if ($Phase28L -or $Phase28M) {
                $serial -split "`r?`n" | Where-Object { $_ -match "phase28l|Phase 28L|P28L|DEVELOPER_STUDIO_PHASE28L|Compiler:.*(build|error|diagnostic)" } | ForEach-Object { Write-Host $_ }
            }
            if ($Phase28M) {
                $serial -split "`r?`n" | Where-Object { $_ -match "DEVELOPER_STUDIO_PHASE28M|phase28m|Phase 28M|P28M|DeveloperStudio|Compiler:.*(build|error|diagnostic)" } | ForEach-Object { Write-Host $_ }
                if ($serial) { Write-Host "--- full Phase 28M serial ---"; Write-Host $serial }
            }
            if ($Phase28N) {
                $serial -split "`r?`n" | Where-Object { $_ -match "DEVELOPER_STUDIO_PHASE28N|phase28n|Phase 28N|P28M|DeveloperStudio|Compiler:.*(build|error|diagnostic)" } | ForEach-Object { Write-Host $_ }
            }
            if ($Phase27V) {
            ...
            if (-not $Phase27U -and -not $Phase28FOnly -and -not $Phase28HOnly -and -not $Phase28IOnly -and -not $Phase28JOnly -and -not $Phase28KOnly -and -not $Phase28LOnly -and -not $Phase28MOnly -and -not $Phase28OOnly -and $serial) { Write-Host $serial }
        ...
        if ($Phase28OOnly) {
            ...
            $serial -split "`r?`n" | Where-Object { $_ -match "DEVELOPER_STUDIO_PHASE28M|phase28m|Phase 28M|P28M|DeveloperStudio|NativeElf: artifact_begin" } |
                ForEach-Object { Write-Host $_ }
        } elseif ($Phase28LOnly) {
            $serial -split "`r?`n" | Where-Object { $_ -match "DEVELOPER_STUDIO_PHASE28L|phase28l|Phase 28L|P28L|NativeElf: artifact_begin" } |
                ForEach-Object { Write-Host $_ }
        } elseif ($Phase28JOnly -or $Phase28KOnly) {
            $serial -split "`r?`n" | Where-Object { $_ -match "DEVELOPER_STUDIO_PHASE28J|phase28j|Phase 28J|P28J|DEVELOPER_STUDIO_PHASE28K|phase28k|Phase 28K|P28K|NativeElf: artifact_begin" } |
                ForEach-Object { Write-Host $_ }
        } elseif ($Phase28IOnly) {
            ...
            $serial -split "`r?`n" |
                Where-Object { $_ -notmatch "NativeElf: artifact_hex=" -and
                    $_ -match "DEVELOPER_STUDIO_PHASE28G|phase28g|Phase 28G|P28G|NativeElf: artifact_begin" } |
                ForEach-Object { Write-Host $_ }
        } elseif ($Phase28FOnly) {
            $serial -split "`r?`n" |
                Where-Object { $_ -notmatch "NativeElf: artifact_hex=" -and
                    $_ -match "DEVELOPER_STUDIO_PHASE28F|phase28f|Phase 28F|P28F|NativeElf: artifact_begin" } |
                ForEach-Object { Write-Host $_ }
        } elseif ($Phase28EOnly) {
            $serial -split "`r?`n" |
                Where-Object { $_ -notmatch "NativeElf: artifact_hex=" -and
                    $_ -match "DEVELOPER_STUDIO_PHASE28E|phase28e|Phase 28E|dedicated application stack" } |
                ForEach-Object { Write-Host $_ }
        } else {
            $serial -split "`r?`n" |
                Where-Object { $_ -notmatch "NativeElf: artifact_hex=" -and
...
try {
    ...
    if ($Phase28E) { $env:EXTRA_CFLAGS += " -DGXOS_PHASE28E_SMOKE" }
    ...
    if ($Phase28K) { $env:EXTRA_CFLAGS += " -DGXOS_PHASE28K_SMOKE" }
    if ($Phase28L) { $env:EXTRA_CFLAGS += " -DGXOS_PHASE28L_SMOKE" }
    if ($Phase28M) { $env:EXTRA_CFLAGS += " -DGXOS_PHASE28M_SMOKE" }
    ...
    try {
        ...
        if ($Phase27E -or $Phase27F -or $Phase27M -or $Phase27O -or $Phase27P -or $Phase27Q -or $Phase27R -or $Phase27S -or $Phase27T -or $Phase27U -or $Phase27V -or $Phase27W -or $Phase27X -or $Phase27Y -or $Phase27Z -or $Phase28A -or $Phase28B -or $Phase28C -or $Phase28D -or $Phase28E -or $Phase28F -or $Phase28G -or $Phase28H -or $Phase28I -or $Phase28J -or $Phase28K -or $Phase28L -or $Phase28M) {
    ...
    if ($Phase28E) {
        $target = Join-Path $espDirectory "P28E"
        if (Test-Path $target -PathType Leaf) { throw "ESP target is a file: $target" }
        if (Test-Path $target -PathType Container) {
            $backup = Join-Path $tempDirectory "backup-directory-P28E"
            Copy-Item $target $backup -Recurse -Force
            $directoryBackups["P28E"] = $backup
        }
    }
    if ($Phase28F) {
        $target = Join-Path $espDirectory "P28F"
        if (Test-Path $target -PathType Leaf) { throw "ESP target is a file: $target" }
        if (Test-Path $target -PathType Container) {
            $backup = Join-Path $tempDirectory "backup-directory-P28F"
            Copy-Item $target $backup -Recurse -Force
            $directoryBackups["P28F"] = $backup
        }
    }
    if ($Phase28G) {
        $target = Join-Path $espDirectory "P28G"
        if (Test-Path $target -PathType Leaf) { throw "ESP target is a file: $target" }
        if (Test-Path $target -PathType Container) {
            $backup = Join-Path $tempDirectory "backup-directory-P28G"
            Copy-Item $target $backup -Recurse -Force
            $directoryBackups["P28G"] = $backup
        }
    }
    if ($Phase28H) {
        $target = Join-Path $espDirectory "P28H"
        if (Test-Path $target -PathType Leaf) { throw "ESP target is a file: $target" }
        if (Test-Path $target -PathType Container) {
            $backup = Join-Path $tempDirectory "backup-directory-P28H"
            Copy-Item $target $backup -Recurse -Force
    ...
    if ($Phase28K) {
        $target = Join-Path $espDirectory "P28K"
        if (Test-Path $target -PathType Leaf) { throw "ESP target is a file: $target" }
        if (Test-Path $target -PathType Container) {
            $backup = Join-Path $tempDirectory "backup-directory-P28K"
            Copy-Item $target $backup -Recurse -Force
            $directoryBackups["P28K"] = $backup
        }
    }
    if ($Phase28L) {
        $target = Join-Path $espDirectory "P28L"
        if (Test-Path $target -PathType Leaf) { throw "ESP target is a file: $target" }
        if (Test-Path $target -PathType Container) {
            $backup = Join-Path $tempDirectory "backup-directory-P28L"
            Copy-Item $target $backup -Recurse -Force
    ...
    if ($Phase28M) {
        ...
        if (Test-Path $target -PathType Container) {
            $backup = Join-Path $tempDirectory "backup-directory-Apps-DeveloperStudio"
            Copy-Item $target $backup -Recurse -Force
            $directoryBackups["Apps/DeveloperStudio"] = $backup
        }
    }
    if ($Phase27R) {
    ...
    if ($Phase28E) {
        $phase28eEspDirectory = Join-Path $espDirectory "P28E"
        Stage-Phase28EProject $phase28eEspDirectory
        New-Item -ItemType Directory -Force -Path (Join-Path $espDirectory "P28E/out") | Out-Null
        if (!(Test-Path -LiteralPath (Join-Path $espDirectory "P28E/guidexos.project") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28E/app/app.json") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28E/src/main.cpp") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28E/src/helper.cpp") -PathType Leaf)) {
            throw "Phase 28E project fixture was not staged into ESP"
        }
    }
    if ($Phase28F) {
        $phase28fEspDirectory = Join-Path $espDirectory "P28F"
        Stage-Phase28FProject $phase28fEspDirectory
        New-Item -ItemType Directory -Force -Path (Join-Path $espDirectory "P28F/out") | Out-Null
        if (!(Test-Path -LiteralPath (Join-Path $espDirectory "P28F/guidexos.project") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28F/app/app.json") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28F/src/main.cpp") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28F/src/helper.cpp") -PathType Leaf)) {
            throw "Phase 28F project fixture was not staged into ESP"
        }
    }
    if ($Phase28G) {
        $phase28gEspDirectory = Join-Path $espDirectory "P28G"
        Stage-Phase28GProject $phase28gEspDirectory
        New-Item -ItemType Directory -Force -Path (Join-Path $espDirectory "P28G/out") | Out-Null
        if (!(Test-Path -LiteralPath (Join-Path $espDirectory "P28G/guidexos.project") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28G/app/app.json") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28G/src/main.cpp") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28G/src/helper.cpp") -PathType Leaf)) {
            throw "Phase 28G project fixture was not staged into ESP"
        }
    }
    if ($Phase28H) {
        $phase28hEspDirectory = Join-Path $espDirectory "P28H"
        Stage-Phase28HProject $phase28hEspDirectory
        New-Item -ItemType Directory -Force -Path (Join-Path $espDirectory "P28H/out") | Out-Null
        if (!(Test-Path -LiteralPath (Join-Path $espDirectory "P28H/guidexos.project") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28H/app/app.json") -PathType Leaf) -or
    ...
    if ($Phase28K) {
        $phase28kEspDirectory = Join-Path $espDirectory "P28K"
        Stage-Phase28KProject $phase28kEspDirectory
        New-Item -ItemType Directory -Force -Path (Join-Path $espDirectory "P28K/out") | Out-Null
        if (!(Test-Path -LiteralPath (Join-Path $espDirectory "P28K/guidexos.project") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28K/app/app.json") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28K/src/main.cpp") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28K/src/helper.cpp") -PathType Leaf)) {
            throw "Phase 28K project fixture was not staged into ESP"
        }
    }
    if ($Phase28L) {
        $phase28lEspDirectory = Join-Path $espDirectory "P28L"
        Stage-Phase28LProject $phase28lEspDirectory
        New-Item -ItemType Directory -Force -Path (Join-Path $espDirectory "P28L/out") | Out-Null
        if (!(Test-Path -LiteralPath (Join-Path $espDirectory "P28L/guidexos.project") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28L/app/app.json") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28L/src/main.cpp") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $espDirectory "P28L/src/helper.cpp") -PathType Leaf)) {
            throw "Phase 28L project fixture was not staged into ESP"
        }
    }
    if ($Phase28M) {
        $phase28ProjectName = if ($Phase28OOnly) { "P28O" } else { "P28M" }
        $phase28EspDirectory = Join-Path $espDirectory $phase28ProjectName
        if ($Phase28OOnly) { Stage-Phase28OProject $phase28EspDirectory }
        ...
        if (!(Test-Path -LiteralPath (Join-Path $phase28EspDirectory "guidexos.project") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $phase28EspDirectory "app/app.json") -PathType Leaf) -or
            !(Test-Path -LiteralPath (Join-Path $phase28EspDirectory "src/main.cpp") -PathType Leaf) -or
        ...
        if ($Phase28OOnly) {
            ...
            [IO.File]::WriteAllText((Join-Path $phase28mEspPackage ".phase28m-diagnostic"), "guideXOS-phase28m")
            if (!(Test-Path -LiteralPath (Join-Path $phase28mEspPackage ".phase28m-diagnostic") -PathType Leaf)) {
    ...
    if ($Phase28E -and $run -gt 1) {
        Stage-Phase28EProject (Join-Path $espDirectory "P28E")
        New-Item -ItemType Directory -Force -Path (Join-Path $espDirectory "P28E/out") | Out-Null
    }
    if ($Phase28F -and $run -gt 1) {
        Stage-Phase28FProject (Join-Path $espDirectory "P28F")
        New-Item -ItemType Directory -Force -Path (Join-Path $espDirectory "P28F/out") | Out-Null
    }
    if ($Phase28G -and $run -gt 1) {
        $phase28gRunDirectory = Join-Path $espDirectory "P28G"
        if (Test-Path -LiteralPath $phase28gRunDirectory) {
            Remove-Item -LiteralPath $phase28gRunDirectory -Recurse -Force
        }
        Stage-Phase28GProject $phase28gRunDirectory
        New-Item -ItemType Directory -Force -Path (Join-Path $phase28gRunDirectory "out") | Out-Null
    }
    if ($Phase28H -and $run -gt 1) {
        $phase28hRunDirectory = Join-Path $espDirectory "P28H"
        if (Test-Path -LiteralPath $phase28hRunDirectory) {
            Remove-Item -LiteralPath $phase28hRunDirectory -Recurse -Force
        }
    ...
    if ($Phase28K -and $run -gt 1) {
        $phase28kRunDirectory = Join-Path $espDirectory "P28K"
        if (Test-Path -LiteralPath $phase28kRunDirectory) {
            Remove-Item -LiteralPath $phase28kRunDirectory -Recurse -Force
        }
        Stage-Phase28KProject $phase28kRunDirectory
        New-Item -ItemType Directory -Force -Path (Join-Path $phase28kRunDirectory "out") | Out-Null
    }
    if ($Phase28L -and $run -gt 1) {
        $phase28lRunDirectory = Join-Path $espDirectory "P28L"
        if (Test-Path -LiteralPath $phase28lRunDirectory) {
            Remove-Item -LiteralPath $phase28lRunDirectory -Recurse -Force
        }
        Stage-Phase28LProject $phase28lRunDirectory
        New-Item -ItemType Directory -Force -Path (Join-Path $phase28lRunDirectory "out") | Out-Null
    }
    if ($Phase28M -and $run -gt 1) {
        $phase28RunName = if ($Phase28OOnly) { "P28O" } else { "P28M" }
        $phase28RunDirectory = Join-Path $espDirectory $phase28RunName
        if ($Phase28OOnly) { Stage-Phase28OProject $phase28RunDirectory }
    ...
    $serialArtifacts = @("r42")
    foreach ($candidate in @("d27a", "d27b", "d27c", "f28main", "g28main", "h28main", "j28main", "k28main", "l28main")) {
    ...
    if ($Phase28E) {
        Export-SerialArtifact $finalSerial "e28main" (Join-Path $evidenceDirectory "e28main.elf")
    }
    if ($Phase28F) {
        Export-SerialArtifact $finalSerial "f28main" (Join-Path $evidenceDirectory "f28main.elf")
    }
    if ($Phase28G) {
        Export-SerialArtifact $finalSerial "g28main" (Join-Path $evidenceDirectory "g28main.elf")
    }
    if ($Phase28H) {
    ...
    if ($Phase28K) {
        Export-SerialArtifact $finalSerial "k28main" (Join-Path $evidenceDirectory "k28main.elf")
    }
    if ($Phase28L) {
        Export-SerialArtifact $finalSerial "l28main" (Join-Path $evidenceDirectory "l28main.elf")
    }
    ...
    if ($Phase28E) {
        Write-Host "--- external audit of guest-generated e28main.elf ---" -ForegroundColor Cyan
        & $readelf -h -l (Join-Path $evidenceDirectory "e28main.elf")
        & $objdump -D -Mintel -b binary -m i386:x86-64 --adjust-vma=0x10000000 `
            --start-address=0x10001000 --stop-address=0x10004000 (Join-Path $evidenceDirectory "e28main.elf")
        if ($LASTEXITCODE -ne 0) { throw "external Phase 28E ELF inspection failed" }
    }
    if ($Phase28F) {
        Write-Host "--- external audit of guest-generated f28main.elf ---" -ForegroundColor Cyan
        & $readelf -h -l (Join-Path $evidenceDirectory "f28main.elf")
        & $objdump -D -Mintel -b binary -m i386:x86-64 --adjust-vma=0x10000000 `
            --start-address=0x10001000 --stop-address=0x10004000 (Join-Path $evidenceDirectory "f28main.elf")
        if ($LASTEXITCODE -ne 0) { throw "external Phase 28F ELF inspection failed" }
    }
    if ($Phase28G) {
        Write-Host "--- external audit of guest-generated g28main.elf ---" -ForegroundColor Cyan
        & $readelf -h -l (Join-Path $evidenceDirectory "g28main.elf")
        & $objdump -D -Mintel -b binary -m i386:x86-64 --adjust-vma=0x10000000 `
            --start-address=0x10001000 --stop-address=0x10004000 (Join-Path $evidenceDirectory "g28main.elf")
        if ($LASTEXITCODE -ne 0) { throw "external Phase 28G ELF inspection failed" }
    }
    if ($Phase28H) {
        Write-Host "--- external audit of guest-generated h28main.elf ---" -ForegroundColor Cyan
        & $readelf -h -l (Join-Path $evidenceDirectory "h28main.elf")
        & $objdump -D -Mintel -b binary -m i386:x86-64 --adjust-vma=0x10000000 `
    ...
    if ($Phase28K) {
        Write-Host "--- external audit of guest-generated k28main.elf ---" -ForegroundColor Cyan
        & $readelf -h -l (Join-Path $evidenceDirectory "k28main.elf")
        & $objdump -D -Mintel -b binary -m i386:x86-64 --adjust-vma=0x10000000 `
            --start-address=0x10001000 --stop-address=0x10004000 (Join-Path $evidenceDirectory "k28main.elf")
        if ($LASTEXITCODE -ne 0) { throw "external Phase 28K ELF inspection failed" }
    }
    if ($Phase28L) {
        Write-Host "--- external audit of guest-generated l28main.elf ---" -ForegroundColor Cyan
        & $readelf -h -l (Join-Path $evidenceDirectory "l28main.elf")
        & $objdump -D -Mintel -b binary -m i386:x86-64 --adjust-vma=0x10000000 `
            --start-address=0x10001000 --stop-address=0x10004000 (Join-Path $evidenceDirectory "l28main.elf")
        if ($LASTEXITCODE -ne 0) { throw "external Phase 28L ELF inspection failed" }
    }
    if ($Phase27N) {
    ...
    if ($Phase28OOnly) {
        ...
        Write-Host "Phase 28N source editor debugger integration proof completed across $BootCount fresh boot(s)." -ForegroundColor Green
    } elseif ($Phase28MOnly) {
        Write-Host "Phase 28M integrated Developer Studio UI proof completed across $BootCount fresh boot(s)." -ForegroundColor Green
    } elseif ($Phase28LOnly) {
        Write-Host "Phase 28L focused QEMU proof completed across $BootCount fresh boot(s)." -ForegroundColor Green
    } elseif ($Phase28KOnly) {
        Write-Host "Phase 28K focused QEMU proof completed across $BootCount fresh boot(s)." -ForegroundColor Green
    } elseif ($Phase28JOnly) {
        ...
        Write-Host "Phase 28F focused QEMU proof completed across $BootCount fresh boot(s)." -ForegroundColor Green
    } elseif ($Phase28EOnly) {
        Write-Host "Phase 28E focused QEMU proof completed across $BootCount fresh boot(s)." -ForegroundColor Green
    } elseif ($Phase28DOnly) {
    ...
    if ($Phase28LOnly -or $Phase28MOnly -or $Phase28NOnly -or $Phase28OOnly) {
        $proofLabel = if ($Phase28OOnly) { "Phase 28O" } elseif ($Phase28NOnly) { "Phase 28N" } elseif ($Phase28MOnly) { "Phase 28M" } else { "Phase 28L" }
