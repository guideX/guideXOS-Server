...
if ([string]::IsNullOrWhiteSpace($ArtifactDirectory)) { $ArtifactDirectory = Join-Path $repoRoot 'out\aarch64-phase7' }
...
foreach ($required in @('EFI\BOOT\BOOTAA64.EFI','kernel.elf','ramdisk.img')) {
    if (!(Test-Path -LiteralPath (Join-Path $esp $required) -PathType Leaf)) { throw "Phase 7 ESP is missing $required" }
}
...
if ([string]::IsNullOrWhiteSpace($LogPath)) { $LogPath = Join-Path $ArtifactDirectory 'qemu-aarch64-phase7.log' }
...
$arguments = @(
    ...
    '-display',$DisplayBackend,'-device','ramfb',
...
while ([DateTime]::UtcNow -lt $deadline) {
    ...
    if ($output -match 'AARCH64_PHASE7_PASS|AARCH64_PHASE7_ERROR|\[guideXOS\].*(FATAL|ERROR)') { break }
...
if ($output -match 'AARCH64_PHASE7_PASS') { exit 0 }
exit 1
