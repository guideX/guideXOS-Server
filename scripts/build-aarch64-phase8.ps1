...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) { $OutputDirectory = Join-Path $repoRoot 'out\aarch64-phase8' }
...
$loaderObject = Join-Path $OutputDirectory 'phase8_loader.obj'
...
$loaderFlags = @(
    '--target=aarch64-pc-windows-msvc', '-DGXOS_AARCH64_PHASE4', '-DGXOS_AARCH64_PHASE5', '-DGXOS_AARCH64_PHASE6', '-DGXOS_AARCH64_PHASE7', '-DGXOS_AARCH64_PHASE8',
...
Write-Host '[1/10] Compiling ARM64 UEFI GOP loader...' -ForegroundColor Yellow
...
$kernelObjects = @()
$kernelFlags = @(
    '--target=aarch64-none-elf','-std=c++14','-march=armv8-a','-O2','-ffreestanding','-nostdlib','-nostdinc++','-fno-builtin','-fno-stack-protector','-fno-exceptions','-fno-rtti',
    '-fno-unwind-tables','-fno-asynchronous-unwind-tables','-fno-pic','-fno-pie','-mcmodel=small','-mgeneral-regs-only','-mstrict-align','-mno-outline-atomics',
    '-ffunction-sections','-fdata-sections','-Wno-c++11-narrowing','-DGXOS_AARCH64_PHASE4','-DGXOS_AARCH64_PHASE5','-DGXOS_AARCH64_PHASE6','-DGXOS_AARCH64_PHASE7','-DGXOS_AARCH64_PHASE8',
...
Write-Host '[3/10] Compiling common compositor, App Model, and ARM64 input sources...' -ForegroundColor Yellow
...
Write-Host '[4/10] Linking Phase 8 kernel.elf...' -ForegroundColor Yellow
...
$appObject = Join-Path $OutputDirectory 'phase8-arm64-gui-proof.o'
$appElf = Join-Path $OutputDirectory 'phase8-arm64-gui-proof.elf'
$appSource = Join-Path $repoRoot 'sdk\samples\phase8_arm64_gui\main.cpp'
$appFlags = @('--target=aarch64-none-elf','-std=c++14','-march=armv8-a','-O2','-ffreestanding','-nostdlib','-nostdinc++','-fno-builtin','-fno-exceptions','-fno-rtti','-fno-stack-protector',
    '-fno-unwind-tables','-fno-asynchronous-unwind-tables','-fno-pic','-fno-pie','-mcmodel=small','-mgeneral-regs-only','-mstrict-align','-I',(Join-Path $repoRoot 'sdk\include'),'-c',$appSource,'-o',$appObject)
Write-Host '[5/10] Independently compiling graphical ARM64 NativeElf...' -ForegroundColor Yellow
...
if (!(Test-Elf $appElf 183)) { throw 'Phase 8 application failed ELF64 EM_AARCH64 verification' }

$hostCxx = 'C:\mingw64\bin\g++.exe'; if (!(Test-Path -LiteralPath $hostCxx -PathType Leaf)) { $hostCxx = 'g++.exe' }
$sdkInclude = Join-Path $repoRoot 'sdk\include'
$abiTest = Join-Path $OutputDirectory 'native-abi-layout-test.exe'
$controlsTest = Join-Path $OutputDirectory 'aarch64-phase8-host-tests.exe'
$amd64Object = Join-Path $OutputDirectory 'phase8-gui-amd64-abi.o'
Write-Host '[7/10] Running ABI layout, negative controls, and AMD64 compile validation...' -ForegroundColor Yellow
Invoke-Checked $hostCxx @('-std=c++17','-Wall','-Wextra','-O2','-idirafter',$repoRoot,'-I',$sdkInclude,(Join-Path $repoRoot 'tests\native_abi_layout_test.cpp'),'-o',$abiTest)
Invoke-Checked $abiTest @()
Invoke-Checked $hostCxx @('-std=c++17','-Wall','-Wextra','-O2','-I',$sdkInclude,(Join-Path $repoRoot 'tests\aarch64_phase8_host_tests.cpp'),'-o',$controlsTest)
Invoke-Checked $controlsTest @()
Invoke-Checked $hostCxx @('-std=c++14','-m64','-ffreestanding','-fno-builtin','-fno-exceptions','-fno-rtti','-I',$sdkInclude,'-c',$appSource,'-o',$amd64Object)

Write-Host '[8/10] Staging normal /Apps package and desktop resources...' -ForegroundColor Yellow
$stageRoot = Join-Path $OutputDirectory 'ramdisk-root'
$null = New-Item -ItemType Directory -Path (Join-Path $stageRoot 'Apps\Phase8Arm64GuiProof\bin\arm64') -Force
$null = New-Item -ItemType Directory -Path (Join-Path $stageRoot 'Apps\Phase8WrongMachine\bin\arm64') -Force
...
Copy-Item (Join-Path $repoRoot 'sdk\samples\phase8_arm64_gui\app.json') (Join-Path $stageRoot 'Apps\Phase8Arm64GuiProof\app.json')
Copy-Item (Join-Path $repoRoot 'sdk\samples\phase8_arm64_gui\wrongmachine-app.json') (Join-Path $stageRoot 'Apps\Phase8WrongMachine\app.json')
Copy-Item $appElf (Join-Path $stageRoot 'Apps\Phase8Arm64GuiProof\bin\arm64\phase8-arm64-gui-proof.elf')
Copy-Item (Join-Path $repoRoot 'Apps\HelloWorld\bin\amd64\helloworld.elf') (Join-Path $stageRoot 'Apps\Phase8WrongMachine\bin\arm64\wrongmachine.elf')
...
if (!(Test-Path -LiteralPath $wallpaper -PathType Leaf)) { throw "Normal desktop wallpaper resource is missing: $wallpaper. Run scripts\generate-wallpaper-pack.ps1 first." }
...
if ($LASTEXITCODE -ne 0) { throw 'Phase 8 FAT32 ramdisk staging failed' }
...
Write-Host '[9/10] Recording ARM64 artifact metadata...' -ForegroundColor Yellow
& $llvmReadobj '--file-headers' '--program-headers' '--sections' '--symbols' $appElf | Set-Content (Join-Path $OutputDirectory 'phase8-app-llvm-readobj.txt') -Encoding ascii
...
Write-Host '[10/10] Verifying staged package metadata...' -ForegroundColor Yellow
if (!(Test-Path (Join-Path $stageRoot 'Apps\Phase8Arm64GuiProof\app.json'))) { throw 'Phase 8 app manifest was not staged' }
...
Write-Host 'Build complete. Use scripts\test-aarch64-phase8.ps1 for three fresh interactive boots.' -ForegroundColor Green
