...
Assert-True ($virtioGpuCpp.Contains('cacheMode=uc(pcd+pwt)')) 'virtio_gpu.cpp should report the UC cache mode'
Assert-True ($virtioGpuCpp.Contains('cacheAttrs=')) 'virtio_gpu.cpp should log the cache-attribute status field'
Assert-True ($virtioGpuCpp.Contains('MMIO transport summary mmioMapped=yes')) 'virtio_gpu.cpp should summarize the mapped transport'
Assert-True ($virtioGpuCpp.Contains('sanityReads=ok')) 'virtio_gpu.cpp should report the read-only sanity-read result'
Assert-True ($virtioGpuCpp.Contains('reason=transport writes intentionally disabled')) 'virtio_gpu.cpp should carry the final stop reason'
Assert-True ($virtioGpuCpp.Contains('displayInfo=not-queried')) 'virtio_gpu.cpp should keep display info unqueried in this pass'
...
Assert-True ($virtioGpuCpp.Contains('mmioMapped=yes')) 'virtio_gpu.cpp should report the mapped transport state in the final probe line'
Assert-True ($virtioGpuCpp.Contains('cacheMode=uc(pcd+pwt)')) 'virtio_gpu.cpp should report the cache mode in the final probe line'
...
Assert-True ($initializeBody.Contains('record_probe_outcome(state, false, DisplayInfoOutcome::NotQueried, 0, transport.mmioStopReason);')) 'initialize_device should end with the transport stop reason'
...
Assert-True ($virtioGpuCpp.Contains('mmio_read32(device_cfg_addr(transport, 0x08))')) 'virtio_gpu.cpp should read the first device config field only after mapping'
Assert-True ($virtioGpuCpp.Contains('mmio_read32(device_cfg_addr(transport, 0x0C))')) 'virtio_gpu.cpp should read the second device config field only after mapping'
