...
Assert-True ($mmioHeader.Contains('MMIO mappings must be kernel-only and no-executable')) 'mmio.h should enforce kernel-only, no-exec MMIO mappings'
Assert-True ($mmioHeader.Contains('PAT/MTRR')) 'mmio.h should note the cache-attribute TODO'
Assert-True ($mmioHeader.Contains('runtime MMIO page-table mapping is not implemented yet')) 'mmio.h should name the runtime page-table blocker'
Assert-True ($mmioHeader.Contains('MMIO unmap is not implemented yet; runtime page-table tracking is absent')) 'mmio.h should describe the unmap blocker'
...
Assert-True ($virtioGpuCpp.Contains('MMIO mappings must be kernel-only and no-executable')) 'virtio_gpu.cpp should keep the mapping safety rule visible'
Assert-True ($virtioGpuCpp.Contains('cacheAttrs=')) 'virtio_gpu.cpp should log the cache-attribute status field'
Assert-True ($virtioGpuCpp.Contains('todo(PAT/MTRR)')) 'virtio_gpu.cpp should keep the cache-attribute TODO visible'
Assert-True ($virtioGpuCpp.Contains('runtime MMIO page-table mapping is not implemented yet')) 'virtio_gpu.cpp should surface the runtime page-table blocker'
Assert-True ($virtioGpuCpp.Contains('runtime MMIO mapping helper is still stubbed')) 'virtio_gpu.cpp should surface the stubbed helper blocker'
