...
$outputDir = Join-Path $env:TEMP "guidex-navigator-javascript-js24-test"
New-Item -ItemType Directory -Force -Path $outputDir | Out-Null
$exe = Join-Path $outputDir "navigator_javascript_js24_test.exe"
...
$compileOutput = & g++ -std=c++17 -O2 -Wall -Wextra -Werror -pedantic `
    ...
    tests/navigator_javascript_js24_test.cpp `
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "Navigator JavaScript JS24 focused test build failed."
}

& $exe
if ($LASTEXITCODE -ne 0) { throw "Navigator JavaScript JS24 focused tests failed." }
...
if ($LASTEXITCODE -ne 0) {
    $strict | Write-Output
    throw "Navigator JavaScript JS24 strict adapter/runtime build failed."
}

Write-Output "Navigator JavaScript JS24 focused smoke PASS"
