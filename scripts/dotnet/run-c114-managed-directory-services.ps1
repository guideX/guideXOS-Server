...
if ($FreshBootCount -lt 3) { throw "C114 requires at least three fresh boots." }
...
if ([string]::IsNullOrWhiteSpace($EvidenceRoot)) {
    $EvidenceRoot = Join-Path $RepoRoot "out\dotnet\c011ec114-managed-directory-services"
}
...
if (-not $EvidenceRoot.StartsWith($allowedRoot, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "C114 evidence must remain under $allowedRoot"
}
...
$stagingImage = Join-Path $EvidenceRoot "staging\ramdisk-c114.img"
...
function Invoke-C114Boot([string]$EspPath, [string]$SerialPath, [string]$StdoutPath,
    ...
    try {
        ...
        while ((Get-Date) -lt $deadline) {
            ...
            if (Test-Path -LiteralPath $SerialPath) {
                ...
                if ($partial -match '(?m)^\[C114-RESULT\] outcome=(?:PASS|FAIL)') { break }
            }
            $process.Refresh()
...
function Assert-C114Serial([string]$Serial) {
    $required = @(
        '^\[C114-APPMODEL\] catalogValid=true result=PASS',
        '^\[C114-RESULT\] outcome=PASS',
        '^\[C114-MIXED\].*result=PASS',
        '^\[C114-NOTES-LIST\] entries=NOTES.TXT,SECOND.TXT result=PASS',
        '^\[C114-MANAGED-OUTPUT\] C114-NOTES list=PASS source=VFS',
        '^\[C114-MANAGED-OUTPUT\] C114-NOTES open=PASS source=directory-list',
        '^\[C114-MANAGED-OUTPUT\] C114-NOTES action=edit result=PASS',
        '^\[C114-MANAGED-OUTPUT\] C114-NOTES action=save result=PASS',
        '^\[C114-MANAGED-OUTPUT\] C114-NOTES threadStatic=PASS',
        '^\[C113-FILE-WRITE\].*result=PASS',
        '^\[C113-MANAGED-OUTPUT\] C113-NOTES launch=PASS file=LOADED source=VFS',
        '^\[C114-VFS-VERIFY\] stage=first-stat .*result=PASS',
        '^\[C114-VFS-VERIFY\] stage=first-save .*result=PASS',
        '^\[C114-VFS-VERIFY\] stage=second-stat .*result=PASS',
        '^\[C114-VFS-VERIFY\] stage=second-open .*result=PASS',
        '^\[C114-CAPABILITY-DOWNGRADE\] directoryList=omitted .*result=PASS',
        '^\[C114-CAPABILITY-DOWNGRADE\] fileStat=omitted .*result=PASS',
        '^\[C114-DIRECTORY-NEGATIVE\].*result=PASS',
        '^\[C114-CAPACITY\].*result=PASS',
        '^\[C112-ABI-MISMATCH\].*result=PASS',
        '^\[C114-NATIVE-REGRESSION\] app=Notepad result=PASS',
        '^\[C114-REGRESSION\] app=Counter result=PASS',
        '^\[C114-REGRESSION\] app=Status result=PASS')
    foreach ($pattern in $required) {
        if ($Serial -notmatch "(?m)$pattern") { throw "C114 missing required serial marker: $pattern" }
    }
    if ($Serial -match '(?m)^\[C114-[^\r\n]*result=FAIL|PageFault|triple.?fault|FAIL_FAST|fatal kernel failure|boot failure') {
        throw "C114 serial output contains a failure or fault marker."
    }
    return [pscustomobject]@{
...
if (-not $SkipManagedBuild -and -not $useProvidedComposite) {
    ...
    Invoke-Checked "powershell" @(
        ...
        "-AllocationMode", "Allocating", "-ManagedProjectMode", "C114Composite",
...
if (-not (Test-Path -LiteralPath $compositeElf -PathType Leaf)) { throw "C114 composite ELF is missing: $compositeElf" }
Invoke-Checked "powershell" @(
...
if (-not $SkipKernelBuild) {
    Invoke-Checked "mingw32-make" @(
        ...
        "EXTRA_CFLAGS=-DGXOS_NATIVEAOT_PRODUCTION_APPLICATION -DGXOS_NATIVEAOT_PRODUCTION_COMPOSITE_LAUNCH -DGXOS_NATIVEAOT_C112_REUSABLE_MANAGED_APPLICATION -DGXOS_NATIVEAOT_C113_MANAGED_FILE_SERVICES -DGXOS_NATIVEAOT_C114_MANAGED_DIRECTORY_SERVICES")
}
...
NativeHostCallTable C113 prefix: 88 bytes; C114 table: 104 bytes
fileReadAll offset 72; fileWriteAll offset 80; directoryList offset 88; fileStat offset 96
capabilities: surface,text,primitive,action,close,launchContext,log,fileRead,fileWrite,directoryList,fileStat
file path root: /system/apps/; maximum path bytes: 96; maximum file bytes: 16384
directory maximum entries: 64; maximum entry name bytes: 127; entry stride: 144; file-info size: 16
managed file results: success=0 notFound=-10 invalidPath=-11 bufferTooSmall=-12 tooLarge=-13 ioFailure=-14 capabilityUnavailable=-15 invalidArgument=-16 notDirectory=-17 entryNameTooLong=-18
...
@"
managed: build-managed-hostlog-proof.ps1 -UseGuideXosRuntimePack -ProductionApplication -PersistentCompositeLifecycle -AllocationMode Allocating -ManagedProjectMode C114Composite
staging: generate-wallpaper-pack.ps1 -ProductionCompositeApplicationPath <HostLogProof.elf> -C114ManagedDirectoryServices
kernel: mingw32-make -C kernel -B ARCH=amd64 EXTRA_CFLAGS=-DGXOS_NATIVEAOT_PRODUCTION_APPLICATION -DGXOS_NATIVEAOT_PRODUCTION_COMPOSITE_LAUNCH -DGXOS_NATIVEAOT_C112_REUSABLE_MANAGED_APPLICATION -DGXOS_NATIVEAOT_C113_MANAGED_FILE_SERVICES -DGXOS_NATIVEAOT_C114_MANAGED_DIRECTORY_SERVICES
...
if (-not $SkipQemu) {
    ...
    for ($index = 1; $index -le $FreshBootCount; $index++) {
        ...
        $boot = Invoke-C114Boot $espRoot $serialPath $stdoutPath $stderrPath $qemuPath $ovmfPath
        try { $classification = Assert-C114Serial $boot.serial }
        ...
        Write-Host ("[C114] boot={0} outcome={1} serial={2}" -f $index, $classification.outcome, $serialPath)
        if ($classification.outcome -ne "PASS") { throw "C114 fresh boot $index failed: $($classification.error)" }
    }
}
...
$evidenceSerial = if ($bootResults.Count -gt 0) {
    ...
    @("QEMU not executed; build-only evidence.")
}
$evidenceSerial | Where-Object { $_ -match '^\[C114-DIRECTORY-(ENTRY|LIST)\]' } |
    Set-Content -LiteralPath (Join-Path $EvidenceRoot "managed-observed-listing.txt") -Encoding ASCII
@"
GXOSAPP.ELF (managed composite image)
NOTES.TXT (24 bytes, regular file)
SECOND.TXT (23 bytes, regular file)
"@ | Set-Content -LiteralPath (Join-Path $EvidenceRoot "native-expected-listing.txt") -Encoding ASCII
$evidenceSerial | Where-Object { $_ -match '^\[C114-(FILE-STAT|VFS-VERIFY)\]' } |
    Set-Content -LiteralPath (Join-Path $EvidenceRoot "metadata-results.txt") -Encoding ASCII
$evidenceSerial | Where-Object { $_ -match '^\[C111-SURFACE(-TEXT)?\].*(Managed Notes|Content:|Selected:|Status:|size=)' } |
    Set-Content -LiteralPath (Join-Path $EvidenceRoot "interaction-ui-evidence.txt") -Encoding ASCII
$evidenceSerial | Where-Object { $_ -match '^\[C114-CAPACITY\]' } |
    Set-Content -LiteralPath (Join-Path $EvidenceRoot "capacity-truncation.txt") -Encoding ASCII
$evidenceSerial | Where-Object { $_ -match '^\[C112-ABI-MISMATCH\]' } |
    Set-Content -LiteralPath (Join-Path $EvidenceRoot "unsupported-abi.txt") -Encoding ASCII
$evidenceSerial | Where-Object { $_ -match '^\[C114-DIRECTORY-NEGATIVE\]' } |
    Set-Content -LiteralPath (Join-Path $EvidenceRoot "invalid-abi-results.txt") -Encoding ASCII
@"
compositeElfSha256=$(Get-Hash $compositeElf)
kernelSha256=$(Get-Hash $kernelPath)
"@ | Set-Content -LiteralPath (Join-Path $EvidenceRoot "image-hashes.txt") -Encoding ASCII
...
$sourceFiles = @(
    ...
    (Join-Path $RepoRoot "scripts\dotnet\run-c114-managed-directory-services.ps1"),
    (Join-Path $RepoRoot "docs\dotnet\NATIVEAOT_C114_MANAGED_DIRECTORY_SERVICES.md"))
...
heapPreserveCount=$(@($evidenceSerial | Where-Object { $_ -match '^\[NATIVEAOT-HEAP\] action=preserve' }).Count) (boot-01 observed; resident managed calls)
...
@"
directoryList omitted -> CapabilityUnavailable
fileStat omitted -> CapabilityUnavailable
nativeResult=PASS
...
@"
sequence=Workspace -> Notes(list,first,save,second) -> Notepad -> Counter -> Status
managedNotesDirectoryBrowser=PASS
nativeNotepad=PASS
managedCounter=PASS
managedStatus=PASS
"@ | Set-Content -LiteralPath (Join-Path $EvidenceRoot "mixed-sequence.txt") -Encoding ASCII
@"
C113 read/write/save/reload foundation=PASS
C114 reusable managed application regression=PASS
...
@"
directory=/system/apps
expected=GXOSAPP.ELF,NOTES.TXT,SECOND.TXT (lexical managed order)
NOTES.TXT type=regular size=24
SECOND.TXT type=regular size=23
/system/apps type=directory
/system/apps/MISSING.C114=NotFound
capacity=1 count<=1 hasMore=1; full enumeration count<=64
"@ | Set-Content -LiteralPath (Join-Path $EvidenceRoot "directory-stat-proof.txt") -Encoding ASCII
$manifest = [ordered]@{
    schemaVersion = 1; phase = "C114"; outcome = if ($SkipQemu) { "BUILD_ONLY" } else { "PASS" }
    ...
    hostAbi = [ordered]@{ version = 1; c113PrefixSize = 88; tableSize = 104; fileReadOffset = 72; fileWriteOffset = 80; directoryListOffset = 88; fileStatOffset = 96; filePathMaxBytes = 96; fileMaxBytes = 16384; maxEntries = 64; maxNameBytes = 127; entryStride = 144; fileInfoSize = 16 }
    capabilityBits = "surface=0x1,text=0x2,primitive=0x4,action=0x8,close=0x10,launchContext=0x20,log=0x40,fileRead=0x80,fileWrite=0x100,directoryList=0x200,fileStat=0x400"
    vfsPath = "/system/apps"; testFiles = "NOTES.TXT (24), SECOND.TXT (23)"; savedContent = "Hello from Managed Notes [edited]"
    sequence = "Workspace -> Notes(list,first,save,second) -> Notepad -> Counter -> Status"
    productionConfiguration = "C114Composite + PersistentCompositeLifecycle + GXOS_NATIVEAOT_C112_REUSABLE_MANAGED_APPLICATION + GXOS_NATIVEAOT_C114_MANAGED_DIRECTORY_SERVICES"
    ...
    documentation = "docs/dotnet/NATIVEAOT_C114_MANAGED_DIRECTORY_SERVICES.md"
}
$manifest | ConvertTo-Json -Depth 35 | Set-Content -LiteralPath (Join-Path $EvidenceRoot "c114.manifest.json") -Encoding ASCII
Write-Host "C114 outcome=$($manifest.outcome) evidence=$EvidenceRoot" -ForegroundColor Green
exit 0
