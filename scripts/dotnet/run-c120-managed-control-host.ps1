param(
    ...
    [ValidateSet("C120", "C121", "C122", "C123", "C124")]
...
function Assert-C120Serial([string]$Serial) {
    ...
    if (-not $isC121 -and -not $isC122 -and -not $isC123 -and -not $isC124) {
        $required += @(
    ...
    if (-not $isC121 -and -not $isC122 -and -not $isC123 -and -not $isC124 -and $spaceMarker -ne 1) { throw "C120 expected one exact-once Space marker, got $spaceMarker." }
    $saveActivation = @([regex]::Matches($Serial,
        '(?m)^\[C102-MANAGED-OUTPUT\] C120-ACTIVATE control=Save result=PASS\r?$')).Count
    if (-not $isC121 -and -not $isC122 -and -not $isC123 -and -not $isC124 -and $saveActivation -ne 1) { throw "C120 expected one managed Save activation, got $saveActivation." }
    if ($Serial -match '(?m)^\[(?:C120|C121|C122|C123|C124)-[^\r\n]*FAIL|PageFault|triple.?fault|FAIL_FAST|fatal kernel failure|boot failure') {
...
if (-not $SkipManagedBuild -and -not $providedComposite) {
    ...
    Invoke-Checked "powershell" @(
        ...
        "-AllocationMode", "Allocating", "-ManagedProjectMode",
         $(if ($isC124) { "C124Composite" } elseif ($isC123) { "C123Composite" } elseif ($isC122) { "C122Composite" } elseif ($isC121) { "C121Composite" } else { "C120Composite" }),
        "-PythonExe", $PythonExe)
}
...
if ($isC124) { $kernelFlags += " -DGXOS_NATIVEAOT_C121_MANAGED_CHECKBOX -DGXOS_NATIVEAOT_C122_MANAGED_LABEL -DGXOS_NATIVEAOT_C123_MANAGED_SEPARATOR -DGXOS_NATIVEAOT_C124_MANAGED_RADIO_BUTTON" }
...
$evidenceSerial | Where-Object { $_ -match '^\[(?:C124|C123|C122|C121|C120|C119|C118|C117|C116|C115)-' } |
...
$manifest = [ordered]@{
    ...
    controlHost = [ordered]@{ api = "GuideXosControlHost"; capacity = 8; pickerCapacity = 2; tests = if($isC124){"radio host focused suite"}elseif($isC123){33}elseif($isC122){22}elseif($isC121){17}else{50}; legacyC120HostSuite = if($isC121 -or $isC122 -or $isC123 -or $isC124){"separate C120 runner"}else{"same image"}; modal = "one shallow picker scope with saved-ID restoration and forward fallback" }
    ...
    notes = [ordered]@{ order = if ($isC124) { "Open, Save, Save As, Show Path, Full Path, File Name, Document; label and separator are not registered" } elseif ($isC121 -or $isC122 -or $isC123) { "Open, Save, Save As, Show Path, Document; label and separator are not registered" } else { "Open, Save, Save As, Document" }; initialFocus = "none"; commands = if ($isC124) { "managed Open/Save/Save As; checkbox controls path-label visibility; radio group controls full-path/file-name presentation; native Reload retained" } elseif ($isC123) { "managed Open/Save/Save As; GuideXosLabel controls Path presentation; GuideXosSeparator divides content/status from command controls; native Reload retained" } elseif ($isC122) { "managed Open/Save/Save As; GuideXosLabel controls Path presentation; native Reload retained" } elseif ($isC121) { "managed Open/Save/Save As; checkbox controls Path presentation; native Reload retained" } else { "managed Open/Save/Save As; native Reload retained" }; space = if ($isC124) { "radio selection commits only on KeyChar Space; checkbox visibility remains independent" } elseif ($isC123 -or $isC122) { "checkbox toggles label visibility only from KeyChar Space; label and separator have no input API" } elseif ($isC121) { "checkbox toggles only from KeyChar Space; text/button/list/input routing remains isolated" } else { "exactly one Save activation from KeyChar Space" } }
    ...
    documentation = if ($isC124) { "docs\dotnet\NATIVEAOT_C124_MANAGED_RADIO_BUTTON.md" } elseif ($isC123) { "docs\dotnet\NATIVEAOT_C123_MANAGED_SEPARATOR.md" } elseif ($isC122) { "docs\dotnet\NATIVEAOT_C122_MANAGED_LABEL.md" } elseif ($isC121) { "docs\dotnet\NATIVEAOT_C121_MANAGED_CHECKBOX.md" } else { "docs\dotnet\NATIVEAOT_C120_MANAGED_CONTROL_HOST.md" }
}
