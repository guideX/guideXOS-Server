...
$testBinary = Join-Path $outputDirectory "native-elf-runtime-host-test.exe"
New-Item -ItemType Directory -Force -Path $outputDirectory | Out-Null

$gxx = (Get-Command g++ -ErrorAction Stop).Source
$sources = @(
    (Join-Path $repoRoot "tests/native_elf_runtime_host_test.cpp"),
...
if ($LASTEXITCODE -ne 0) { throw "NativeElf runtime host test build failed" }

& $testBinary
if ($LASTEXITCODE -ne 0) { throw "NativeElf runtime host test failed" }
