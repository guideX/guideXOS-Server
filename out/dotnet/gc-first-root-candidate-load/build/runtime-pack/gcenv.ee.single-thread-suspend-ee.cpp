...
extern "C" void __cdecl guideXosNativeAotFirstPerThreadRootThreadStaticStorageEntered(uintptr_t thread, uintptr_t storage);
...
{
    guideXosNativeAotSuspendEeEntry((uint32_t)reason);
...
void GCToEEInterface::GcScanRoots(ScanFunc* fn, int condemned, int max_gen, ScanContext* sc)
{
    ...
    guideXosNativeAotFirstPerThreadRootForeachThreadEntered();
    {
        ...
        {
            guideXosNativeAotFirstPerThreadRootThreadExcluded(reinterpret_cast<uintptr_t>(pThread), 1u);
            continue;
        }

        if (GCHeapUtilities::GetGCHeap()->IsThreadUsingAllocationContextHeap(pThread->GetAllocContext(), sc->thread_number))
        {
            guideXosNativeAotFirstPerThreadRootThreadIncluded(reinterpret_cast<uintptr_t>(pThread));
            InlinedThreadStaticRoot* pRoot = pThread->GetInlinedThreadStaticList();
            guideXosNativeAotFirstPerThreadRootThreadStaticListObserved(reinterpret_cast<uintptr_t>(pThread), reinterpret_cast<uintptr_t>(pRoot));
            while (pRoot != NULL)
            {
            ...
            Object** threadStaticStorage = pThread->GetThreadStaticStorage();
            guideXosNativeAotFirstPerThreadRootThreadStaticStorageEntered(reinterpret_cast<uintptr_t>(pThread), reinterpret_cast<uintptr_t>(threadStaticStorage));
...
{
    guideXosNativeAotAllocationContextFixupEnumerationEntry();
    FOREACH_THREAD(thread)
    {
        guideXosNativeAotAllocationContextFixupContextVisited(reinterpret_cast<uintptr_t>(thread->GetAllocContext()));
...
{
    guideXosNativeAotDisablePreemptiveEntry();
#ifndef DACCESS_COMPILE
