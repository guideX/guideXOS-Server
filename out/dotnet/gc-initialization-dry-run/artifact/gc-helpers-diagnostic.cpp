...
extern "C" uint32_t g_guidexos_nativeaot_gc_startup_reserve_pages;
extern "C" uint32_t g_guidexos_nativeaot_gc_startup_reserve_mib;
...
bool InitializeGC()
{
    g_guidexos_nativeaot_gc_startup_diagnostic_stage = 0x62000000u;
    ...
    if (FAILED(hr)) {
        g_guidexos_nativeaot_gc_startup_diagnostic_stage =
            0x63000000u | (static_cast<uint32_t>(hr) & 0x00FFFFFFu);
        return false;
    }
    ...
    g_guidexos_nativeaot_gc_startup_diagnostic_stage = 0x64000000u;
    hr = g_pGCHeap->Initialize();
    if (FAILED(hr)) {
        // Preserve the exact startup-platform reservation request that
        // failed. This lets the bounded QEMU diagnostic distinguish an
        // undersized VM/frame budget from a later GC initialization failure.
        g_guidexos_nativeaot_gc_startup_diagnostic_stage =
            0x65000000u |
            ((g_guidexos_nativeaot_gc_startup_reserve_mib != 0
                ? g_guidexos_nativeaot_gc_startup_reserve_mib
                : g_guidexos_nativeaot_gc_startup_reserve_pages) & 0x00FFFFFFu);
        return false;
    }

    g_guidexos_nativeaot_gc_startup_diagnostic_stage = 0x66000000u;
    if (!RhInitializeFinalization()) {
        g_guidexos_nativeaot_gc_startup_diagnostic_stage = 0x67000000u;
        return false;
    }

    // Initialize HandleTable.
    g_guidexos_nativeaot_gc_startup_diagnostic_stage = 0x68000000u;
    if (!GCHandleUtilities::GetGCHandleManager()->Initialize()) {
        g_guidexos_nativeaot_gc_startup_diagnostic_stage = 0x69000000u;
        return false;
    }

    g_guidexos_nativeaot_gc_startup_diagnostic_stage = 0x6A000000u;
    return true;
}
