...
The current host call surface is intentionally small and fixed for this experiment: `log`, `get_api_version`, `request_window`, `draw_text`, `draw_rect`, `wait_for_close`, `poll_event`, `file_exists`, and `file_read_all`. ABI v1 appends reusable extensions for `request_window_ex`, chunked `file_read`, retained `present_frame`, `get_ticks_ms`, hosted-development workspace calls `file_stat`, `file_read_workspace`, `file_list`, `file_write_all`, `file_create_directory`, and `file_remove`, plus the hosted-only `build_project_start`, `build_project_poll`, and `build_project_release` calls. It then appends a separate bare-metal build/VFS capability block: `bare_metal_build_project_start`, `bare_metal_build_project_poll`, `bare_metal_build_project_release`, and the six VFS operations. The directory calls are exact-path, non-recursive development operations used by Developer Studio's rollback-aware project generator; the hosted build calls run only the fixed Native GUI Application recipe, while bare-metal build calls select the kernel VFS compiler subset and validate its NativeElf artifact. Existing v1 table offsets are preserved and all extensions are appended; callers gate every extension by the advertised table size.
