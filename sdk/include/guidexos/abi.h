...
typedef struct gx_host_calls {
    ...
    /* In-OS Developer Studio build service. These slots are append-only and
     * are exposed only by a NativeElf runtime with the VFS compiler. */
    ...
    /* In-OS Developer Studio Run service. These slots are append-only and
     * are exposed only by a NativeElf runtime with the App Model resolver. */
