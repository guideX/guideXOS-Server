...
{
    log(ctx, "DEVELOPER_STUDIO_PHASE28M_TARGET_START");
    int root_value = 100;
    int before_call = 27;
    ...
    int after_call = 200;
    int window = gx_window_create(320, 180, "Developer Studio Phase 28M");
    if (window == 0) return -1;
    if (gx_window_set_text(window, "28M GUI") != 0) return -2;
