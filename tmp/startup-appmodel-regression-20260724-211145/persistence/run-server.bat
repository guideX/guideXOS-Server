...
set "EXE=%~dp0guideXOSServer.exe"

if not exist "%EXE%" (
    echo ERROR: Missing hosted runtime executable: %EXE%
...
set "RC=%ERRORLEVEL%"
if "%RC%"=="-1073741515" (
    echo ERROR: Hosted runtime failed to start with 0xC0000135. A runtime DLL dependency is missing.
