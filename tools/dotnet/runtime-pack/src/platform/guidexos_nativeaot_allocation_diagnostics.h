...
enum {
    GUIDEXOS_NATIVEAOT_C011EC53_MAX_ALLOCATIONS = 256u,
    GUIDEXOS_NATIVEAOT_C011EC53_MAX_COLLECTIONS = 4u,
};

/*
 * C011EC53 follows the C40 recovered compacted tail after C41 has finished
 * its fixed eight-allocation provenance window.  The record deliberately
 * separates representation/visibility, eligibility, consideration,
 * selection, and consumption.  No field is an allocator input: every value
 * is a scalar snapshot or a source-observed event from the locked Workstation
 * GC path.
 */
typedef struct guidexos_nativeaot_c011ec53_collection_record {
    uint32_t observed;
    uint32_t ordinal;
    uint32_t condemnedGeneration;
    uint32_t collectionReason;
    ...
    uintptr_t ephemeralSegmentBase;
    uintptr_t ephemeralSegmentAllocated;
    uintptr_t ephemeralSegmentCommitted;
    uintptr_t ephemeralSegmentReserved;
    uintptr_t ephemeralGeneration;
} guidexos_nativeaot_c011ec53_collection_record;

typedef struct guidexos_nativeaot_c011ec53_allocation_record {
    uint32_t observed;
    uint32_t ordinal;
    uint32_t requestedPayload;
    uint32_t requestedSize;
    ...
    uintptr_t candidateResidualStart;
    uintptr_t candidateResidualEnd;
    uintptr_t selectedStart;
    uintptr_t selectedEnd;
    uintptr_t allocationPath;
    uintptr_t sentinel;
} guidexos_nativeaot_c011ec53_allocation_record;

typedef struct guidexos_nativeaot_c011ec53_lifecycle_record {
    uint32_t started;
    uint32_t preflightEmitted;
    uint32_t reclaimedEmitted;
    uint32_t lifecycleEmitted;
    uint32_t eligibilityEmitted;
    uint32_t consideredEmitted;
    uint32_t selectedEmitted;
    uint32_t reuseEmitted;
    uint32_t resumeEmitted;
    uint32_t completionMarkerEmitted;
    ...
    uint32_t allocatorVisible;
    uint32_t allocatorVisibleTransition;
    uint32_t tailEligible;
    uint32_t eligibilityTransition;
    uint32_t tailConsidered;
    uint32_t tailSelected;
    uint32_t tailConsumed;
    uint32_t directReuse;
    uint32_t segmentLevelReuse;
    uint32_t tailStillMapped;
    ...
    uint32_t c18Valid;
    uint32_t coffCodeManagerValid;
    uint32_t findMethodInfoValid;
    uint32_t rootScanValid;
    uint32_t markClosureValid;
    uint32_t plannerValid;
    uint32_t restartValid;
    uint32_t managedResumeValid;
    uint32_t pageFaultObserved;
    uint32_t failFastObserved;
    uint32_t requestActive;
    uint32_t reserved[3];
    ...
    uintptr_t tailSegmentCommittedAfter;
    uintptr_t tailSegmentReservedAfter;
    uintptr_t generation0StartBefore;
    uintptr_t generation0EndBefore;
    uintptr_t generation0SegmentBefore;
    uintptr_t generation0StartAfter;
    uintptr_t generation0EndAfter;
    uintptr_t generation0SegmentAfter;
    ...
    uintptr_t firstPointerAfter;
    uintptr_t firstLimit;
    uintptr_t firstPayload;
    uintptr_t firstAlignedSize;
    uintptr_t firstObjectType;
    uintptr_t firstSupplyingSegment;
    uintptr_t firstSupplyingGeneration;
    uintptr_t firstSentinel;
    uintptr_t firstReadback;

    guidexos_nativeaot_c011ec53_collection_record collections[
        GUIDEXOS_NATIVEAOT_C011EC53_MAX_COLLECTIONS];
    guidexos_nativeaot_c011ec53_allocation_record allocations[
...
/*
 * C011EC58 records the provenance of the value supplied to every production
 * generation_to_condemn call in the locked Workstation GC build.  The record
 * is deliberately scalar and fixed-size: the source observer never allocates,
 * never changes a GC decision, and stops recording rather than growing state.
 */
enum {
    GUIDEXOS_NATIVEAOT_C011EC58_MAX_ENTRIES = 256u,
    GUIDEXOS_NATIVEAOT_C011EC58_CALLSITE_CHECK_ONLY = 1u,
    GUIDEXOS_NATIVEAOT_C011EC58_CALLSITE_MULTI_HEAP = 2u,
    GUIDEXOS_NATIVEAOT_C011EC58_CALLSITE_SINGLE_HEAP = 3u,
    GUIDEXOS_NATIVEAOT_C011EC58_ORIGIN_CHECK_FOR_FULL_GC = 1u,
    GUIDEXOS_NATIVEAOT_C011EC58_ORIGIN_BUDGET_TRIGGER = 2u,
    GUIDEXOS_NATIVEAOT_C011EC58_ORIGIN_EPHEMERAL_OOS = 3u,
    GUIDEXOS_NATIVEAOT_C011EC58_ORIGIN_FULL_OOS = 4u,
    GUIDEXOS_NATIVEAOT_C011EC58_ORIGIN_OTHER = 5u,
    GUIDEXOS_NATIVEAOT_C011EC58_FREE_REGION_UNKNOWN = 0u,
    GUIDEXOS_NATIVEAOT_C011EC58_FREE_REGION_AVAILABLE = 1u,
    GUIDEXOS_NATIVEAOT_C011EC58_FREE_REGION_UNAVAILABLE = 2u,
};

typedef struct guidexos_nativeaot_c011ec58_entry_record {
    uint32_t observed;
    uint32_t ordinal;
    uint32_t callSite;
    uint32_t nInitial;
    ...
    uintptr_t tailEnd;
    int64_t gen1Budget;
} guidexos_nativeaot_c011ec58_entry_record;

typedef struct guidexos_nativeaot_c011ec58_lifecycle_record {
    uint32_t started;
    uint32_t entryCount;
    uint32_t lastN0Entry;
    uint32_t firstN2Entry;
    uint32_t n0Count;
    uint32_t n1Count;
    uint32_t n2Count;
    uint32_t originObservationCount;
    uint32_t allocationAttemptCount;
    uint32_t freeRegionObservationCount;
    uint32_t lastFreeRegionObserved;
    uint32_t lastFreeRegionResult;
    uint32_t lastFreeRegionPath;
    uint32_t b02Observed;
    uint32_t b02Crossed;
    uint32_t b12Eligible;
    uint32_t n0ToN2Proven;
    uint32_t tailMarkerEmitted;
    uint32_t restartObserved;
    uint32_t resumeObserved;
    uint32_t invariantFailures;
    uint32_t safeStopReason;
    uint32_t lastOriginBranch;
    uint32_t lastOriginNInitial;
    uint32_t lastOriginReason;
    uint32_t lastCollectionOrdinal;
    uint32_t lastPolicyOrdinal;
    uint32_t reserved[2];
    uintptr_t lastRequestSize;
    uintptr_t lastAlignedSize;
    uintptr_t lastAllocationContext;
    uintptr_t lastAllocationPointer;
    uintptr_t lastAllocationLimit;
    uintptr_t lastAllocationSegment;
    uintptr_t lastTailSegment;
    uintptr_t lastTailStart;
    uintptr_t lastTailEnd;
    int64_t lastGen1Budget;
    guidexos_nativeaot_c011ec58_entry_record entries[
...
/* C011EC62 traces the bounded production transition after the first
 * promotion-derived gen1 budget debit.  The record is intentionally
 * fixed-size and observational: source callbacks retain only scalar state
 * and never select a region, alter a budget, or invoke GC. */
enum {
    GUIDEXOS_NATIVEAOT_C011EC62_MAX_EVENTS = 256u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_PROMOTE = 1u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_GEN1_DEBIT = 2u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_POST_GC_ALLOCCTX = 3u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_ALLOC_ENTER = 4u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_ALLOC_RESULT = 5u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_GC_HEAP_ALLOC = 6u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_SOH_TRY_FIT = 7u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_ALLOCATE_SOH = 8u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_FREE_REGION = 9u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_N2_COMMIT = 10u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_POST_DEBIT_N0 = 11u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_B02 = 12u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_FIRST_N2 = 13u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_RESTART = 14u,
    GUIDEXOS_NATIVEAOT_C011EC62_EVENT_RESUME = 15u,
};

typedef struct guidexos_nativeaot_c011ec62_event_record {
    uint32_t observed;
    uint32_t eventOrdinal;
    uint32_t kind;
    uint32_t collectionOrdinal;
    ...
    uintptr_t candidateRegionCount;
    uintptr_t minimumCapacity;
    uint32_t entryOrdinal;
    uint32_t survivorCount;
    uintptr_t gen1DesiredAllocation;
    uintptr_t gen1GcNewBefore;
    uintptr_t gen1GcNewAfter;
    uintptr_t gen1GcNewAllocation;
    uintptr_t gen1NewRaw;
    uintptr_t promotedBytes;
    uintptr_t debitBytes;
    int64_t gen1NewSigned;
    int64_t b02Margin;
} guidexos_nativeaot_c011ec62_event_record;

typedef struct guidexos_nativeaot_c011ec62_lifecycle_record {
    uint32_t started;
    uint32_t strategy;
    uint32_t windowOpen;
    uint32_t eventCount;
    ...
    uint32_t postGcAllocContextObserved;
    uint32_t postGcAllocContextValid;
    uint32_t restartObserved;
    uint32_t managedResumeObserved;
    uint32_t managedAllocationCount;
    uint32_t firstAllocationObserved;
    uint32_t firstAllocationCompleted;
    uint32_t firstAllocationFast;
    uint32_t firstAllocationRare;
    uint32_t firstAllocationSucceeded;
    uint32_t allocationContextFit;
    uint32_t sohTryFitObserved;
    uint32_t sohTryFitResult;
    uint32_t allocateSohObserved;
    uint32_t allocateSohResult;
    uint32_t refillRequired;
    uint32_t freeRegionObserved;
    uint32_t freeRegionResult;
    uint32_t freeRegionFailureBranch;
    uint32_t n2CommitObserved;
    uint32_t postDebitN0Observed;
    uint32_t firstN2Observed;
    uint32_t b02Observed;
    uint32_t b02Crossed;
    uint32_t finalCondemnedGeneration;
    uint32_t collectionReason;
    ...
    uint32_t reserved[3];
    uint32_t promotionEventOrdinal;
    uint32_t debitEventOrdinal;
    uint32_t debitCollectionOrdinal;
    uint32_t postGcEventOrdinal;
    uint32_t firstAllocationEventOrdinal;
    uint32_t firstAllocationResultEventOrdinal;
    uint32_t n2CommitEventOrdinal;
    uint32_t postDebitN0EventOrdinal;
    uint32_t firstN2EventOrdinal;
    uint32_t firstN2EntryOrdinal;
    uint32_t postDebitN0EntryOrdinal;
    uint32_t postDebitN0CallSite;
    uint32_t postDebitN0NInitial;
    uint32_t postDebitN0Reason;
    uint32_t postDebitN0LastGcBeforeOom;
    uint32_t b02EventOrdinal;
    uint32_t b02PolicyEntryOrdinal;
    uint32_t b02NBefore;
    uint32_t b02NAfter;
    ...
    uint32_t n2CallSite;
    uint32_t n2Reason;
    uint32_t n2OriginBranch;
    uintptr_t promotionBytes;
    uintptr_t debitBytes;
    uintptr_t publishedGen1NewAllocation;
    uintptr_t promotionDesiredAllocation;
    uintptr_t promotionGen1GcNewBefore;
    uintptr_t promotionGen1GcNewAfter;
    uintptr_t postGcAllocationPointer;
    uintptr_t postGcAllocationLimit;
    uintptr_t postGcRemainingBytes;
    uintptr_t postGcAllocationContext;
    uintptr_t postGcGeneration;
    uintptr_t postGcActiveSegment;
    uintptr_t postGcRegionStart;
    uintptr_t postGcRegionEnd;
    uintptr_t postGcRegionRemaining;
    uintptr_t firstAllocationOrdinal;
    uintptr_t firstAllocationPayload;
    uintptr_t firstAllocationAlignedSize;
    uintptr_t firstAllocationPointerBefore;
    uintptr_t firstAllocationLimitBefore;
    uintptr_t firstAllocationPointerAfter;
    uintptr_t firstAllocationLimitAfter;
    uintptr_t firstAllocationObject;
    uintptr_t firstAllocationSegment;
    uintptr_t firstAllocationRegionStart;
    uintptr_t firstAllocationRegionEnd;
    uintptr_t firstAllocationRegionRemaining;
    uint32_t activeAllocationOrdinal;
    uint32_t activeAllocationEventOrdinal;
    uintptr_t activeAllocationPayload;
    uintptr_t activeAllocationSize;
    uintptr_t activeAllocationPointer;
    uintptr_t activeAllocationLimit;
    uintptr_t lastRequestSize;
    uintptr_t lastActiveSegment;
    uintptr_t lastRegionRemaining;
    uintptr_t lastCandidateRegionCount;
    uintptr_t lastMinimumCapacity;
    uint32_t lastSohTryFitBranch;
    uint32_t lastAllocateSohState;
    uint32_t lastFreeRegionBranch;
    uint32_t lastFreeRegionResult;
    uint32_t lastN2CommitBranch;
    uint32_t temporalInvariant;
    uint32_t successLevel;
    guidexos_nativeaot_c011ec62_event_record events[
...
/* C011EC65 keeps the C64 workload open across the complete post-debit tail
 * and records the allocator-to-OOS provenance without making a production
 * decision.  The callbacks only copy scalar values into these fixed-size
 * records; serial formatting is deferred to the explicit Finish call. */
enum {
    GUIDEXOS_NATIVEAOT_C011EC65_MAX_EVENTS = 1024u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_ATTEMPT = 1u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_NORMAL_REFILL = 2u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_POST_DEBIT = 3u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_ALLOC_FAIL = 4u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_REGION_SEARCH = 5u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_REGION_CANDIDATE = 6u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_REGION_RESULT = 7u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_EXPANSION = 8u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_OOS = 9u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_LAST_GC_BEFORE_OOM = 10u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_GEN_REQUEST = 11u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_GC_CALL = 12u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_N_INITIAL = 13u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_FIRST_DIVERGENCE = 14u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_RESTART = 15u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_RESUME = 16u,
    GUIDEXOS_NATIVEAOT_C011EC65_EVENT_ENTRY = 17u,
};

typedef struct guidexos_nativeaot_c011ec65_refill_record {
    uint32_t observed;
    uint32_t phase;
    uint32_t generation;
    uint32_t fitResult;
    uint32_t branch;
    uint32_t commitFailed;
    uint32_t shortSegmentEnd;
    uint32_t allocationState;
    uintptr_t requestSize;
    uintptr_t allocationContext;
    uintptr_t allocationPointer;
    ...
    uintptr_t candidateRegion;
    uint32_t regionResult;
    uint32_t regionBranch;
    uint32_t expansionAttempted;
    uint32_t expansionSucceeded;
    uint32_t reserved[2];
} guidexos_nativeaot_c011ec65_refill_record;

typedef struct guidexos_nativeaot_c011ec65_event_record {
    uint32_t observed;
    uint32_t eventOrdinal;
    uint32_t kind;
    uint32_t phase;
...
} guidexos_nativeaot_c011ec65_event_record;

typedef struct guidexos_nativeaot_c011ec65_lifecycle_record {
    uint32_t started;
    uint32_t variant;
    uint32_t eventCount;
    uint32_t eventOverflow;
    ...
    uint32_t postDebitObserved;
    uint32_t allocationFailureObserved;
    uint32_t regionSearchObserved;
    uint32_t regionCandidateObserved;
    uint32_t regionResultObserved;
    uint32_t expansionAttempted;
    uint32_t expansionSucceeded;
    uint32_t oosObserved;
    uint32_t lastGcBeforeOom;
    uint32_t genRequestObserved;
    uint32_t gcCallObserved;
    uint32_t nInitialObserved;
    uint32_t firstDivergenceObserved;
    uint32_t entryObserved;
    uint32_t completionObserved;
    uint32_t noPolicyMutation;
    uint32_t noAllocatorMutation;
    uint32_t freeRegionMutation;
    uint32_t requestedGenerationMutation;
    uint32_t noOosSuppression;
    uint32_t allocationAttemptCount;
    uint32_t regionEventCount;
    uint32_t oosEventCount;
    uint32_t retryCount;
    uint32_t normalAttemptEventOrdinal;
    uint32_t normalFitEventOrdinal;
    uint32_t postAttemptEventOrdinal;
    uint32_t postFitEventOrdinal;
    uint32_t firstDivergenceEventOrdinal;
    uint32_t oosEventOrdinal;
    uint32_t genRequestEventOrdinal;
    uint32_t nInitialEventOrdinal;
    uint32_t entryEventOrdinal;
    uint32_t firstCallSite;
    uint32_t firstRequestedGeneration;
    uint32_t firstNInitial;
    uint32_t firstCollectionReason;
    uint32_t firstOriginBranch;
    uint32_t firstLastGcBeforeOom;
    uint32_t firstDivergenceBranch;
    uint32_t postAllocationState;
    uint32_t postGeneration;
    uint32_t postFitBranch;
    uint32_t postCommitFailed;
    uint32_t postShortSegmentEnd;
    uint32_t postRegionResult;
    uint32_t postRegionBranch;
    uint32_t postExpansionAttempted;
    uint32_t postExpansionSucceeded;
    uint32_t allocationRegionObserved;
    uint32_t allocationRegionResult;
    uint32_t allocationRegionBranch;
    uint32_t allocationRegionGeneration;
    uint32_t postOosGeneration;
    uint32_t postOosReason;
    uint32_t postOosBranch;
    uint32_t postGcCallGeneration;
    uint32_t postGcCallReason;
    ...
    uintptr_t normalAllocationPointer;
    uintptr_t normalAllocationLimit;
    uintptr_t normalActiveSegment;
    uintptr_t normalRegionRemaining;
    uintptr_t normalFreeRegionsBefore;
    uintptr_t normalFreeRegionsAfter;
    uintptr_t normalCandidateRegion;
    uintptr_t postRequestSize;
    uintptr_t postAllocationContext;
    uintptr_t postAllocationPointer;
    uintptr_t postAllocationLimit;
    uintptr_t postActiveSegment;
    uintptr_t postRegionRemaining;
    uintptr_t postFreeRegionsBefore;
    uintptr_t postFreeRegionsAfter;
    uintptr_t postCandidateRegion;
    uintptr_t postCommittedBytes;
    uintptr_t postReservedBytes;
    uintptr_t allocationRegionSize;
    uintptr_t allocationRegion;
    uintptr_t allocationRegionFreeBefore;
    uintptr_t allocationRegionFreeAfter;
