...
/*
 * The locked Windows-targeted NativeAOT code manager calls the Win64
 * RtlVirtualUnwind import.  guideXOS executes the converted image without a
 * Windows kernel, so the import must be backed by the same AMD64 UNWIND_INFO
 * contract rather than left as a null placeholder.  This is a bounded,
 * allocation-free implementation of the Windows x64 unwind operations used
 * by NativeAOT's .pdata/.xdata records.  It is intentionally kept here as the
 * PAL import adapter; the locked CoffNativeCodeManager call site remains the
 * unwind decision point.
 */
namespace {
...
extern "C" void* __cdecl guideXosRtlVirtualUnwind(
    ...
    GuidexosRuntimeFunction* functionEntry, void* context,
    void** handlerData, uint64_t* establisherFrame,
    void* contextPointers) {
    (void)handlerType;
    if (functionEntry == nullptr || context == nullptr) return nullptr;
    ...
    const uint64_t functionStart = imageBase + functionEntry->beginAddress;
    ...
    for (uint32_t index = 0u; index < codeCount;) {
        ...
        if (!inPrologue || codeOffset <= controlOffset) {
            switch (unwindOp) {
                ...
                default:
                    // The first C011EC20 frame uses only the integer
                    // nonvolatile/stack operations above.  XMM save records
                    // consume their documented slots but do not affect the
                    // caller-state proof.
                    break;
            }
        }
