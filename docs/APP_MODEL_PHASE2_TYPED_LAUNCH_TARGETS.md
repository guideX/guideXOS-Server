...
| `DiskManager` | `BuiltInApp` with `appId=gxos.builtin.diskmanager` | Safe typed-ready built-in app; hosted and bare-metal dispatch both resolve directly to `DiskManager` with normal built-in-app behavior. |
