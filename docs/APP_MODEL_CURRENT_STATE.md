...
App Model v1.2 adds `DisplayOptions` as the next typed-ready built-in target (`gxos.builtin.displayoptions`). The shell labels `Settings`, `System Settings`, and `Control Panel` still intentionally bridge to it as non-fatal drift, so their user-visible launch behavior remains unchanged.
