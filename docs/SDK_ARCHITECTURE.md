...
- Available host calls: log, get_api_version, request_window, draw_text, draw_rect, wait_for_close, poll_event, file_exists, file_read_all, request_window_ex, file_read, present_frame, get_ticks_ms, hosted-development workspace calls file_stat, file_read_workspace, file_list, file_write_all, file_create_directory, and file_remove, hosted build calls build_project_start/poll/release, and owner-bound temporary Run Project calls development_run_prepare/start/poll/request_close/release. Workspace, build, and Run Project calls are hosted-development extensions, not a bare-metal filesystem or arbitrary launch contract. See `docs/DEVELOPMENT_RUN.md` for the Run Project validation and cleanup policy.
