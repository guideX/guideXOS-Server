...
using namespace gxos;
    ...
    while (std::getline(std::cin, line)){
        ...
        } else if (cmd=="gui.keyto") { if(!requireCompositor()) continue; uint64_t windowId=0; int keyCode=0, modifiers=0; std::string action; iss>>windowId>>keyCode>>action>>modifiers; if(windowId==0 || (action!="down" && action!="up")){ std::cout<<"Usage: gui.keyto <windowId> <keyCode> <down|up> [modifiers]"<<std::endl; continue; } ipc::Message m; m.type=(uint32_t)gui::MsgType::MT_InputKey; m.dstPid=Lifecycle::state().compositorPid; const std::string payload=std::to_string(windowId)+"|"+std::to_string(keyCode)+"|"+action+"|"+std::to_string(modifiers); m.data.assign(payload.begin(), payload.end()); ipc::Bus::publish("gui.input", std::move(m), false); std::cout<<"Key queued window="<<windowId<<" key="<<keyCode<<" action="<<action<<std::endl;
