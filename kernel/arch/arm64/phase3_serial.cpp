...
// Phase 3 uses the validated Phase-2 PL011 platform fallback until DTB
// parsing selects the active console address.
...
extern "C" uint64_t phase3_serial_base() { return gUartBase; }
...
extern "C" void phase3_serial_putc(char character)
{
...
extern "C" void phase3_serial_print(const char* text)
{
    if (!text) return;
    while (*text != 0) {
        if (*text == '\n') phase3_serial_putc('\r');
        phase3_serial_putc(*text++);
    }
}
...
extern "C" void phase3_serial_hex(uint64_t value)
{
    ...
    phase3_serial_print("0x");
    for (int shift = 60; shift >= 0; shift -= 4) phase3_serial_putc(digits[(value >> shift) & 0xf]);
}
...
extern "C" void phase3_serial_dec(uint64_t value)
{
    ...
    while (count != 0) phase3_serial_putc(digits[--count]);
}
