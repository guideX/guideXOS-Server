// IA-64 OHCI Host Controller Interface — Implementation
//
// Provides the platform-specific USB HCI backend for IA-64 (Itanium).
// Implements the kernel::usb::hci namespace functions declared in usb.h.
//
// On Itanium, OHCI/EHCI controllers are PCI devices.  PCI config
// space is accessed via MMIO (SAL/EFI firmware tables provide the
// base address).  For the ski simulator, USB hardware is not
// physically present, so init() returns false gracefully.
//
// When running on real hardware or HP Integrity servers, the PCI
// configuration space is at a platform-dependent MMIO base that
// would be obtained from ACPI/SAL tables.
//
...
// ================================================================
// MMIO helpers — IA-64 uses uncacheable loads/stores
// ================================================================
...
//
// On IA-64, PCI config space is typically MMIO-mapped by firmware.
// The base address comes from ACPI MCFG table.  For now, we try
// the common HP rx2600 / ski simulator address.
// ================================================================

static const uint64_t PCI_MCFG_BASE = 0x00000000FC000000ULL; // common on HP rx-series
