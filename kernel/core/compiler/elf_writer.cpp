                                            ...
                                            ElfLayout* layout)
{
    ...
    if (!add_u64(imageBase, BOOTSTRAP_CODE_OFFSET, &entryPoint)) return false;
    ...
    put_u16(output, 18, target_elf_machine(resolvedTarget));
    ...
    put_u64(output, ph + 16, imageBase);
    put_u64(output, ph + 24, imageBase);
...
bool write_bootstrap_elf(const uint8_t* code,
                         ...
                         ElfLayout* layout)
{
    return write_bootstrap_elf_for_target(code, codeBytes, CompilerTarget::Amd64,
                                          output, outputCapacity, layout);
}

bool validate_bootstrap_elf_for_target(const uint8_t* image,
                                       uint32_t imageBytes,
                                       CompilerTarget target,
                                       ...
                                       ElfValidationResult* result)
{
    ...
    const CompilerTarget resolvedTarget = target == CompilerTarget::Current ? current_target() : target;
    if (get_u16(image, 18) != target_elf_machine(resolvedTarget)) return fail(result, "ELF machine does not match compiler target");
    ...
    if (expectedCodeBytes != 0) {
        ...
        for (uint32_t i = 0; i < expectedCodeBytes; ++i) {
            if (image[expectedCodeOffset + i] != expectedCode[i]) return fail(result, "ELF code bytes do not match generated target code");
        }
    }
...
bool validate_bootstrap_elf(const uint8_t* image,
                            ...
                            ElfValidationResult* result)
{
    return validate_bootstrap_elf_for_target(image, imageBytes, CompilerTarget::Amd64,
                                             expectedImageBase, expectedCodeOffset,
                                             expectedCode, expectedCodeBytes, result);
}
