...
bool compile_for_target(const char* sourcePath,
                        ...
                        CompileSummary* summary)
{
    ...
    bool emitted = false;
    if (resolvedTarget == CompilerTarget::Amd64) emitted = amd64::emit_function(function, s_code, sizeof(s_code), &codeBytes);
    if (resolvedTarget == CompilerTarget::Arm64) emitted = arm64::emit_function(function, s_code, sizeof(s_code), &codeBytes);
    if (!emitted) {
        diagnostics.error(driverLocation, "compiler target backend is unavailable for this kernel build", "backend");
        return fail_build(diagnostics, summary);
    }
    ...
    if (!validate_bootstrap_elf_for_target(s_elf, layout.outputBytes, resolvedTarget,
                                           layout.imageBase, layout.codeOffset,
                                           s_code, codeBytes, &producedValidation)) {
    ...
    if (!validate_bootstrap_elf_for_target(s_reopened, static_cast<uint32_t>(reopenedBytes),
                                           resolvedTarget, layout.imageBase, layout.codeOffset,
                                           s_code, codeBytes, &reopenedValidation)) {
...
bool compile(const char* sourcePath,
             ...
             CompileSummary* summary)
{
    return compile_for_target(sourcePath, outputPath, CompilerTarget::Current, summary);
}
...
void run_bootstrap_smoke()
{
    ...
    const uint32_t expectedReturnCodeBytes = current_target() == CompilerTarget::Amd64 ? 6u : 12u;
    const bool returnValueProof = proof42 && proof41 &&
        ...
        return42.codeBytes == expectedReturnCodeBytes && return41.codeBytes == expectedReturnCodeBytes;
