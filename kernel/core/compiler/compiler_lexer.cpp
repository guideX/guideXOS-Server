...
static TokenKind identifier_kind(const char* source, uint32_t offset, uint32_t length)
{
    ...
    if (text_equals(source, offset, length, "break")) return TokenKind::KeywordBreak;
    if (text_equals(source, offset, length, "continue")) return TokenKind::KeywordContinue;
        ...
        switch (c) {
            ...
            case '|':
                if (index + 1 < sourceLength && source[index + 1] == '|') {
                    ...
                    diagnostics.error(location, "unexpected '|'; logical OR is '||'", "'|'");
                    advance(source, sourceLength, &index, &line, &column);
                    return false;
                }
                break;
