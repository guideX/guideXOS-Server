                           ...
                           uint8_t* output, uint32_t capacity, uint32_t* outputBytes)
{
    ...
    writer.u32(edgeCount); writer.u32(module.exportCount); writer.u32(module.importCount); writer.u32(module.relocationCount); writer.u32(0); writer.u32(objectBytes); writer.u64(0);
    ...
    writer.patch_u32(80, writer.position()); *outputBytes = writer.position(); return writer.ok();
}
                               ...
                               uint8_t* output, uint32_t capacity, uint32_t* outputBytes)
{
    ...
    const uint32_t checksumOffset = sections[meta].offset + 88U;
