...
#endif
#if defined(GXOS_PHASE28G_SMOKE)
auto phase28g_run_request = [&](const gx_build_snapshot& build,
                                bool debugControlled) -> gx_development_run_request
{
    gx_development_run_request request = {};
    request.size = sizeof(request);
    request.version = GX_DEVELOPMENT_RUN_API_VERSION;
    request.projectRoot = "/P28G";
    request.projectId = "dev.guidexos.phase28g";
    request.projectKind = "native-gui-application";
    request.targetProfile = "guidexos.amd64.baremetal.bootstrap.native";
    request.manifestPath = "app/app.json";
    request.artifactPath = build.artifactPath;
...
auto phase28g_debug_request = [&](
    const gx_build_snapshot& build, gx_development_run_handle handle,
    uint64_t generation, uint32_t command, const gx_development_debug_snapshot* stop)
{
    gx_development_debug_request request = {};
    request.size = sizeof(request);
    request.version = GX_DEVELOPMENT_DEBUG_API_VERSION;
...
auto run_phase28g_build = [&](gx_build_snapshot* snapshot) -> bool
{
    gx_build_request request = {};
    request.size = sizeof(request);
    request.version = GX_BUILD_API_VERSION;
    ...
    request.targetProfile = "guidexos.amd64.baremetal.bootstrap.native";
    request.buildSystem = "guidexos-native-baremetal-bootstrap-v1";
    request.expectedArtifact = "build/bin/amd64/p28g.elf";
    request.configuration = "Debug";
    gx_build_handle handle = 0;
    if (compiler::BareMetalBuildService::start(&request, &handle) != GX_OK) return false;
    gx_build_snapshot local = {};
    ...
    if (snapshot) *snapshot = local;
    if (!polled || local.state != GX_BUILD_SUCCEEDED) {
        serial::puts("Phase28G build diagnostic state=");
        print_decimal(local.state);
        serial::puts(" error=");
        print_decimal(local.errorCode);
        serial::puts(" message=");
        serial::puts(local.errorMessage);
        serial::puts(" outputs=");
        print_decimal(local.outputCount);
        serial::putc('\n');
        for (uint32_t index = 0; index < local.outputCount && index < GX_BUILD_MAX_OUTPUT_LINES; ++index) {
            serial::puts("Phase28G build output: ");
            serial::puts(local.output[index].text);
            serial::putc('\n');
        }
    }
    return polled && released;
...
auto phase28g_value = [&](const gx_development_debug_variables& result,
                          const char* name, uint32_t kind, int64_t value) -> bool
{
    for (uint32_t i = 0; i < result.variableCount; ++i) {
        const gx_development_debug_variable& variable = result.variables[i];
        if (!equal_text(variable.name, name)) continue;
        return variable.kind == kind && variable.type == GX_DEVELOPMENT_DEBUG_VARIABLE_TYPE_SIGNED_INT32 &&
            variable.location == GX_DEVELOPMENT_DEBUG_VARIABLE_LOCATION_RBP_RELATIVE &&
            variable.sizeBytes == 4 && variable.availability == GX_DEVELOPMENT_DEBUG_VARIABLE_AVAILABILITY_AVAILABLE &&
            (variable.flags & GX_DEVELOPMENT_DEBUG_VARIABLE_VALUE_VALID) != 0 &&
            variable.signedValue == value;
    }
    return false;
};

auto phase28g_primary = [&](const gx_development_debug_variables& result, bool changed) -> bool
{
    return result.status == GX_DEVELOPMENT_DEBUG_VARIABLES_STATUS_SUCCESS && result.truncated == 0 &&
        result.variableCount >= 4 && equal_text(result.functionName, "helper") &&
        phase28g_value(result, "input", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_ARGUMENT, 10) &&
        phase28g_value(result, "delta", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_ARGUMENT, 4) &&
        phase28g_value(result, "adjusted", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_LOCAL, changed ? 15 : 14) &&
        phase28g_value(result, "doubled", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_LOCAL, 28);
};

auto phase28g_same = [&](const gx_development_debug_variables& left,
                         const gx_development_debug_variables& right) -> bool
{
    if (left.status != right.status || left.variableCount != right.variableCount ||
        left.truncated != right.truncated || left.instructionPointer != right.instructionPointer ||
        left.framePointer != right.framePointer || !equal_text(left.functionName, right.functionName)) return false;
    for (uint32_t i = 0; i < left.variableCount; ++i) {
        const gx_development_debug_variable& a = left.variables[i];
        const gx_development_debug_variable& b = right.variables[i];
        if (!equal_text(a.name, b.name) || a.kind != b.kind || a.type != b.type ||
            a.location != b.location || a.flags != b.flags || a.sizeBytes != b.sizeBytes ||
            a.frameOffset != b.frameOffset || a.availability != b.availability ||
            a.rawValue != b.rawValue || a.signedValue != b.signedValue ||
            a.unsignedValue != b.unsignedValue) return false;
    }
    return true;
};

auto run_phase28g_variable_session = [&](const gx_build_snapshot& build) -> bool
{
    gx_development_run_request runRequest = phase28g_run_request(build, true);
    gx_development_run_handle handle = 0;
    gx_development_run_snapshot prepared = {};
    prepared.size = sizeof(prepared);
    if (NativeElfRunService::prepare(runRequest, &handle, &prepared) != GX_OK || handle == 0) return false;
    ...
    const bool paused = started && NativeElfRunService::poll(handle, &pausedRun) == GX_OK &&
        pausedRun.state == GX_DEVELOPMENT_RUN_PAUSED && NativeElfRunService::debug(
            phase28g_debug_request(build, handle, generation, GX_DEVELOPMENT_DEBUG_POLL, nullptr),
            &initial) == GX_OK && initial.sourceMappingValid != 0 &&
        equal_text(initial.functionName, "helper") && equal_text(initial.sourcePath, "src/helper.cpp");
    if (paused) serial::puts("DEVELOPER_STUDIO_PHASE28G_PAUSED_PASS\n");

    gx_development_debug_call_stack stack = {};
    stack.size = sizeof(stack);
    const bool callStack = paused && NativeElfRunService::call_stack(
        phase28g_debug_request(build, handle, generation, GX_DEVELOPMENT_DEBUG_CALL_STACK, &initial),
        &stack) == GX_OK && stack.status == GX_DEVELOPMENT_DEBUG_CALL_STACK_STATUS_SUCCESS &&
        stack.frameCount >= 2 && equal_text(stack.frames[0].functionName, "helper") &&
        equal_text(stack.frames[1].functionName, "gx_main");
    if (callStack) serial::puts("DEVELOPER_STUDIO_PHASE28G_CALL_STACK_REGRESSION_PASS\n");

    gx_development_debug_variables first = {};
    first.size = sizeof(first);
    const bool firstQuery = callStack && NativeElfRunService::inspect_variables(
        phase28g_debug_request(build, handle, generation, GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &initial),
        &first) == GX_OK && phase28g_primary(first, false) &&
        equal_text(first.functionName, stack.frames[0].functionName);
    if (firstQuery) {
        serial::puts("DEVELOPER_STUDIO_PHASE28G_VARIABLE_METADATA_PASS\n");
        serial::puts("DEVELOPER_STUDIO_PHASE28G_ARGUMENTS_PASS\n");
        serial::puts("DEVELOPER_STUDIO_PHASE28G_LOCALS_PASS\n");
        serial::puts("DEVELOPER_STUDIO_PHASE28G_FRAME_MATCH_PASS\n");
    }
    gx_development_debug_variables second = {};
    second.size = sizeof(second);
    const bool repeated = firstQuery && NativeElfRunService::inspect_variables(
        phase28g_debug_request(build, handle, generation, GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &initial),
        &second) == GX_OK && phase28g_same(first, second);
    gx_development_debug_snapshot afterQuery = {};
    const bool readOnly = repeated && NativeElfRunService::debug(
        phase28g_debug_request(build, handle, generation, GX_DEVELOPMENT_DEBUG_POLL, &initial),
        &afterQuery) == GX_OK && afterQuery.context.rip == initial.context.rip &&
        afterQuery.context.rsp == initial.context.rsp && afterQuery.context.rbp == initial.context.rbp &&
        afterQuery.context.rflags == initial.context.rflags;
    ...
    const bool stepped = readOnly && NativeElfRunService::debug(
        ...
        &changedStop) == GX_OK && changedStop.status == GX_DEVELOPMENT_DEBUG_STATUS_TRAP &&
        equal_text(changedStop.functionName, "helper");
    gx_development_debug_variables changed = {};
    changed.size = sizeof(changed);
    const bool valueChanged = stepped && NativeElfRunService::inspect_variables(
    ...
    const bool stepOut = valueChanged && NativeElfRunService::debug(
        phase28g_debug_request(build, handle, generation, GX_DEVELOPMENT_DEBUG_STEP_SOURCE_OUT, &changedStop),
        &steppedOut) == GX_OK && steppedOut.status == GX_DEVELOPMENT_DEBUG_STATUS_TRAP &&
        equal_text(steppedOut.functionName, "gx_main");
    gx_development_debug_variables caller = {};
    caller.size = sizeof(caller);
    const bool callerVariables = stepOut && NativeElfRunService::inspect_variables(
        phase28g_debug_request(build, handle, generation, GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &steppedOut),
        &caller) == GX_OK && caller.status == GX_DEVELOPMENT_DEBUG_VARIABLES_STATUS_SUCCESS &&
        equal_text(caller.functionName, "gx_main");
    if (callerVariables) serial::puts("DEVELOPER_STUDIO_PHASE28G_STEP_OUT_FRAME_PASS\n");

    gx_development_debug_variables stale = {};
    stale.size = sizeof(stale);
    const bool staleRejected = callerVariables && NativeElfRunService::inspect_variables(
        phase28g_debug_request(build, handle, generation + 1ULL, GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &steppedOut),
        &stale) == GX_ERROR_FAILED && stale.status == GX_DEVELOPMENT_DEBUG_VARIABLES_STATUS_STALE;
    if (staleRejected) serial::puts("DEVELOPER_STUDIO_PHASE28G_STALE_PASS\n");
    gx_development_debug_snapshot resumed = {};
    const bool resumedRequest = callerVariables && NativeElfRunService::debug(
        phase28g_debug_request(build, handle, generation, GX_DEVELOPMENT_DEBUG_RESUME, &steppedOut),
        &resumed) == GX_OK;
    gx_development_debug_variables runningQuery = {};
    runningQuery.size = sizeof(runningQuery);
    const bool runningRejected = resumedRequest && NativeElfRunService::inspect_variables(
        phase28g_debug_request(build, handle, generation, GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &steppedOut),
        &runningQuery) != GX_OK && runningQuery.status == GX_DEVELOPMENT_DEBUG_VARIABLES_STATUS_NO_PAUSED_CONTEXT;
    NativeElfGuiRuntimeSnapshot gui = {};
    const bool rendered = resumedRequest && NativeElfRunService::poll(handle, &pausedRun) == GX_OK &&
        pausedRun.state == GX_DEVELOPMENT_RUN_RUNNING && native_elf_gui_runtime_snapshot(&gui) &&
        gui.active && gui.rendered && equal_text(gui.content, "28G GUI 33");
    if (runningRejected) serial::puts("DEVELOPER_STUDIO_PHASE28G_RESUME_PASS\n");
    if (rendered) serial::puts("DEVELOPER_STUDIO_PHASE28G_RENDER_PASS\n");
    const bool closeRequested = rendered && NativeElfRunService::request_close(handle) == GX_OK;
    gx_development_run_snapshot completed = {};
    completed.size = sizeof(completed);
    const bool complete = closeRequested && NativeElfRunService::poll(handle, &completed) == GX_OK &&
        completed.state == GX_DEVELOPMENT_RUN_COMPLETED && completed.cleanupComplete != 0 &&
        completed.outputCount == 1 && equal_text(completed.output[0].text,
                                                  "DEVELOPER_STUDIO_PHASE28G_PRE_BREAKPOINT_ONCE");
    if (complete) serial::puts("DEVELOPER_STUDIO_PHASE28G_CLOSE_PASS\n");
    const bool released = NativeElfRunService::release(handle) == GX_OK;
    set_gui_automation_close(false);
    const bool clean = released && !NativeElfDevelopmentAppModel::has_active_registration() &&
        compositor::KernelCompositor::getWindowCount() == 0;
    if (clean) serial::puts("DEVELOPER_STUDIO_PHASE28G_CLEANUP_PASS\n");
    return paused && callStack && firstQuery && repeated && readOnly && stepped && valueChanged &&
        stepOut && callerVariables && staleRejected && resumedRequest && runningRejected && rendered && complete && clean;
};

auto run_phase28g_normal_session = [&](const gx_build_snapshot& build) -> bool
{
    gx_development_run_request request = phase28g_run_request(build, false);
    gx_development_run_handle handle = 0;
    gx_development_run_snapshot prepared = {};
    prepared.size = sizeof(prepared);
    if (NativeElfRunService::prepare(request, &handle, &prepared) != GX_OK || handle == 0) return false;
    ...
    const bool clean = released && !NativeElfDevelopmentAppModel::has_active_registration() &&
        compositor::KernelCompositor::getWindowCount() == 0;
    if (rendered && complete && clean) serial::puts("DEVELOPER_STUDIO_PHASE28G_RUN_REGRESSION_PASS\n");
    return rendered && complete && clean;
};
#endif

#if defined(GXOS_PHASE28G_SMOKE)
    serial::puts("ELF Loader: Phase 28G arguments and locals inspection smoke begin\n");
    serial::puts("DEVELOPER_STUDIO_PHASE28G_BEGIN\n");
    static gx_build_snapshot g28Build = {};
    const bool g28BuildPass = run_phase28g_build(&g28Build) &&
        g28Build.state == GX_BUILD_SUCCEEDED && g28Build.artifactValid != 0 &&
        g28Build.artifactSize != 0 && equal_text(g28Build.artifactArchitecture, "amd64");
    print_marker("phase28g_build_pass", g28BuildPass);
    if (g28BuildPass) serial::puts("DEVELOPER_STUDIO_PHASE28G_BUILD_PASS\n");
    static gx_build_snapshot g28WarmBuild = {};
    const bool g28WarmCachePass = g28BuildPass && run_phase28g_build(&g28WarmBuild) &&
        g28WarmBuild.state == GX_BUILD_SUCCEEDED && g28WarmBuild.artifactValid != 0 &&
        g28WarmBuild.cachedModuleCount != 0;
    print_marker("phase28g_warm_cache_pass", g28WarmCachePass);
    if (g28WarmCachePass) serial::puts("DEVELOPER_STUDIO_PHASE28G_WARM_CACHE_PASS\n");
    const bool g28Variables = g28WarmCachePass && run_phase28g_variable_session(g28WarmBuild);
    print_marker("phase28g_variables_pass", g28Variables);
    const bool g28NormalRun = g28WarmCachePass && run_phase28g_normal_session(g28WarmBuild);
    print_marker("phase28g_run_pass", g28NormalRun);
    const bool g28Artifact = g28BuildPass &&
        emit_serial_artifact("/P28G/build/bin/amd64/p28g.elf", "g28main");
    print_marker("phase28g_artifact", g28Artifact);
    const bool phase28gPassed = g28BuildPass && g28WarmCachePass && g28Variables && g28NormalRun &&
        g28Artifact && !NativeElfDevelopmentAppModel::has_active_registration() &&
        compositor::KernelCompositor::getWindowCount() == 0;
    print_marker("phase28g_debug_pass", phase28gPassed && g28Variables);
    print_marker("phase28g", phase28gPassed);
    serial::puts(phase28gPassed ?
        "ELF Loader: Phase 28G arguments and locals inspection smoke PASS\nDEVELOPER_STUDIO_PHASE28G_PASS\n" :
        "ELF Loader: Phase 28G arguments and locals inspection smoke FAIL\n");
#endif

#if defined(GXOS_PHASE28H_SMOKE) || defined(GXOS_PHASE28I_SMOKE)
#if defined(GXOS_PHASE28I_SMOKE)
    serial::puts("ELF Loader: Phase 28I selected-frame watches smoke begin\n");
    serial::puts("DEVELOPER_STUDIO_PHASE28I_BEGIN\n");
#endif
    serial::puts("ELF Loader: Phase 28H selected-frame variables smoke begin\n");
    serial::puts("DEVELOPER_STUDIO_PHASE28H_BEGIN\n");
    auto phase28h_run_request = [&](const gx_build_snapshot& build,
                                    bool debugControlled) -> gx_development_run_request {
        gx_development_run_request request = {};
        request.size = sizeof(request);
        request.version = GX_DEVELOPMENT_RUN_API_VERSION;
        request.projectRoot = "/P28H";
        request.projectId = "dev.guidexos.phase28h";
        request.projectKind = "native-gui-application";
        request.targetProfile = "guidexos.amd64.baremetal.bootstrap.native";
        request.manifestPath = "app/app.json";
        request.artifactPath = build.artifactPath;
    ...
    auto phase28h_debug_request = [&](const gx_build_snapshot& build,
                                      gx_development_run_handle handle,
                                      uint64_t generation, uint32_t command,
                                      const gx_development_debug_snapshot* stop,
        ...
        request.threadId = 1;
        request.stopGeneration = stop ? stop->context.stopGeneration : 0;
        request.auxiliaryAddress = frameIndex;
        return request;
    };

    auto run_phase28h_build = [&](gx_build_snapshot* snapshot) -> bool {
        gx_build_request request = {};
        request.size = sizeof(request);
        request.version = GX_BUILD_API_VERSION;
        request.projectRoot = "/P28H";
        ...
        request.targetProfile = "guidexos.amd64.baremetal.bootstrap.native";
        request.buildSystem = "guidexos-native-baremetal-bootstrap-v1";
        request.expectedArtifact = "build/bin/amd64/p28h.elf";
        request.configuration = "Debug";
        gx_build_handle handle = 0;
        if (compiler::BareMetalBuildService::start(&request, &handle) != GX_OK) return false;
        gx_build_snapshot local = {};
        ...
        if (snapshot) *snapshot = local;
        if (!polled || local.state != GX_BUILD_SUCCEEDED) {
            serial::puts("Phase28H build diagnostic state=");
            print_decimal(local.state);
            serial::puts(" error=");
            print_decimal(local.errorCode);
            serial::puts(" message=");
    ...
    auto phase28h_value = [&](const gx_development_debug_variables& result,
                              const char* name, uint32_t kind, int64_t value) -> bool {
        for (uint32_t i = 0; i < result.variableCount; ++i) {
            const gx_development_debug_variable& variable = result.variables[i];
            if (!equal_text(variable.name, name)) continue;
            return variable.kind == kind &&
                variable.type == GX_DEVELOPMENT_DEBUG_VARIABLE_TYPE_SIGNED_INT32 &&
                variable.location == GX_DEVELOPMENT_DEBUG_VARIABLE_LOCATION_RBP_RELATIVE &&
                variable.sizeBytes == 4 &&
    ...
    auto phase28h_has_value = [&](const gx_development_debug_variables& result,
                                  const char* name) -> bool {
        for (uint32_t i = 0; i < result.variableCount; ++i)
            if (equal_text(result.variables[i].name, name)) return true;
        return false;
    };

    auto phase28h_same_registers = [](const gx_development_debug_register_context& left,
                                      const gx_development_debug_register_context& right) {
        return left.architecture == right.architecture && left.valid == right.valid &&
            left.processId == right.processId && left.nativeRuntimeId == right.nativeRuntimeId &&
            left.threadId == right.threadId && left.sessionGeneration == right.sessionGeneration &&
            left.stopGeneration == right.stopGeneration && left.rip == right.rip &&
            left.rflags == right.rflags && left.rsp == right.rsp && left.rbp == right.rbp &&
            left.rax == right.rax && left.rbx == right.rbx && left.rcx == right.rcx &&
            left.rdx == right.rdx && left.rsi == right.rsi && left.rdi == right.rdi &&
            left.r8 == right.r8 && left.r9 == right.r9 && left.r10 == right.r10 &&
            left.r11 == right.r11 && left.r12 == right.r12 && left.r13 == right.r13 &&
            left.r14 == right.r14 && left.r15 == right.r15;
    };

    auto phase28h_same_stack = [&](const gx_development_debug_call_stack& left,
                                   const gx_development_debug_call_stack& right) {
        if (left.status != right.status || left.frameCount != right.frameCount ||
            left.truncated != right.truncated || left.stackLow != right.stackLow ||
            left.stackHigh != right.stackHigh) return false;
        for (uint32_t i = 0; i < left.frameCount; ++i) {
            const gx_development_debug_call_stack_frame& a = left.frames[i];
            const gx_development_debug_call_stack_frame& b = right.frames[i];
            if (a.depth != b.depth || a.flags != b.flags ||
                a.instructionPointer != b.instructionPointer || a.stackPointer != b.stackPointer ||
                a.framePointer != b.framePointer || a.returnAddress != b.returnAddress ||
                a.sourceLine != b.sourceLine || a.sourceColumn != b.sourceColumn ||
                !equal_text(a.functionName, b.functionName) ||
                !equal_text(a.sourcePath, b.sourcePath)) return false;
        }
        return true;
    };

    auto phase28h_same_variables = [&](const gx_development_debug_variables& left,
                                       const gx_development_debug_variables& right) {
        if (left.status != right.status || left.variableCount != right.variableCount ||
            left.truncated != right.truncated || left.instructionPointer != right.instructionPointer ||
            left.framePointer != right.framePointer || !equal_text(left.functionName, right.functionName) ||
            !equal_text(left.sourcePath, right.sourcePath)) return false;
        for (uint32_t i = 0; i < left.variableCount; ++i) {
            const gx_development_debug_variable& a = left.variables[i];
            const gx_development_debug_variable& b = right.variables[i];
            if (!equal_text(a.name, b.name) || a.kind != b.kind || a.type != b.type ||
    ...
    static gx_build_snapshot h28Build = {};
    const bool h28BuildPass = run_phase28h_build(&h28Build) &&
        h28Build.state == GX_BUILD_SUCCEEDED && h28Build.artifactValid != 0 &&
    ...
    const bool h28WarmCachePass = h28BuildPass && run_phase28h_build(&h28WarmBuild) &&
        ...
        h28WarmBuild.cachedModuleCount != 0;
    print_marker("phase28h_warm_cache_pass", h28WarmCachePass);
    if (h28WarmCachePass) serial::puts("DEVELOPER_STUDIO_PHASE28H_WARM_CACHE_PASS\n");

    bool h28NestedPause = false;
    bool h28CallStack = false;
    bool h28Frame0 = false;
    bool h28Frame1 = false;
    bool h28Frame2 = false;
    bool h28CallerLiveRange = false;
    bool h28RoundTrip = false;
    bool h28ReadOnly = false;
    bool h28StackUnchanged = false;
    bool h28Invalid = false;
    bool h28StepOutReset = false;
    bool h28Stale = false;
    bool h28Resume = false;
    bool h28Render = false;
    bool h28Complete = false;
    bool h28Clean = false;
    ...
    bool h28WatchReadOnly = false;
    bool h28WatchReevaluate = false;
    if (h28WarmCachePass) {
        gx_development_run_request runRequest = phase28h_run_request(h28WarmBuild, true);
        gx_development_run_handle handle = 0;
        gx_development_run_snapshot prepared = {};
        prepared.size = sizeof(prepared);
        const bool preparedOk = NativeElfRunService::prepare(runRequest, &handle, &prepared) == GX_OK && handle != 0;
        const uint64_t generation = prepared.generation;
        set_gui_automation_close(false);
        ...
        pausedRun.size = sizeof(pausedRun);
        gx_development_debug_snapshot initial = {};
        h28NestedPause = started && NativeElfRunService::poll(handle, &pausedRun) == GX_OK &&
            pausedRun.state == GX_DEVELOPMENT_RUN_PAUSED && NativeElfRunService::debug(
                phase28h_debug_request(h28WarmBuild, handle, generation,
                                       GX_DEVELOPMENT_DEBUG_POLL, nullptr), &initial) == GX_OK &&
        ...
        h28CallStack = h28NestedPause && NativeElfRunService::call_stack(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_CALL_STACK, &initial),
            &initialStack) == GX_OK && initialStack.status == GX_DEVELOPMENT_DEBUG_CALL_STACK_STATUS_SUCCESS &&
            initialStack.frameCount == 3 && equal_text(initialStack.frames[0].functionName, "helper_tail") &&
            equal_text(initialStack.frames[1].functionName, "helper") &&
            equal_text(initialStack.frames[2].functionName, "gx_main") &&
            equal_text(initialStack.frames[0].sourcePath, "src/helper.cpp") &&
            equal_text(initialStack.frames[1].sourcePath, "src/helper.cpp") &&
            equal_text(initialStack.frames[2].sourcePath, "src/main.cpp") &&
            (initialStack.frames[1].flags & GX_DEVELOPMENT_DEBUG_CALL_STACK_FRAME_CALL_SITE_MAPPED) != 0 &&
            (initialStack.frames[2].flags & GX_DEVELOPMENT_DEBUG_CALL_STACK_FRAME_CALL_SITE_MAPPED) != 0;
        if (h28CallStack) serial::puts("DEVELOPER_STUDIO_PHASE28H_CALL_STACK_PASS\nDEVELOPER_STUDIO_PHASE28H_CALL_STACK_REGRESSION_PASS\n");
        if (h28CallStack) {
            serial::puts("DEVELOPER_STUDIO_PHASE28H_FRAME_EVIDENCE frame0_rbp=");
            serial::put_hex64(initialStack.frames[0].framePointer);
            serial::puts(" frame0_rip=");
            serial::put_hex64(initialStack.frames[0].instructionPointer);
            serial::puts(" frame1_rbp=");
            serial::put_hex64(initialStack.frames[1].framePointer);
            serial::puts(" frame1_rip=");
            serial::put_hex64(initialStack.frames[1].instructionPointer);
            serial::puts(" frame2_rbp=");
            serial::put_hex64(initialStack.frames[2].framePointer);
            serial::puts(" frame2_rip=");
            serial::put_hex64(initialStack.frames[2].instructionPointer);
            serial::putc('\n');
        }

        gx_development_debug_variables frame0 = {};
        frame0.size = sizeof(frame0);
        gx_development_debug_request frame0Request = phase28h_debug_request(
            h28WarmBuild, handle, generation, GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &initial, 0);
        const bool frame0Query = h28CallStack && NativeElfRunService::inspect_variables(frame0Request, &frame0) == GX_OK;
        h28Frame0 = frame0Query && frame0.status == GX_DEVELOPMENT_DEBUG_VARIABLES_STATUS_SUCCESS &&
            equal_text(frame0.functionName, "helper_tail") &&
            frame0.framePointer == initialStack.frames[0].framePointer &&
            phase28h_value(frame0, "tail_input", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_ARGUMENT, 28) &&
            phase28h_value(frame0, "tail_bonus", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_ARGUMENT, 5) &&
            phase28h_value(frame0, "tail_value", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_LOCAL, 33);
        if (frame0Query) serial::puts("DEVELOPER_STUDIO_PHASE28H_SELECT_FRAME0_PASS\n");
        if (frame0Query && !h28Frame0) {
            serial::puts("Phase28H frame0 values:");
            for (uint32_t i = 0; i < frame0.variableCount; ++i) {
                serial::putc(' ');
                serial::puts(frame0.variables[i].name);
                serial::putc('=');
                serial::put_hex64(static_cast<uint64_t>(frame0.variables[i].signedValue));
            }
            serial::putc('\n');
        }
        if (h28Frame0) serial::puts("DEVELOPER_STUDIO_PHASE28H_FRAME0_VARIABLES_PASS\nDEVELOPER_STUDIO_PHASE28H_LOCALS_REGRESSION_PASS\n");

        gx_development_debug_variables frame1 = {};
        frame1.size = sizeof(frame1);
        const bool frame1Query = h28CallStack && NativeElfRunService::inspect_variables(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &initial, 1), &frame1) == GX_OK;
        h28Frame1 = frame1Query && frame1.status == GX_DEVELOPMENT_DEBUG_VARIABLES_STATUS_SUCCESS &&
            equal_text(frame1.functionName, "helper") &&
            frame1.framePointer == initialStack.frames[1].framePointer &&
            frame1.instructionPointer == initialStack.frames[1].instructionPointer &&
            phase28h_value(frame1, "input", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_ARGUMENT, 10) &&
            phase28h_value(frame1, "delta", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_ARGUMENT, 4) &&
            phase28h_value(frame1, "adjusted", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_LOCAL, 14) &&
            phase28h_value(frame1, "doubled", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_LOCAL, 28);
        if (frame1Query) serial::puts("DEVELOPER_STUDIO_PHASE28H_SELECT_FRAME1_PASS\n");
        if (h28Frame1) serial::puts("DEVELOPER_STUDIO_PHASE28H_FRAME1_VARIABLES_PASS\n");

        gx_development_debug_variables frame2 = {};
        frame2.size = sizeof(frame2);
        const bool frame2Query = h28CallStack && NativeElfRunService::inspect_variables(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &initial, 2), &frame2) == GX_OK;
        h28Frame2 = frame2Query && frame2.status == GX_DEVELOPMENT_DEBUG_VARIABLES_STATUS_SUCCESS &&
            equal_text(frame2.functionName, "gx_main") &&
            frame2.framePointer == initialStack.frames[2].framePointer &&
            phase28h_value(frame2, "root_value", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_LOCAL, 100) &&
            phase28h_value(frame2, "before_call", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_LOCAL, 27);
        h28CallerLiveRange = h28Frame2 && !phase28h_has_value(frame2, "after_call");
        if (frame2Query) serial::puts("DEVELOPER_STUDIO_PHASE28H_SELECT_FRAME2_PASS\n");
        if (frame2Query && !h28CallerLiveRange) {
            serial::puts("Phase28H root variables:");
            for (uint32_t i = 0; i < frame2.variableCount; ++i) {
                serial::putc(' ');
                serial::puts(frame2.variables[i].name);
                serial::putc('=');
                serial::put_hex64(static_cast<uint64_t>(frame2.variables[i].signedValue));
            }
            serial::putc('\n');
        }
        if (h28Frame2) serial::puts("DEVELOPER_STUDIO_PHASE28H_FRAME2_VARIABLES_PASS\n");
        ...
        frame0Again.size = sizeof(frame0Again);
        const bool finalFrame0Query = NativeElfRunService::inspect_variables(
            frame0Request, &frame0Again) == GX_OK;
        gx_development_debug_call_stack stackAfterSelections = {};
        stackAfterSelections.size = sizeof(stackAfterSelections);
        const bool stackAfterQuery = NativeElfRunService::call_stack(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_CALL_STACK, &initial),
            &stackAfterSelections) == GX_OK;
        h28RoundTrip = finalFrame0Query && frame0Query && phase28h_same_variables(frame0, frame0Again);
        h28StackUnchanged = stackAfterQuery && phase28h_same_stack(initialStack, stackAfterSelections);
        if (h28RoundTrip) serial::puts("DEVELOPER_STUDIO_PHASE28H_ROUND_TRIP_PASS\n");
        if (h28StackUnchanged) serial::puts("DEVELOPER_STUDIO_PHASE28H_STACK_UNCHANGED_PASS\n");

        auto pausedRegistersStable = [&]() {
            gx_development_debug_snapshot current = {};
            const bool polled = NativeElfRunService::debug(
                phase28h_debug_request(h28WarmBuild, handle, generation,
                                       GX_DEVELOPMENT_DEBUG_POLL, &initial), &current) == GX_OK;
            return polled && phase28h_same_registers(initial.context, current.context) &&
                current.context.rip == initial.context.rip && current.context.rsp == initial.context.rsp &&
                current.context.rbp == initial.context.rbp && current.context.rflags == initial.context.rflags;
        };
        h28ReadOnly = frame0Query && frame1Query && frame2Query && finalFrame0Query &&
            pausedRegistersStable() && pausedRegistersStable() && pausedRegistersStable();
        if (h28ReadOnly) serial::puts("DEVELOPER_STUDIO_PHASE28H_READ_ONLY_GPRS_PASS\n");
#if defined(GXOS_PHASE28I_SMOKE)
        h28WatchReadOnly = h28ReadOnly && h28StackUnchanged;
        if (h28WatchReadOnly) serial::puts("DEVELOPER_STUDIO_PHASE28I_READ_ONLY_PASS\n");
#endif

        gx_development_debug_variables invalid = {};
        invalid.size = sizeof(invalid);
        const gx_result invalidResult = NativeElfRunService::inspect_variables(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &initial, 3), &invalid);
        gx_development_debug_variables invalidCapacity = {};
        invalidCapacity.size = sizeof(invalidCapacity);
        const gx_result invalidCapacityResult = NativeElfRunService::inspect_variables(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &initial, 16), &invalidCapacity);
        h28Invalid = invalidResult == GX_ERROR_INVALID_ARGUMENT &&
            invalid.status == GX_DEVELOPMENT_DEBUG_VARIABLES_STATUS_INVALID_FRAME &&
            invalidCapacityResult == GX_ERROR_INVALID_ARGUMENT &&
            invalidCapacity.status == GX_DEVELOPMENT_DEBUG_VARIABLES_STATUS_INVALID_FRAME &&
            pausedRegistersStable();
        if (h28Invalid) serial::puts("DEVELOPER_STUDIO_PHASE28H_INVALID_FRAME_PASS\n");

        gx_development_debug_snapshot afterStepOut = {};
        const bool stepOut = NativeElfRunService::debug(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_STEP_SOURCE_OUT, &initial),
            &afterStepOut) == GX_OK && afterStepOut.status == GX_DEVELOPMENT_DEBUG_STATUS_TRAP &&
            equal_text(afterStepOut.functionName, "helper");
        gx_development_debug_call_stack steppedStack = {};
        steppedStack.size = sizeof(steppedStack);
        const bool stepOutStack = stepOut && NativeElfRunService::call_stack(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_CALL_STACK, &afterStepOut),
            &steppedStack) == GX_OK && steppedStack.status == GX_DEVELOPMENT_DEBUG_CALL_STACK_STATUS_SUCCESS &&
            steppedStack.frameCount == 2 && equal_text(steppedStack.frames[0].functionName, "helper") &&
            equal_text(steppedStack.frames[1].functionName, "gx_main");
        gx_development_debug_variables afterStepOutVariables = {};
        afterStepOutVariables.size = sizeof(afterStepOutVariables);
        const bool stepOutVariables = stepOutStack && NativeElfRunService::inspect_variables(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &afterStepOut),
            &afterStepOutVariables) == GX_OK && equal_text(afterStepOutVariables.functionName, "helper") &&
            phase28h_value(afterStepOutVariables, "input", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_ARGUMENT, 10);
        gx_development_debug_variables staleSelection = {};
        staleSelection.size = sizeof(staleSelection);
        gx_result staleResult = GX_ERROR_FAILED;
        if (stepOut) staleResult = NativeElfRunService::inspect_variables(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &initial, 1),
            &staleSelection);
        h28StepOutReset = stepOutStack && stepOutVariables && steppedStack.frames[0].framePointer !=
            initialStack.frames[0].framePointer;
        h28Stale = staleResult == GX_ERROR_FAILED &&
            staleSelection.status == GX_DEVELOPMENT_DEBUG_VARIABLES_STATUS_STALE;
        if (!h28Stale) {
            serial::puts("Phase28H stale result=");
            print_decimal(static_cast<uint32_t>(staleResult));
            serial::puts(" status=");
            print_decimal(staleSelection.status);
            serial::puts(" message=");
            serial::puts(staleSelection.errorMessage);
            serial::putc('\n');
        }
        if (h28StepOutReset) serial::puts("DEVELOPER_STUDIO_PHASE28H_STEP_OUT_RESET_PASS\nDEVELOPER_STUDIO_PHASE28H_STEP_REGRESSION_PASS\n");
        ...
        gx_development_debug_variables rootAfterStepOut = {};
        rootAfterStepOut.size = sizeof(rootAfterStepOut);
        const bool rootAfterStepOutQuery = stepOutStack && NativeElfRunService::inspect_variables(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &afterStepOut, 1),
            &rootAfterStepOut) == GX_OK && equal_text(rootAfterStepOut.functionName, "gx_main") &&
            phase28h_value(rootAfterStepOut, "before_call", GX_DEVELOPMENT_DEBUG_VARIABLE_KIND_LOCAL, 27);
        gx_development_debug_snapshot resumed = {};
        const bool resumedRequest = stepOutVariables && NativeElfRunService::debug(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_RESUME, &afterStepOut), &resumed) == GX_OK;
        gx_development_debug_variables runningVariables = {};
        runningVariables.size = sizeof(runningVariables);
        const bool runningRejected = resumedRequest && NativeElfRunService::inspect_variables(
            phase28h_debug_request(h28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &afterStepOut, 1),
            &runningVariables) != GX_OK &&
        ...
        gx_development_run_snapshot completed = {};
        completed.size = sizeof(completed);
        h28Complete = closeRequested && NativeElfRunService::poll(handle, &completed) == GX_OK &&
            completed.state == GX_DEVELOPMENT_RUN_COMPLETED && completed.cleanupComplete != 0 &&
            completed.outputCount == 1 && equal_text(completed.output[0].text,
    ...
    print_marker("phase28i_read_only", h28WatchReadOnly);
    print_marker("phase28i_reevaluate", h28WatchReevaluate);
    print_marker("phase28i", phase28iPassed);
    serial::puts(phase28iPassed ? "DEVELOPER_STUDIO_PHASE28I_PASS\n" : "DEVELOPER_STUDIO_PHASE28I_FAIL\n");
#endif
    print_marker("phase28h_nested_pause", h28NestedPause);
    print_marker("phase28h_call_stack", h28CallStack);
    print_marker("phase28h_frame0", h28Frame0);
    print_marker("phase28h_frame1", h28Frame1);
    print_marker("phase28h_frame2", h28Frame2);
    print_marker("phase28h_caller_live_range", h28CallerLiveRange);
    print_marker("phase28h_round_trip", h28RoundTrip);
    print_marker("phase28h_read_only_gprs", h28ReadOnly);
    print_marker("phase28h_stack_unchanged", h28StackUnchanged);
    print_marker("phase28h_invalid_frame", h28Invalid);
    print_marker("phase28h_step_out_reset", h28StepOutReset);
    print_marker("phase28h_stale_selection", h28Stale);
    print_marker("phase28h_resume", h28Resume);
    print_marker("phase28h_render", h28Render);
    print_marker("phase28h_close", h28Complete);
    print_marker("phase28h_cleanup", h28Clean);
    print_marker("phase28h_run_regression", h28Render && h28Complete && h28Clean);
    if (h28Render && h28Complete && h28Clean)
        serial::puts("DEVELOPER_STUDIO_PHASE28H_RUN_REGRESSION_PASS\n");
    print_marker("phase28h_debug_pass", h28Primary);
    print_marker("phase28h", h28Primary);
    const bool h28Artifact = h28BuildPass && emit_serial_artifact("/P28H/build/bin/amd64/p28h.elf", "h28main");
    if (h28Artifact) serial::puts("DEVELOPER_STUDIO_PHASE28H_CALL_STACK_REGRESSION_PASS\n");
    const bool phase28hPassed = h28BuildPass && h28WarmCachePass && h28Primary && h28Artifact &&
        !NativeElfDevelopmentAppModel::has_active_registration() &&
        compositor::KernelCompositor::getWindowCount() == 0;
    print_marker("phase28h", phase28hPassed);
    serial::puts(phase28hPassed ?
        "ELF Loader: Phase 28H selected-frame variables smoke PASS\nDEVELOPER_STUDIO_PHASE28H_PASS\n" :
        "ELF Loader: Phase 28H selected-frame variables smoke FAIL\n");
#endif

#if defined(GXOS_PHASE28J_SMOKE)
    serial::puts("ELF Loader: Phase 28J conditional source breakpoint smoke begin\n");
    serial::puts("DEVELOPER_STUDIO_PHASE28J_BEGIN\n");

    auto phase28j_build = [](gx_build_snapshot* snapshot) -> bool {
        gx_build_request request = {};
        request.size = sizeof(request);
        request.version = GX_BUILD_API_VERSION;
        request.projectRoot = "/P28J";
        request.projectId = "dev.guidexos.phase28j";
        request.projectKind = "native-gui-application";
        request.targetProfile = "guidexos.amd64.baremetal.bootstrap.native";
        request.buildSystem = "guidexos-native-baremetal-bootstrap-v1";
        request.expectedArtifact = "build/bin/amd64/p28j.elf";
        request.configuration = "Debug";
        ...
        if (!polled || local.state != GX_BUILD_SUCCEEDED) {
            serial::puts("Phase28J build diagnostic state=");
            print_decimal(local.state);
            serial::puts(" error=");
            print_decimal(local.errorCode);
            serial::puts(" message=");
            serial::puts(local.errorMessage);
            serial::putc('\n');
            for (uint32_t index = 0; index < local.outputCount; ++index) {
                serial::puts("Phase28J build output=");
                serial::puts(local.output[index].text);
                serial::putc('\n');
            }
        }
    ...
    auto phase28j_run_request = [](const gx_build_snapshot& build,
        ...
        if (debugControlled) {
            request.debugSourcePath = "src/main.cpp";
            request.debugSourceLine = 9;
            request.debugSourceCondition = condition;
        }
        return request;
    };

    auto phase28j_debug_request = [](const gx_build_snapshot& build,
                                     gx_development_run_handle handle,
                                     uint64_t generation, uint32_t command,
                                     const gx_development_debug_snapshot* stop) {
        gx_development_debug_request request = {};
    ...
    auto phase28j_wait_pause = [&](gx_development_run_handle handle,
                                  const gx_build_snapshot& build,
                                  uint64_t generation,
        ...
        for (uint32_t attempt = 0; attempt < 96; ++attempt) {
            ...
            if (run.state == GX_DEVELOPMENT_RUN_PAUSED) {
                gx_development_debug_request request = phase28j_debug_request(
                    build, handle, generation, GX_DEVELOPMENT_DEBUG_POLL, nullptr);
    ...
    auto phase28j_wait_complete = [&](gx_development_run_handle handle,
                                     bool* rendered) -> bool {
        for (uint32_t attempt = 0; attempt < 128; ++attempt) {
            ...
            if (native_elf_gui_runtime_snapshot(&gui) && gui.rendered &&
                equal_text(gui.content, "28J GUI 50")) {
    ...
    auto phase28j_drain_terminal = [](gx_development_run_handle handle) -> bool {
    ...
    auto phase28j_same_context = [](const gx_development_debug_register_context& left,
                                    const gx_development_debug_register_context& right) {
        return left.architecture == right.architecture && left.valid == right.valid &&
            left.sessionGeneration == right.sessionGeneration && left.stopGeneration == right.stopGeneration &&
            left.rip == right.rip && left.rflags == right.rflags && left.rsp == right.rsp &&
            left.rbp == right.rbp && left.rax == right.rax && left.rbx == right.rbx &&
            left.rcx == right.rcx && left.rdx == right.rdx && left.rsi == right.rsi &&
            left.rdi == right.rdi && left.r8 == right.r8 && left.r9 == right.r9 &&
            left.r10 == right.r10 && left.r11 == right.r11 && left.r12 == right.r12 &&
            left.r13 == right.r13 && left.r14 == right.r14 && left.r15 == right.r15;
    };

    gx_build_snapshot j28Build = {};
    const bool j28BuildPass = phase28j_build(&j28Build) &&
        j28Build.state == GX_BUILD_SUCCEEDED && j28Build.artifactValid != 0 &&
        j28Build.artifactSize != 0 && equal_text(j28Build.artifactArchitecture, "amd64");
    if (j28BuildPass) serial::puts("DEVELOPER_STUDIO_PHASE28J_BUILD_PASS\n");
    gx_build_snapshot j28WarmBuild = {};
    const bool j28WarmBuildPass = j28BuildPass && phase28j_build(&j28WarmBuild) &&
        j28WarmBuild.state == GX_BUILD_SUCCEEDED && j28WarmBuild.cachedModuleCount != 0;
    if (j28WarmBuildPass) serial::puts("DEVELOPER_STUDIO_PHASE28J_WARM_CACHE_PASS\n");

    const char* j28Condition = "((input - 1) * (input - 2)) * (input - 4)";
    NativeDebugWatchResult j28Validation = {};
    const bool j28ConditionParse = native_debug_watch_validate_expression(
        j28Condition, &j28Validation) && j28Validation.status == NativeDebugWatchStatus::Success;
    if (j28ConditionParse) serial::puts("DEVELOPER_STUDIO_PHASE28J_CONDITION_PARSE_PASS\n");

    bool j28BreakpointConfig = false;
    bool j28FalseHits = false;
    bool j28FalseContinue = false;
    bool j28Rearm = false;
    bool j28TrueHit = false;
    bool j28ConditionValue = false;
    bool j28Watch = false;
    bool j28Paused = false;
    bool j28ReadOnly = false;
    bool j28Stale = false;
    bool j28Locals = false;
    bool j28CallStack = false;
    bool j28Resume = false;
    bool j28Rendered = false;
    bool j28Closed = false;
    bool j28Clean = false;
    bool j28Step = false;
    if (j28WarmBuildPass && j28ConditionParse) {
        gx_development_run_request runRequest = phase28j_run_request(j28WarmBuild, j28Condition, true);
        gx_development_run_handle handle = 0;
        gx_development_run_snapshot prepared = {};
        prepared.size = sizeof(prepared);
        const gx_result prepareResult = NativeElfRunService::prepare(runRequest, &handle, &prepared);
        j28BreakpointConfig = prepareResult == GX_OK && handle != 0;
        if (!j28BreakpointConfig) {
            serial::puts("Phase28J prepare failed result=");
            print_decimal(static_cast<uint32_t>(prepareResult));
            serial::puts(" error=");
            print_decimal(prepared.errorCode);
            serial::puts(" message=");
            serial::puts(prepared.errorMessage);
            serial::putc('\n');
        }
        if (j28BreakpointConfig) serial::puts("DEVELOPER_STUDIO_PHASE28J_BREAKPOINT_CONFIG_PASS\n");
        const uint64_t generation = prepared.generation;
        set_gui_automation_close(true);
        const bool started = j28BreakpointConfig && NativeElfRunService::start(handle) == GX_OK;
        gx_development_debug_snapshot stop = {};
        const bool paused = started && phase28j_wait_pause(handle, j28WarmBuild, generation, &stop);
        j28Paused = paused && stop.pauseReason == GX_DEVELOPMENT_DEBUG_PAUSE_REASON_CONDITIONAL_SOURCE_BREAKPOINT &&
            stop.sourceMappingValid != 0 && stop.sourceLine == 9 && stop.rawTrapRip == stop.targetAddress + 1ULL &&
            stop.bindingInstalled == 0 && stop.conditionStatus == GX_DEVELOPMENT_DEBUG_CONDITION_STATUS_TRUE;
        j28TrueHit = paused && stop.conditionTrueHitCount == 1;
        j28FalseHits = paused && stop.conditionFalseHitCount == 2;
        j28FalseContinue = j28FalseHits;
        j28Rearm = j28FalseHits;
        j28ConditionValue = paused && stop.conditionResultKind == GX_DEVELOPMENT_DEBUG_EXPRESSION_RESULT_SIGNED_INTEGER &&
            stop.conditionSignedValue == -2;
        if (j28FalseHits) {
            serial::puts("DEVELOPER_STUDIO_PHASE28J_FALSE_HIT_1_PASS\n");
            serial::puts("DEVELOPER_STUDIO_PHASE28J_FALSE_CONTINUE_1_PASS\n");
            serial::puts("DEVELOPER_STUDIO_PHASE28J_REARM_1_PASS\n");
            serial::puts("DEVELOPER_STUDIO_PHASE28J_FALSE_HIT_2_PASS\n");
            serial::puts("DEVELOPER_STUDIO_PHASE28J_REARM_2_PASS\n");
        }
        if (j28TrueHit) serial::puts("DEVELOPER_STUDIO_PHASE28J_TRUE_HIT_PASS\n");
        if (j28ConditionValue) serial::puts("DEVELOPER_STUDIO_PHASE28J_CONDITION_VALUE_PASS\n");
        if (j28Paused) serial::puts("DEVELOPER_STUDIO_PHASE28J_PAUSED_PASS\n");

        gx_development_debug_request expressionRequest = phase28j_debug_request(
            j28WarmBuild, handle, generation, GX_DEVELOPMENT_DEBUG_EVALUATE_EXPRESSION, &stop);
        expressionRequest.expression = j28Condition;
        gx_development_debug_expression expressionResult = {};
        expressionResult.size = sizeof(expressionResult);
        j28Watch = paused && NativeElfRunService::evaluate_expression(
            expressionRequest, &expressionResult) == GX_OK &&
            expressionResult.status == GX_DEVELOPMENT_DEBUG_EXPRESSION_STATUS_SUCCESS &&
            expressionResult.resultKind == GX_DEVELOPMENT_DEBUG_EXPRESSION_RESULT_SIGNED_INTEGER &&
            expressionResult.signedValue == -2;
        if (j28Watch) serial::puts("DEVELOPER_STUDIO_PHASE28J_WATCH_EQUIVALENCE_PASS\nDEVELOPER_STUDIO_PHASE28J_WATCH_REGRESSION_PASS\n");

        gx_development_debug_variables locals = {};
        locals.size = sizeof(locals);
        const bool localsQuery = paused && NativeElfRunService::inspect_variables(
            phase28j_debug_request(j28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_INSPECT_VARIABLES, &stop), &locals) == GX_OK;
        for (uint32_t index = 0; localsQuery && index < locals.variableCount; ++index) {
            if (equal_text(locals.variables[index].name, "input") &&
                locals.variables[index].signedValue == 3) j28Locals = true;
        }
        if (j28Locals) serial::puts("DEVELOPER_STUDIO_PHASE28J_LOCALS_REGRESSION_PASS\n");

        gx_development_debug_call_stack stack = {};
        stack.size = sizeof(stack);
        j28CallStack = paused && NativeElfRunService::call_stack(
            phase28j_debug_request(j28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_CALL_STACK, &stop), &stack) == GX_OK &&
            stack.status == GX_DEVELOPMENT_DEBUG_CALL_STACK_STATUS_SUCCESS && stack.frameCount != 0;
        if (j28CallStack) serial::puts("DEVELOPER_STUDIO_PHASE28J_CALL_STACK_REGRESSION_PASS\n");

        gx_development_debug_snapshot afterWatch = {};
        afterWatch.size = sizeof(afterWatch);
        const bool readOnlyPoll = paused && NativeElfRunService::debug(
            phase28j_debug_request(j28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_POLL, &stop), &afterWatch) == GX_OK;
        j28ReadOnly = readOnlyPoll && phase28j_same_context(stop.context, afterWatch.context) &&
            afterWatch.context.rip == stop.context.rip;
        if (j28ReadOnly) serial::puts("DEVELOPER_STUDIO_PHASE28J_READ_ONLY_PASS\n");

        gx_development_debug_request staleRequest = phase28j_debug_request(
            j28WarmBuild, handle, generation + 1ULL, GX_DEVELOPMENT_DEBUG_EVALUATE_EXPRESSION, &stop);
        staleRequest.expression = j28Condition;
        gx_development_debug_expression stale = {};
        stale.size = sizeof(stale);
        j28Stale = paused && NativeElfRunService::evaluate_expression(staleRequest, &stale) == GX_ERROR_FAILED &&
            stale.status == GX_DEVELOPMENT_DEBUG_EXPRESSION_STATUS_STALE;
        if (j28Stale) serial::puts("DEVELOPER_STUDIO_PHASE28J_STALE_PASS\n");

        gx_development_debug_snapshot resumed = {};
        j28Resume = paused && NativeElfRunService::debug(
            phase28j_debug_request(j28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_RESUME, &stop), &resumed) == GX_OK;
        if (j28Resume) serial::puts("DEVELOPER_STUDIO_PHASE28J_RESUME_REARM_PASS\n");
        const bool completed = j28Resume && phase28j_wait_complete(handle, &j28Rendered);
        j28Closed = completed;
        if (j28Rendered) serial::puts("DEVELOPER_STUDIO_PHASE28J_RENDER_PASS\n");
        if (j28Closed) serial::puts("DEVELOPER_STUDIO_PHASE28J_CLOSE_PASS\n");
        const bool released = NativeElfRunService::release(handle) == GX_OK;
        set_gui_automation_close(false);
        j28Clean = released && !NativeElfDevelopmentAppModel::has_active_registration() &&
            compositor::KernelCompositor::getWindowCount() == 0;
        if (j28Clean) serial::puts("DEVELOPER_STUDIO_PHASE28J_CLEANUP_PASS\n");
    }

    bool j28ConstantFalse = false;
    {
        gx_development_run_request request = phase28j_run_request(j28WarmBuild, "0", true);
        gx_development_run_handle handle = 0;
        gx_development_run_snapshot prepared = {};
        prepared.size = sizeof(prepared);
        set_gui_automation_close(true);
        const bool preparedOk = j28WarmBuildPass && NativeElfRunService::prepare(request, &handle, &prepared) == GX_OK;
        const bool started = preparedOk && NativeElfRunService::start(handle) == GX_OK;
        bool rendered = false;
        j28ConstantFalse = started && phase28j_wait_complete(handle, &rendered);
        if (j28ConstantFalse) serial::puts("DEVELOPER_STUDIO_PHASE28J_CONSTANT_FALSE_PASS\n");
        (void)NativeElfRunService::release(handle);
        set_gui_automation_close(false);
    }

    bool j28ConstantTrue = false;
    {
        gx_development_run_request request = phase28j_run_request(j28WarmBuild, "1", true);
        gx_development_run_handle handle = 0;
        gx_development_run_snapshot prepared = {};
        prepared.size = sizeof(prepared);
        set_gui_automation_close(false);
        ...
        gx_development_debug_snapshot stop = {};
        const bool paused = started && phase28j_wait_pause(handle, j28WarmBuild, generation, &stop);
        j28ConstantTrue = paused && stop.conditionStatus == GX_DEVELOPMENT_DEBUG_CONDITION_STATUS_TRUE &&
            stop.conditionFalseHitCount == 0 && stop.conditionTrueHitCount == 1;
        if (j28ConstantTrue) serial::puts("DEVELOPER_STUDIO_PHASE28J_CONSTANT_TRUE_PASS\n");
        gx_development_debug_snapshot stepped = {};
        const bool stepRequested = paused && NativeElfRunService::debug(
            phase28j_debug_request(j28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_STEP_INSTRUCTION, &stop), &stepped) == GX_OK;
        bool stepFinished = stepRequested && stepped.status == GX_DEVELOPMENT_DEBUG_STATUS_TRAP;
        if (stepRequested && !stepFinished)
            stepFinished = phase28j_wait_pause(handle, j28WarmBuild, generation, &stepped);
        j28Step = stepFinished && stepped.status == GX_DEVELOPMENT_DEBUG_STATUS_TRAP;
        if (j28Step) serial::puts("DEVELOPER_STUDIO_PHASE28J_STEP_REGRESSION_PASS\n");
        gx_development_debug_snapshot cancelled = {};
        const gx_development_debug_snapshot* cancelStop = stepRequested ? &stepped : &stop;
        (void)NativeElfRunService::debug(
            phase28j_debug_request(j28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_CANCEL_EXECUTION, cancelStop), &cancelled);
        (void)phase28j_drain_terminal(handle);
        (void)NativeElfRunService::release(handle);
    }

    bool j28ConditionError = false;
    {
        gx_development_run_request request = phase28j_run_request(j28WarmBuild, "1 / 0", true);
        gx_development_run_handle handle = 0;
        gx_development_run_snapshot prepared = {};
        prepared.size = sizeof(prepared);
        const bool preparedOk = j28WarmBuildPass && NativeElfRunService::prepare(request, &handle, &prepared) == GX_OK;
        ...
        gx_development_debug_snapshot stop = {};
        const bool paused = started && phase28j_wait_pause(handle, j28WarmBuild, generation, &stop);
        j28ConditionError = paused && stop.conditionStatus == GX_DEVELOPMENT_DEBUG_CONDITION_STATUS_ERROR &&
            stop.conditionErrorCategory == GX_DEVELOPMENT_DEBUG_EXPRESSION_ERROR_DIVIDE_BY_ZERO;
        if (j28ConditionError) serial::puts("DEVELOPER_STUDIO_PHASE28J_CONDITION_ERROR_PASS\n");
        if (paused) {
            gx_development_debug_snapshot cancelled = {};
            (void)NativeElfRunService::debug(
                phase28j_debug_request(j28WarmBuild, handle, generation,
                                       GX_DEVELOPMENT_DEBUG_CANCEL_EXECUTION, &stop), &cancelled);
            (void)phase28j_drain_terminal(handle);
        } else if (handle != 0) {
            (void)NativeElfRunService::cancel(handle);
            (void)phase28j_drain_terminal(handle);
        }
        (void)NativeElfRunService::release(handle);
    }

    gx_development_run_handle invalidHandle = 0;
    gx_development_run_snapshot invalidSnapshot = {};
    invalidSnapshot.size = sizeof(invalidSnapshot);
    gx_development_run_request invalidRequest = phase28j_run_request(
        j28WarmBuild, "input + * 2", true);
    const gx_result invalidResult = j28WarmBuildPass &&
        NativeElfRunService::prepare(invalidRequest, &invalidHandle, &invalidSnapshot);
    const bool j28SyntaxRejected = j28WarmBuildPass &&
        invalidResult == GX_OK && invalidHandle == 0 &&
        invalidSnapshot.state == GX_DEVELOPMENT_RUN_FAILED &&
        invalidSnapshot.errorCode == GX_DEVELOPMENT_RUN_ERROR_INVALID_REQUEST;

    bool j28RunRegression = false;
    {
        gx_development_run_request request = phase28j_run_request(j28WarmBuild, nullptr, false);
        gx_development_run_handle handle = 0;
        gx_development_run_snapshot prepared = {};
        prepared.size = sizeof(prepared);
        set_gui_automation_close(false);
        ...
        bool rendered = false;
        if (started) {
            for (uint32_t attempt = 0; attempt < 64 && !rendered; ++attempt) {
                running = {};
                running.size = sizeof(running);
                if (NativeElfRunService::poll(handle, &running) != GX_OK) break;
                if (native_elf_gui_runtime_snapshot(&gui) && gui.active && gui.rendered &&
                    equal_text(gui.content, "28J GUI 50")) {
                    rendered = true;
                    break;
                }
                if (running.state == GX_DEVELOPMENT_RUN_RUNNING &&
                    NativeElfRunService::pump(handle) != GX_OK) break;
            }
        }
        const bool closeRequested = rendered && NativeElfRunService::request_close(handle) == GX_OK;
        gx_development_run_snapshot completed = {};
        ...
        const bool clean = released && !NativeElfDevelopmentAppModel::has_active_registration() &&
            compositor::KernelCompositor::getWindowCount() == 0;
        j28RunRegression = rendered && complete && clean;
        if (j28RunRegression) serial::puts("DEVELOPER_STUDIO_PHASE28J_RUN_REGRESSION_PASS\n");
        set_gui_automation_close(false);
    }

    const bool j28Artifact = j28BuildPass && emit_serial_artifact(
        "/P28J/build/bin/amd64/p28j.elf", "j28main");
    const bool phase28jPassed = j28BuildPass && j28WarmBuildPass && j28BreakpointConfig &&
        j28ConditionParse && j28FalseHits && j28FalseContinue && j28Rearm && j28TrueHit &&
        j28ConditionValue && j28Watch && j28Paused && j28ReadOnly && j28Stale &&
        j28Locals && j28CallStack && j28Resume && j28Rendered && j28Closed && j28Clean &&
        j28ConstantFalse && j28ConstantTrue && j28ConditionError && j28SyntaxRejected &&
        j28Step && j28RunRegression && j28Artifact &&
        !NativeElfDevelopmentAppModel::has_active_registration() &&
        compositor::KernelCompositor::getWindowCount() == 0;
    print_marker("phase28j_build", j28BuildPass);
    print_marker("phase28j_condition_parse", j28ConditionParse);
    print_marker("phase28j_false_hits", j28FalseHits);
    print_marker("phase28j_true_hit", j28TrueHit);
    print_marker("phase28j_watch", j28Watch);
    print_marker("phase28j_constant_false", j28ConstantFalse);
    print_marker("phase28j_constant_true", j28ConstantTrue);
    print_marker("phase28j_condition_error", j28ConditionError);
    print_marker("phase28j", phase28jPassed);
...
#if defined(GXOS_PHASE28M_SMOKE)
{
    ...
    serial::puts("ELF Loader: Phase 28L hit-count and logpoint smoke begin\n");
    serial::puts("DEVELOPER_STUDIO_PHASE28L_BEGIN\n");

    auto phase28l_build = [](gx_build_snapshot* snapshot) -> bool {
        gx_build_request request = {};
        request.size = sizeof(request);
        request.version = GX_BUILD_API_VERSION;
        request.projectRoot = "/P28L";
        request.projectId = "dev.guidexos.phase28l";
        request.projectKind = "native-gui-application";
        request.targetProfile = "guidexos.amd64.baremetal.bootstrap.native";
        request.buildSystem = "guidexos-native-baremetal-bootstrap-v1";
        request.expectedArtifact = "build/bin/amd64/p28l.elf";
        request.configuration = "Debug";
        gx_build_handle buildHandle = 0;
        if (compiler::BareMetalBuildService::start(&request, &buildHandle) != GX_OK) return false;
        gx_build_snapshot local = {};
    ...
    auto phase28l_run_request = [](const gx_build_snapshot& build) {
        gx_development_run_request request = {};
        request.size = sizeof(request);
        request.version = GX_DEVELOPMENT_RUN_API_VERSION;
        request.projectRoot = "/P28L";
        ...
        request.debugSourceCondition = "input - 1";
        return request;
    };

    auto phase28l_debug_request = [](const gx_build_snapshot& build,
                                     gx_development_run_handle handle,
                                     uint64_t generation, uint32_t command,
                                     const gx_development_debug_snapshot* stop) {
        gx_development_debug_request request = {};
        request.size = GX_DEVELOPMENT_DEBUG_REQUEST_POLICY_BYTES;
        request.version = GX_DEVELOPMENT_DEBUG_API_VERSION;
        request.command = command;
        request.handle = handle;
        request.sessionGeneration = generation;
    ...
    auto phase28l_wait_pause = [&](gx_development_run_handle handle,
                                   const gx_build_snapshot& build,
                                   uint64_t generation,
                                   gx_development_debug_snapshot* stop) -> bool {
        for (uint32_t attempt = 0; attempt < 192; ++attempt) {
            ...
            if (NativeElfRunService::poll(handle, &run) != GX_OK) return false;
            if (run.state == GX_DEVELOPMENT_RUN_PAUSED) {
                gx_development_debug_request request = phase28l_debug_request(
                    build, handle, generation, GX_DEVELOPMENT_DEBUG_POLL, nullptr);
                stop->size = sizeof(*stop);
                return NativeElfRunService::debug(request, stop) == GX_OK &&
                    stop->status == GX_DEVELOPMENT_DEBUG_STATUS_TRAP;
    ...
    auto phase28l_wait_complete = [&](gx_development_run_handle handle,
                                      bool* rendered) -> bool {
        for (uint32_t attempt = 0; attempt < 192; ++attempt) {
            gx_development_run_snapshot run = {};
            run.size = sizeof(run);
            ...
            NativeElfGuiRuntimeSnapshot gui = {};
            if (native_elf_gui_runtime_snapshot(&gui) && gui.rendered &&
                equal_text(gui.content, "28L GUI 9")) {
                if (rendered) *rendered = true;
            }
            if (run.state == GX_DEVELOPMENT_RUN_COMPLETED) return run.cleanupComplete != 0;
            if (run.state == GX_DEVELOPMENT_RUN_FAILED ||
    ...
    gx_build_snapshot l28Build = {};
    const bool l28BuildPass = phase28l_build(&l28Build) &&
        l28Build.state == GX_BUILD_SUCCEEDED && l28Build.artifactValid != 0 &&
        l28Build.artifactSize != 0 && equal_text(l28Build.artifactArchitecture, "amd64");
    if (l28BuildPass) serial::puts("DEVELOPER_STUDIO_PHASE28L_BUILD_PASS\n");
    gx_build_snapshot l28WarmBuild = {};
    const bool l28WarmBuildPass = l28BuildPass && phase28l_build(&l28WarmBuild) &&
        l28WarmBuild.state == GX_BUILD_SUCCEEDED && l28WarmBuild.cachedModuleCount != 0;

    bool l28Configure = false;
    bool l28Condition = false;
    bool l28Policy = false;
    bool l28Output = false;
    bool l28Active = false;
    bool l28Coexist = false;
    bool l28Complete = false;
    bool l28Generation = false;
    bool l28SecondRun = false;
    bool l28Clean = false;

    if (l28WarmBuildPass) {
        gx_development_run_request runRequest = phase28l_run_request(l28WarmBuild);
        gx_development_run_handle handle = 0;
        gx_development_run_snapshot prepared = {};
        prepared.size = sizeof(prepared);
        const bool preparedOk = NativeElfRunService::prepare(
            runRequest, &handle, &prepared) == GX_OK;
        const uint64_t generation = prepared.generation;

        gx_development_debug_snapshot initialList = {};
        initialList.size = sizeof(initialList);
        const bool initialListed = preparedOk && NativeElfRunService::debug(
            phase28l_debug_request(l28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_LIST_SOURCE_BREAKPOINTS, nullptr),
            &initialList) == GX_OK && initialList.breakpointCount == 1;
        const uint64_t logId = initialListed ? initialList.breakpoints[0].breakpointId : 0;

        gx_development_debug_request configureRequest = phase28l_debug_request(
            l28WarmBuild, handle, generation,
            GX_DEVELOPMENT_DEBUG_CONFIGURE_SOURCE_BREAKPOINT_POLICY, nullptr);
        configureRequest.breakpointId = logId;
        configureRequest.breakpointAction = GX_DEVELOPMENT_DEBUG_BREAKPOINT_ACTION_LOG;
        configureRequest.hitCountPolicy = GX_DEVELOPMENT_DEBUG_HIT_COUNT_POLICY_NONE;
        configureRequest.hitCountThreshold = 0;
        configureRequest.logTemplate = "input={input} doubled={input * 2}";
        gx_development_debug_snapshot configured = {};
        configured.size = sizeof(configured);
        l28Configure = initialListed && NativeElfRunService::debug(
            configureRequest, &configured) == GX_OK &&
            configured.breakpointOperationStatus ==
                GX_DEVELOPMENT_DEBUG_BREAKPOINT_STATUS_SUCCESS &&
            configured.breakpoints[0].action == GX_DEVELOPMENT_DEBUG_BREAKPOINT_ACTION_LOG;
        if (l28Configure) serial::puts("DEVELOPER_STUDIO_PHASE28L_CONFIGURE_PASS\n");

        gx_development_debug_request countRequest = phase28l_debug_request(
            l28WarmBuild, handle, generation,
            GX_DEVELOPMENT_DEBUG_ADD_SOURCE_BREAKPOINT, nullptr);
        countRequest.sourcePath = "src/helper.cpp";
        countRequest.sourceLine = 5;
        countRequest.breakpointAction = GX_DEVELOPMENT_DEBUG_BREAKPOINT_ACTION_BREAK;
        countRequest.hitCountPolicy = GX_DEVELOPMENT_DEBUG_HIT_COUNT_POLICY_EQUAL;
        countRequest.hitCountThreshold = 3;
        gx_development_debug_snapshot countAdded = {};
        countAdded.size = sizeof(countAdded);
        const bool countAddedOk = l28Configure && NativeElfRunService::debug(
            countRequest, &countAdded) == GX_OK && countAdded.bindingId != 0;
        const uint64_t countId = countAddedOk ? countAdded.bindingId : 0;

        gx_development_debug_request coexistingRequest = phase28l_debug_request(
            l28WarmBuild, handle, generation,
            GX_DEVELOPMENT_DEBUG_ADD_SOURCE_BREAKPOINT, nullptr);
        coexistingRequest.sourcePath = "src/main.cpp";
        coexistingRequest.sourceLine = 12;
        coexistingRequest.breakpointAction = GX_DEVELOPMENT_DEBUG_BREAKPOINT_ACTION_BREAK;
        coexistingRequest.hitCountPolicy = GX_DEVELOPMENT_DEBUG_HIT_COUNT_POLICY_NONE;
        gx_development_debug_snapshot coexistingAdded = {};
        coexistingAdded.size = sizeof(coexistingAdded);
        const bool coexistingAddedOk = countAddedOk && NativeElfRunService::debug(
            coexistingRequest, &coexistingAdded) == GX_OK && coexistingAdded.bindingId != 0;
        const uint64_t coexistingId = coexistingAddedOk ? coexistingAdded.bindingId : 0;

        gx_development_debug_snapshot activeList = {};
        activeList.size = sizeof(activeList);
        const bool activeListed = coexistingAddedOk && NativeElfRunService::debug(
            phase28l_debug_request(l28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_LIST_SOURCE_BREAKPOINTS, nullptr),
            &activeList) == GX_OK;
        uint32_t enabledCount = 0;
        for (uint32_t index = 0; activeListed && index < activeList.breakpointCount; ++index)
            if (activeList.breakpoints[index].enabled != 0) ++enabledCount;
        l28Active = activeListed && activeList.breakpointCount == 3 && enabledCount == 3;
        if (l28Active) serial::puts("DEVELOPER_STUDIO_PHASE28L_THREE_ACTIVE_PASS\n");

        set_gui_automation_close(true);
        const bool started = l28Active && NativeElfRunService::start(handle) == GX_OK;
        gx_development_debug_snapshot stop = {};
        const bool paused = started && phase28l_wait_pause(handle, l28WarmBuild, generation, &stop);
        gx_development_debug_snapshot stoppedList = {};
        stoppedList.size = sizeof(stoppedList);
        const bool listedStopped = paused && NativeElfRunService::debug(
            phase28l_debug_request(l28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_LIST_SOURCE_BREAKPOINTS, &stop),
            &stoppedList) == GX_OK;
        const gx_development_debug_breakpoint* logRecord = nullptr;
        const gx_development_debug_breakpoint* countRecord = nullptr;
        for (uint32_t index = 0; listedStopped && index < stoppedList.breakpointCount; ++index) {
            const gx_development_debug_breakpoint& record = stoppedList.breakpoints[index];
            if (record.breakpointId == logId) logRecord = &record;
            if (record.breakpointId == countId) countRecord = &record;
        }
        l28Condition = paused && stop.bindingId == countId && stop.sourceLine == 5 &&
            stop.pauseReason == GX_DEVELOPMENT_DEBUG_PAUSE_REASON_SOURCE_BREAKPOINT &&
            logRecord && logRecord->rawHitCount == 3 && logRecord->falseHitCount >= 1 &&
            logRecord->trueHitCount >= 2;
        if (l28Condition) serial::puts("DEVELOPER_STUDIO_PHASE28L_CONDITION_LOG_PASS\n");

        gx_development_debug_snapshot drained = {};
        drained.size = sizeof(drained);
        const bool drainedOk = paused && NativeElfRunService::debug(
            phase28l_debug_request(l28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_DRAIN_OUTPUT, &stop),
            &drained) == GX_OK;
        l28Output = drainedOk && drained.outputOperationStatus ==
            GX_DEVELOPMENT_DEBUG_OUTPUT_STATUS_SUCCESS && drained.outputCount == 2 &&
            drained.output[0].breakpointId == logId &&
            equal_text(drained.output[0].text, "input=2 doubled=4") &&
            drained.output[1].breakpointId == logId &&
            equal_text(drained.output[1].text, "input=3 doubled=6") &&
            drained.output[0].rawHitCount == 2 && drained.output[1].rawHitCount == 3;
        if (l28Output) serial::puts("DEVELOPER_STUDIO_PHASE28L_OUTPUT_FIFO_PASS\n");
        l28Policy = paused && countRecord && countRecord->rawHitCount == 3 &&
            countRecord->action == GX_DEVELOPMENT_DEBUG_BREAKPOINT_ACTION_BREAK &&
            countRecord->hitCountPolicy == GX_DEVELOPMENT_DEBUG_HIT_COUNT_POLICY_EQUAL &&
            countRecord->hitCountThreshold == 3 &&
            countRecord->installed == 0;
        if (l28Policy) serial::puts("DEVELOPER_STUDIO_PHASE28L_HIT_COUNT_PASS\n");

        gx_development_debug_snapshot resumed = {};
        const bool resumedFromCount = paused && NativeElfRunService::debug(
            phase28l_debug_request(l28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_RESUME, &stop), &resumed) == GX_OK;
        gx_development_debug_snapshot coexistingStop = {};
        const bool coexistingPaused = resumedFromCount && phase28l_wait_pause(
            handle, l28WarmBuild, generation, &coexistingStop);
        gx_development_debug_snapshot coexistingStoppedList = {};
        coexistingStoppedList.size = sizeof(coexistingStoppedList);
        const bool listedCoexisting = coexistingPaused && NativeElfRunService::debug(
            phase28l_debug_request(l28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_LIST_SOURCE_BREAKPOINTS,
                                   &coexistingStop),
            &coexistingStoppedList) == GX_OK;
        const gx_development_debug_breakpoint* coexistingRecordAfter = nullptr;
        for (uint32_t index = 0; listedCoexisting && index < coexistingStoppedList.breakpointCount; ++index) {
            if (coexistingStoppedList.breakpoints[index].breakpointId == coexistingId)
                coexistingRecordAfter = &coexistingStoppedList.breakpoints[index];
        }
        l28Coexist = coexistingPaused && coexistingStop.bindingId == coexistingId &&
            coexistingStop.sourceLine == 12 &&
            coexistingStop.pauseReason == GX_DEVELOPMENT_DEBUG_PAUSE_REASON_SOURCE_BREAKPOINT &&
            coexistingRecordAfter && coexistingRecordAfter->rawHitCount == 1;
        if (l28Coexist) serial::puts("DEVELOPER_STUDIO_PHASE28L_COEXISTING_BREAK_PASS\n");

        gx_development_debug_snapshot resumedFromCoexisting = {};
        const bool resumedCoexisting = coexistingPaused && NativeElfRunService::debug(
            phase28l_debug_request(l28WarmBuild, handle, generation,
                                   GX_DEVELOPMENT_DEBUG_RESUME, &coexistingStop),
            &resumedFromCoexisting) == GX_OK;
        bool rendered = false;
        l28Complete = resumedCoexisting && phase28l_wait_complete(handle, &rendered) && rendered;
        if (l28Complete) serial::puts("DEVELOPER_STUDIO_PHASE28L_GUI_COMPLETE_PASS\n");
        const bool released = NativeElfRunService::release(handle) == GX_OK;
        set_gui_automation_close(false);
        l28Clean = released && !NativeElfDevelopmentAppModel::has_active_registration() &&
            compositor::KernelCompositor::getWindowCount() == 0;
        if (l28Clean) serial::puts("DEVELOPER_STUDIO_PHASE28L_CLEANUP_PASS\n");

        gx_development_run_handle secondHandle = 0;
        gx_development_run_snapshot secondPrepared = {};
        secondPrepared.size = sizeof(secondPrepared);
        const bool secondPreparedOk = NativeElfRunService::prepare(
            phase28l_run_request(l28WarmBuild), &secondHandle, &secondPrepared) == GX_OK;
        const uint64_t secondGeneration = secondPrepared.generation;
        gx_development_debug_snapshot secondList = {};
        secondList.size = sizeof(secondList);
        const bool secondListed = secondPreparedOk && NativeElfRunService::debug(
            phase28l_debug_request(l28WarmBuild, secondHandle, secondGeneration,
                                   GX_DEVELOPMENT_DEBUG_LIST_SOURCE_BREAKPOINTS, nullptr),
            &secondList) == GX_OK;
        gx_development_debug_snapshot secondDrain = {};
        secondDrain.size = sizeof(secondDrain);
        const bool secondDrained = secondPreparedOk && NativeElfRunService::debug(
            phase28l_debug_request(l28WarmBuild, secondHandle, secondGeneration,
                                   GX_DEVELOPMENT_DEBUG_DRAIN_OUTPUT, nullptr),
            &secondDrain) == GX_OK;
        l28Generation = secondPreparedOk && secondGeneration != generation && secondListed &&
            secondList.breakpointCount == 1 && secondList.breakpoints[0].rawHitCount == 0 &&
            secondDrained && secondDrain.outputOperationStatus ==
                GX_DEVELOPMENT_DEBUG_OUTPUT_STATUS_SUCCESS && secondDrain.outputCount == 0;
        if (l28Generation) serial::puts("DEVELOPER_STUDIO_PHASE28L_GENERATION_RESET_PASS\n");
        gx_development_debug_request errorConfigureRequest = phase28l_debug_request(
            l28WarmBuild, secondHandle, secondGeneration,
            GX_DEVELOPMENT_DEBUG_CONFIGURE_SOURCE_BREAKPOINT_POLICY, nullptr);
        errorConfigureRequest.breakpointId = secondListed && secondList.breakpointCount == 1
            ? secondList.breakpoints[0].breakpointId : 0;
        errorConfigureRequest.breakpointAction = GX_DEVELOPMENT_DEBUG_BREAKPOINT_ACTION_LOG;
        errorConfigureRequest.hitCountPolicy = GX_DEVELOPMENT_DEBUG_HIT_COUNT_POLICY_NONE;
        errorConfigureRequest.logTemplate = "value={missing}";
        gx_development_debug_snapshot errorConfigured = {};
        errorConfigured.size = sizeof(errorConfigured);
        const bool errorConfigure = secondListed && NativeElfRunService::debug(
            errorConfigureRequest, &errorConfigured) == GX_OK &&
            errorConfigured.breakpointOperationStatus ==
                GX_DEVELOPMENT_DEBUG_BREAKPOINT_STATUS_SUCCESS;
        set_gui_automation_close(true);
        const bool secondStarted = errorConfigure &&
            NativeElfRunService::start(secondHandle) == GX_OK;
        bool secondRendered = false;
        const bool secondCompleted = secondStarted &&
            phase28l_wait_complete(secondHandle, &secondRendered) && secondRendered;
        gx_development_debug_snapshot errorDrain = {};
        errorDrain.size = sizeof(errorDrain);
        const bool errorDrained = secondCompleted && NativeElfRunService::debug(
            phase28l_debug_request(l28WarmBuild, secondHandle, secondGeneration,
                                   GX_DEVELOPMENT_DEBUG_DRAIN_OUTPUT, nullptr),
            &errorDrain) == GX_OK;
        const uint32_t unknownIdentifier =
            GX_DEVELOPMENT_DEBUG_EXPRESSION_ERROR_UNKNOWN_IDENTIFIER;
        l28SecondRun = secondCompleted && errorDrained && errorDrain.outputCount == 2 &&
            errorDrain.output[0].errorCategory == unknownIdentifier &&
            errorDrain.output[1].errorCategory == unknownIdentifier &&
            equal_text(errorDrain.output[0].text, "value=<error:UNKNOWN_IDENTIFIER>") &&
            equal_text(errorDrain.output[1].text, "value=<error:UNKNOWN_IDENTIFIER>");
        if (l28SecondRun) serial::puts("DEVELOPER_STUDIO_PHASE28L_SECOND_RUN_PASS\n");
        (void)NativeElfRunService::release(secondHandle);
        set_gui_automation_close(false);
    }

    const bool l28Artifact = l28BuildPass && emit_serial_artifact(
        "/P28L/build/bin/amd64/p28l.elf", "l28main");
    const bool phase28lPassed = l28BuildPass && l28WarmBuildPass && l28Configure &&
        l28Condition && l28Policy && l28Output && l28Active && l28Coexist && l28Complete &&
        l28Generation && l28SecondRun && l28Clean && l28Artifact &&
        !NativeElfDevelopmentAppModel::has_active_registration() &&
        compositor::KernelCompositor::getWindowCount() == 0;
    print_marker("phase28l_build", l28BuildPass);
    print_marker("phase28l_configure", l28Configure);
    print_marker("phase28l_condition_log", l28Condition);
    print_marker("phase28l_hit_count", l28Policy);
    print_marker("phase28l_output_fifo", l28Output);
    print_marker("phase28l_three_active", l28Active);
    print_marker("phase28l_coexisting_break", l28Coexist);
    print_marker("phase28l_gui_complete", l28Complete);
    print_marker("phase28l_generation_reset", l28Generation);
    print_marker("phase28l_second_run", l28SecondRun);
    print_marker("phase28l", phase28lPassed);
    serial::puts(phase28lPassed ?
        "ELF Loader: Phase 28L hit-count and logpoint smoke PASS\nDEVELOPER_STUDIO_PHASE28L_PASS\n" :
        "ELF Loader: Phase 28L hit-count and logpoint smoke FAIL\n");
#endif
